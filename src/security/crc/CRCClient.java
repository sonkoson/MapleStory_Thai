...
public class CRCClient {
                ...
                if (crcHeartBeatTask != null) {
                    if (!isCRCHeartBeatStop()) {
                        if (!isCRCHeartBeat()) {
                            ...
                            System.err.println("[CRC Server] " + session.remoteAddress().toString().split(":")[0] + " 하트비트 우회 감지, 접속 끊기");
    ...
    public String getSystemSerialNumber() {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT id, systemSerialNumber FROM accounts WHERE id = " + getMapleClient().getAccID());
            rs = ps.executeQuery();
    ...
    public void saveSystemSerialNumbers(String tempSerialNumbers) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("UPDATE accounts SET systemSerialNumber = '" + tempSerialNumbers + "' WHERE id = " + getMapleClient().getAccID());
            ps.executeUpdate();
            ps.close();
            con.close();
        } catch (SQLException sqlEx) {
