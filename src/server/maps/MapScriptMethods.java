...
public class MapScriptMethods {
    ...
    public static void startScript_FirstUser(MapleClient c, String scriptName) {
        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            case first_kalosChaos1:
                MapleMonster Karlos = MapleLifeFactory.getMonster(8880800);
                c.getPlayer().getMap().spawnMonsterOnGroundBelow(Karlos, new Point(104, 398));
                break;
            case JinHillah_onFirstUserEnter: //여기가 작동
            ...
            case first_SlimeChaos1: {
                c.getPlayer().getClient().getSession().writeAndFlush(MobPacket.BossSlime.SlimePacket(9, c.getPlayer().getMap()));
                c.getPlayer().getClient().getSession().writeAndFlush(MobPacket.BossSlime.SlimePacket(7, c.getPlayer().getMap()));
                break;
            }
            case firstenter_bossBlackMage: { //[othello] 익스트림 보스
                Timer.EventTimer.getInstance().schedule(() -> {
                    boolean chaosBM = c.getPlayer().getMapId() / 1000 == 450017;
                    String chaosBMS = chaosBM ? "미쳐버린 " : "";
                    if (c.getPlayer().getMapId() == 450013100 || c.getPlayer().getMapId() == 450017100) {
                        c.getPlayer().getMap().broadcastMessage(CField.enforceMSG(chaosBMS + "검은 마법사와 대적하기 위해서는 그를 호위하는 " + chaosBMS + "창조와 파괴의 기사들을 물리쳐야 한다.", 265, 8000));
                    } else if (c.getPlayer().getMapId() == 450013300 || c.getPlayer().getMapId() == 450017300) {
                        c.getPlayer().getMap().broadcastMessage(CField.enforceMSG("드디어 " + chaosBMS + "검은 마법사의 앞에 바로 섰다. 모든 힘을 다해 그를 물리치자.", 265, 8000));
                    } else if (c.getPlayer().getMapId() == 450013500 || c.getPlayer().getMapId() == 450017500) {
                    ...
                    if (c.getPlayer().getMapId() == 450013100 || c.getPlayer().getMapId() == 450017100) {
                        c.getPlayer().getMap().broadcastMessage(CField.ImageTalkNpc(0, 4000, "이 지역에서 발생되는 공격은 " + chaosBMS + "창조나 파괴의 저주를 거는 것 같다. 만약 두 저주가 동시에 걸린다면 #b큰 피해#k를 입으니 조심하자"));
                    } else if (c.getPlayer().getMapId() == 450013300 || c.getPlayer().getMapId() == 450017300) {
                        c.getPlayer().getMap().broadcastMessage(CField.ImageTalkNpc(0, 4000, "이 지역에서 발생되는 공격은 " + chaosBMS + "창조나 파괴의 저주를 거는 것 같다. 만약 두 저주가 동시에 걸린다면 #b큰 피해#k를 입으니 조심하자"));
                    } else if (c.getPlayer().getMapId() == 450013500 || c.getPlayer().getMapId() == 450017500) {
                        c.getPlayer().getMap().broadcastMessage(CField.ImageTalkNpc(3003902, 4000, "#face1#가자. 나는 복수를, 너는 세계를 지키는 거야."));
                    } else if (c.getPlayer().getMapId() == 450013700 || c.getPlayer().getMapId() == 450017700) {
                        c.getPlayer().getMap().broadcastMessage(CField.ImageTalkNpc(0, 4000, "아무 것도 없는 공간…… 나 혼자 남은 것인가……"));
                    } else if (c.getPlayer().getMapId() == 450013750 || c.getPlayer().getMapId() == 450017750) {
                        c.getPlayer().getMap().broadcastMessage(CField.ImageTalkNpc(0, 5000, chaosBMS + "창세의 알을 파괴하여 기나긴 싸움을 마무리 하자."));
                    }
                }, 1000L);
                break;
            }
            ...
            case will_phase2: {
                ...
                for (MapleMonster m : c.getPlayer().getMap().getAllMonster()) {
                    if (m.getId() != 8880342 && m.getId() != 8880302) {
                        continue;
                    }
    ...
    public static void startScript_User(final MapleClient c, final String scriptName) {
        ...
        switch (onUserEnter.fromString(scriptName)) {
            ...
            case enter_450011990:
                c.getPlayer().cancelEffectFromBuffStat(SecondaryStat.DebuffIncHp);
                break;

            case JinHillah_onFirstUserEnter:
                c.getPlayer().getMap().broadcastMessage(CField.enforceMSG("영혼이 타오르는 양초를 힐라가 일정 시간마다 베어 없앨 것이다. 영혼을 빼앗기지 않게 조심하자.", 254, 8000));
                c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880402), new Point(0, 266));
                c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880401), new Point(0, 266));
                SkillFactory.getSkill(80002543).getEffect(1).applyTo(c.getPlayer(), false);
                break;
            ...
            case enter_121:
            case enter_120: {
                c.getPlayer().getClient().getSession().writeAndFlush(SLFCGPacket.PoloFrittoEffect(4, "Effect/EventEffect.img/SalonDebut/screenEff/1366"));
                c.getPlayer().getClient().getSession().writeAndFlush(SLFCGPacket.BlackLabel("#fs11##eWelcome :: Fancy 서버", 100, 1000, 4, 0, -250, 1, 4));
            }
            break;
            ...
            case enter_hungryMutoHard: {
                c.getSession().writeAndFlush((Object) CField.environmentChange("event/start", 21));
            ...
            case enter_101070000: {
                c.getPlayer().getClient().getSession().writeAndFlush(CField.environmentChange("Map/Effect.img/temaD/enter/fairyAcademy", 19));
                break;
            }
            case enter_993189100: {
                //c.getSession().writeAndFlush(SLFCGPacket.OnYellowDlg(9062004, 3000, "#b"+ c.getPlayer().getName() +"님#k #face1#오늘도 Fancy에서 좋은 하루 되세요.", ""));
                c.getPlayer().getClient().getSession().writeAndFlush(SLFCGPacket.BlackLabel("#fs11##eWelcome :: Fancy 서버", 100, 1000, 4, 0, -250, 1, 4));
                break;
    ...
    private static void sendDojoStart(MapleClient c, int stage) {
        c.getSession().writeAndFlush(CField.environmentChange("Dojang/startd", 7));
        c.getSession().writeAndFlush(CField.environmentChange("dojang/start/stage", 19));
        c.getSession().writeAndFlush(CField.environmentChange("dojang/start/number/" + stage, 21));
