...
public class AramiaFireWorks {
      ...
      switch (reactor.getState()) {
        ...
        for (ChannelServer cs : ChannelServer.getAllInstances()) {
            for (MapleCharacter chr : cs.getPlayerStorage().getAllCharacters().values())
                chr.setKeyValue(501092, "point", "" + this.sunshines);
        }
        if (this.sunshines >= 1000) {
            this.sunshines = 0;
    ...
    public final short getSuns() {
        return (short) sunshines;
    }
    private final void broadcastSun(final MapleCharacter c) {
        ...
        World.Broadcast.broadcastMessage(CField.getGameMessage(8, "경험치 피버 타임이 시작됩니다! (1시간)"));
        Timer.EventTimer.getInstance().schedule(new Runnable() {
            public final void run() {
                for (ChannelServer ch : ChannelServer.getAllInstances()) {
                    ch.사냥버닝타임(false);
                    World.Broadcast.broadcastMessage(CField.getGameMessage(8, "경험치 피버 타임이 종료되었습니다!"));
                }
            }
        },  60 * 60 * 1000L); // 1시간
    }
