...
public final class MapleMap {
                ...
                if (chr.getSkillCustomValues().size() > 0) {
                    ...
                    for (Map.Entry<Integer, SkillCustomInfo> sci : customInfo.entrySet()) {
                        if (((SkillCustomInfo) sci.getValue()).canCancel(System.currentTimeMillis())) {
                            ...
                            //거탐
                            if (sci.getKey() == 179875) {
                                if (chr.getClient().getLIEDETECT() != null && chr.getClient().getLieDectctCount() > 0) {
            ...
            while (lifes.hasNext()) {
                ...
                if (life.isBuffed(MonsterStatus.MS_Burned)) {
                    ...
                    if (buff != null && life.getHp() > 1L && buff.getInterval() > 0 && buff.getChr() != null) {
                        boolean damage = true;
                        if (buff.getInterval() > 1000 && time - buff.getLastPoisonTime() < buff.getInterval()) {
                            damage = false;
                        }
                        if (damage) {
    ...
    private void dropFromMonster(MapleCharacter chr, MapleMonster mob, boolean instanced) {
        ...
        if (chr.getParty() != null) {
            if (mob.isHellMode() || mob.isExtreme()) {
                ...
                switch (mob.getId()) {
                    ...
                    case 9601511: //마케치
                    case 9400080: { //텐구
                        if (mob.isHellMode()) { // 뭘 어캐해달라는건지 모르겠습니다. 모드는 익스트림이라
                            customs.add(new MonsterDropEntry(2439550, 9999999, quantity, quantity, 0, 0));
                        } else if (mob.isExtreme()) { // 분류가 익스트림이라
        ...
        if (chr.getKeyValue(9919, "DropTear") > 0) {
            int tear = (int) chr.getKeyValue(9919, "DropTear");
            dropBuff += tear == 8 ? 300 : tear == 7 ? 260 : tear == 6 ? 220 : tear == 5 ? 180 : tear == 4 ? 150 : tear == 3 ? 120 : tear == 2 ? 80 : 40;
        }
        ...
        for (MonsterDropEntry de : finals) {
            ...
            if (chr.getMapId() == 261010103) { //피로도
                d1 = d1 / 3;

            }
            
            if (chr.getMapId() == 261020700) { //피로도
                d1 = d1 / 3;
        ...
        if (realfinals != null && !realfinals.isEmpty()) {
            ...
            for (MonsterDropEntry de : realfinals) {
                ...
                if (de.privated == 0 && (mob.getStats().isBoss() || de.itemId == 4001847 || de.itemId == 4001849 || de.itemId == 2434851)) {
                    ...
                    // chr.dropMessageGM(6, "체크2");
                    d = (byte) (d + 1);
                    continue;
                }
                drop(mesoDropped, de, mob, chr, pos, d, realfinals.size());
                // chr.dropMessageGM(6, "체크3");
                d = (byte) (d + 1);
            }
        }
    ...
    public void drop(boolean mesoDropped, MonsterDropEntry de, MapleMonster mob, MapleCharacter chr, Point pos, byte d, int total) {
        ...
        if (de.itemId == 0) {
            ...
            if (GameConstants.isEquip(idrop.getItemId())) {
                Equip equip = (Equip) idrop;
...
//                chr.checkLiveQuest(1, false);
            ...
            if (monster.getId() == 8880010) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 이지 매그너스를 격파하였습니다!"));
            }
            if (monster.getId() == 8880002) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 노말 매그너스를 격파하였습니다!"));
            }
            ...
            if (monster.getId() == 8644655) {
                if (monster.isHellMode()) {
                    System.err.println("monster.isHellMode() " + monster.isHellMode());
            ...
            if (monster.getId() == 8880808) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 칼로스를 격파하였습니다."));
            }

            if (monster.getId() == 8880842) {
                World.Broadcast.broadcastMessage(CField.getGameMessage(8, "[" + chr.getParty().getLeader().getName() + "]님의 파티가 카링을 격파하였습니다."));
            }
        ...
        if (chr.getMap().getId() / 10000 == 92507 && getAllMonster().size() <= 0) {
            ...
            if (floor >= 31 && floor <= 39) {
                ...
                if (chr.getSkillCustomValue0(92507000) >= ((floor == 37 || floor == 38 || floor == 39) ? 3L : 2L)) {
                    ...
                    chr.getClient().getSession().writeAndFlush(CField.environmentChange("Dojang/cleard", 7));
        ...
        if (this.rune == null && !chr.getBuffedValue(80002282) && !isTown() && getAllMonster().size() > 0 && isSpawnPoint() && !GameConstants.isContentsMap(getId()) && !GameConstants.보스맵(getId()) && !GameConstants.사냥컨텐츠맵(getId()) && !GameConstants.튜토리얼(getId()) && !GameConstants.로미오줄리엣(getId()) && !GameConstants.피라미드(getId())) {
            ...
            MapleRune rune = new MapleRune(Randomizer.rand(0, 7), poss.x, poss.y, this);
            while (rune.getRuneType() == 2) { // 거인의 룬 구현 전 까지 임시
                rune.setRuneType(Randomizer.rand(0, 7));
            }
    ...
    public void spawnMonster(final MapleMonster monster, final int spawnType, final boolean overwrite) {
        ...
        if (monster.getSeperateSoul() == 0) {
            ...
            if (monster.getId() == 8880360 || monster.getId() == 8880340 || monster.getId() == 8880300) {
                ...
                /*MapleMonster mob0 = MapleLifeFactory.getMonster(8880321);
                mob0.setHp(90000000);
                spawnMonsterOnGroundBelow(mob0, new Point(352, 0));

                MapleMonster mob1 = MapleLifeFactory.getMonster(8880322);
                mob1.setHp(90000000);
                spawnMonsterOnGroundBelow(mob1, new Point(352, -2020));

                MapleMonster mob2 = MapleLifeFactory.getMonster(8880325);
                mob2.setHp(90000000);
                spawnMonsterOnGroundBelow(mob2, new Point(352, 0));

                MapleMonster mob3 = MapleLifeFactory.getMonster(8880326);
                mob3.setHp(90000000);
                spawnMonsterOnGroundBelow(mob3, new Point(352, -2020));*/
            }
            if (monster.getId() == 8880301 || monster.getId() == 8880341) {
                ...
                //윌 2페 수정
                MapleMonster mob0 = MapleLifeFactory.getMonster(8880323);
                mob0.setHp(90000000);
                spawnMonsterOnGroundBelow(mob0, new Point(352, 215));

                MapleMonster mob1 = MapleLifeFactory.getMonster(8880327);
                mob1.setHp(90000000);
                spawnMonsterOnGroundBelow(mob1, new Point(352, 215));
            ...
            if (monster.getId() == 8880000 || monster.getId() == 8880002 || monster.getId() == 8880010) {
            ...
            } else if (monster.getId() == 8880700) { // 슬라임
                Timer.MapTimer.getInstance().schedule(() -> MapleBossManager.SlimeHandler(monster), 2000L);
            ...
            } else if (monster.getId() == 8880142) {

                MapleMonster mob2 = MapleLifeFactory.getMonster(8880158);
                mob2.setHp(32374800);
                spawnMonsterOnGroundBelow(mob2, new Point(1021, 43));
                ...
                //루시드 1페 수정
                if (monster.getId() == 8880141) {
                    for (int i = 0; i < 1; i++) {
                        MapleMonster mob2 = MapleLifeFactory.getMonster(8880166);
                        mob2.setHp(32374800);
                        spawnMonsterOnGroundBelow(mob2, new Point(1021, 43));

                    }
                } else if (monster.getId() == 8880140) {
                    for (int i = 0; i < 1; i++) {
                        MapleMonster mob2 = MapleLifeFactory.getMonster(8880166);
                        mob2.setHp(32374800);
                        spawnMonsterOnGroundBelow(mob2, new Point(1021, 43));
                ...
                //윌 3페 수정
                MapleMonster mob0 = MapleLifeFactory.getMonster(8880324);
                mob0.setHp(90000000);
                spawnMonsterOnGroundBelow(mob0, new Point(352, 281));

                MapleMonster mob1 = MapleLifeFactory.getMonster(8880328);
                mob1.setHp(90000000);
                spawnMonsterOnGroundBelow(mob1, new Point(352, 281));
            ...
            if (monster.getId() == 8880400 || monster.getId() == 8880405 || monster.getId() == 8880415) {
                ...
                //진힐라 패턴 수정
                MapleMonster mob0 = MapleLifeFactory.getMonster(8880406);
                mob0.setHp(90000000);
                spawnMonsterOnGroundBelow(mob0, new Point(0, 266));

                MapleMonster mob1 = MapleLifeFactory.getMonster(8880407);
                mob1.setHp(90000000);
                spawnMonsterOnGroundBelow(mob1, new Point(0, 266));
    ...
    public final void spawnSummon(final MapleSummon summon, final int duration) {
        ...
        if ((duration > 0 && duration < Integer.MAX_VALUE && summon.getSkill() != 400051046 && summon.getSummonType() != 7) || (summon.getSkill() == 400021068 && summon.getMovementType() != SummonMovementType.SUMMON_JAGUAR)) {
            Timer.MapTimer.getInstance().schedule(new Runnable() {
                public void run() {
                    if (summon.getSkill() == 500001002 || summon.getSkill() == 500001003 || summon.getSkill() == 500001004) { // HEXA 솔 야누스 구현
                        MapleCharacter owner = summon.getOwner();

                        if (owner != null) {
                            for (MapleSummon sum : owner.getSummons(summon.getSkill())) {
                                sum.removeSummon(map, false, false);
                            }
                        }
                    } else {
    ...
    public final void spawnMesoDrop(int meso, Point position, final MapleMapObject dropper, final MapleCharacter owner, boolean playerDrop, byte droptype, final int delay) {
        ...
        if (delay > 0 && owner.getBuffedEffect(SecondaryStat.PickPocket) != null) {
            int skillId = owner.getSkillLevel(4241006) > 0 ? 4241006 : 4211006;
            int max = SkillFactory.getSkill(skillId).getEffect(owner.getSkillLevel(skillId)).getBulletCount();
...
//        }*/
        ...
        if (chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -27) != null && chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -28) != null) {
        ...
        /*int damageSkinCode = (int) chr.getKeyValue(7293, "damage_skin"); // 데미지 스킨 적용
        if (damageSkinCode != -1) {
            chr.getMap().broadcastMessage(CField.showForeignDamageSkin(chr, damageSkinCode));
        }*/
        ...
        switch (this.mapid) {
            ...
            //거탐
            case 993073000:
                if (chr.getClient().getKeyValue("LIE_DETECTOR_WARP") != null) {
                    if (chr.getMapId() != 993073000) {
                        chr.warp(993073000);
                    }

                    chr.getClient().send(CField.getTrueRoomClock(Integer.parseInt(chr.getClient().getKeyValue("LIE_DETECTOR_WARP")) * 1000));
                }

                if (chr.getMapId() == 993073000) {
                    if (chr.getClient().getKeyValue("LIE_DETECTOR_WARP") == null) {
                        chr.warp(ServerConstants.warpMap);
                    } else if (Integer.parseInt(chr.getClient().getKeyValue("LIE_DETECTOR_WARP")) <= 1) {
                        chr.warp(ServerConstants.warpMap);
                    }
                }
                break;
            ...
            default:
                final int[] array;
                skilllist = array = new int[]{80001427, 80001428, 80001432, 80001752, 80001753, 80001755, 80001878, 80002888, 80002889, 80002890};
    ...
    public final void resetFullyAnimate(boolean animate) {
        resetFully(animate);
    }
