        ...
        this.QuizTimer = Timer.EventTimer.getInstance().register(new Runnable() {
            ...
            public void run() {
                ...
                if (this.count == 0) {
                    ...
                    for (MapleCharacter chr : OXQuizGame.this.chars) {
                        chr.getClient().getSession().writeAndFlush(SLFCGPacket.OXQuizQuestion(this.tempq.getQuestion(), OXQuizGame.this.quizes.indexOf(this.tempq), OXQuizGame.this.quizes.size() - OXQuizGame.this.quizes.indexOf(this.tempq) + 1));
