...
public class InnerAbillity {
    ...
    public InnerSkillValueHolder renewSkill(int rank, boolean circulator, int preset) {
        ...
        return new InnerSkillValueHolder(randomSkill, (byte) skillLevel, (byte) SkillFactory.getSkill(randomSkill).getMaxLevel(), randomSkillTriple.getLeft(), (byte) preset);
    }
    ...
    public InnerSkillValueHolder renewLevel(int rank, int skill, int preset) {
        ...
        for (Triple<Byte, Integer, int[]> data : list) {
            if (data.getMid() == skill) {
                return new InnerSkillValueHolder(skill, (byte) data.getRight()[Randomizer.nextInt(data.getRight().length)], (byte) SkillFactory.getSkill(skill).getMaxLevel(), data.getLeft(), (byte) preset);
            }
        }
        return null;
    }
