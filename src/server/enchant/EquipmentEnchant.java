...
public class EquipmentEnchant {
    ...
    public static int scrollType(String name) {
        ...
        if (name.contains("이노센트") || name.contains("아크")) {
            return 4;
        }
        if (name.contains("순백")) {
            return 5;
    ...
    public static boolean isWeapon(MapleWeaponType type) {
        switch (type) {
            case SWORD2H:
            case SWORD1H:
            case BOW:
        ...
        switch (type) {
            ...
            case 1:
                ...
                if (Randomizer.nextInt(1000) < i) {
                    ...
                    for (Pair<EnchantFlag, Integer> stat : starForceStats.getStats()) {
                        if (EnchantFlag.Watk.check(((EnchantFlag) stat.left).getValue())) {
                            item.setStarforceWatk((short) (item.getStarforceWatk() + ((Integer) stat.right).intValue()));
                            if (item2 != null) {
                                item2.setStarforceWatk((short) (item2.getStarforceWatk() + ((Integer) stat.right).intValue()));
                            }
                        }
                        if (EnchantFlag.Matk.check(((EnchantFlag) stat.left).getValue())) {
                            item.setStarforceMatk((short) (item.getStarforceMatk() + ((Integer) stat.right).intValue()));
                            if (item2 != null) {
                                item2.setStarforceMatk((short) (item2.getStarforceMatk() + ((Integer) stat.right).intValue()));
                            }
                        }
                        if (EnchantFlag.Str.check(((EnchantFlag) stat.left).getValue())) {
                            item.setStarforceStr((short) (item.getStarforceStr() + ((Integer) stat.right).intValue()));
                            if (item2 != null) {
                                item2.setStarforceStr((short) (item2.getStarforceStr() + ((Integer) stat.right).intValue()));
                            }
                        }
                        if (EnchantFlag.Dex.check(((EnchantFlag) stat.left).getValue())) {
                            item.setStarforceDex((short) (item.getStarforceDex() + ((Integer) stat.right).intValue()));
                            if (item2 != null) {
                                item2.setStarforceDex((short) (item2.getStarforceDex() + ((Integer) stat.right).intValue()));
                            }
                        }
                        if (EnchantFlag.Int.check(((EnchantFlag) stat.left).getValue())) {
                            item.setStarforceInt((short) (item.getStarforceInt() + ((Integer) stat.right).intValue()));
                            if (item2 != null) {
                                item2.setStarforceInt((short) (item2.getStarforceInt() + ((Integer) stat.right).intValue()));
                            }
                        }
                        if (EnchantFlag.Luk.check(((EnchantFlag) stat.left).getValue())) {
                            item.setStarforceLuk((short) (item.getStarforceLuk() + ((Integer) stat.right).intValue()));
                            if (item2 != null) {
                                item2.setStarforceLuk((short) (item2.getStarforceLuk() + ((Integer) stat.right).intValue()));
                            }
                        }
                    }
                    if ((item.getEnchantBuff() & 0x20) != 0) {
            ...
            case 52: // 스타포스
                ...
                if (item.getEnhance() >= 17) { // 17성 이상 파방 적용되는거 감지
                    shield = false;
                }
                ...
                if (item.getEnhance() < 15) { // 1.2.372 Changed
                    meso *= 3; // 메소 3배
                    down = 0; // 하락 안됨
                    destroy = 0; // 파괴 방지
                }
