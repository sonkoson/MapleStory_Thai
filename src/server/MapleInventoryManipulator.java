...
import maplecontents.Custom.ContentsByItem;
...
public class MapleInventoryManipulator {
    ...
    public static void equip(final MapleClient c, final short src, short dst, final MapleInventoryType type) {
        ...
        //  추가 경험치 지워버림댐?
        for (int itemId : ContentsByItem.ExpBuffMedal.keySet()) {
            if (source.getItemId() == itemId) {
                int incPercent = ContentsByItem.getEquipItemByExpIncreasePercent(c.getPlayer(), itemId);
                c.getPlayer().dropMessage(5, "[Fancy] 훈장 착용으로 인하여 경험치 획득량이 " + incPercent + "% 만큼 증가합니다.");
                c.getPlayer().addKV("incExpByMedal", String.valueOf(incPercent));
            }
        }
        //펜던트 추가 경험치
        for (int itemId : ContentsByItem.ExpBuffPendent.keySet()) {
            if (source.getItemId() == itemId) {
                int incPercent = ContentsByItem.getEquipItemByExpIncreasePercent_Pendent(c.getPlayer(), itemId);
                c.getPlayer().dropMessage(5, "[Fancy] 펜던트 착용으로 인하여 경험치 획득량이 " + incPercent + "% 만큼 증가합니다.");
                c.getPlayer().addKV("incExpByPendent", String.valueOf(incPercent));
            }
        }
        ...
        if (chr.getSkillLevel(80002632) > 0) {
            ...
            if (weapon_item != null) {
                ...
                if (weapon_name != null && weapon_name.startsWith("제네시스 ") && !chr.getBuffedValue(80002632)) {
