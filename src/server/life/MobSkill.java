        ...
        try {
            ...
            switch (this.skillId) {
                ...
                case 170:
                    switch (this.skillLevel) {
                        ...
                        case 42:
                            ...
                            int xs = (int) monster.getTruePosition().getX();
                            boolean facingleft = monster.isFacingLeft();
                            for (int is = 0; is < 3; ++is) {
                                mda.add(new MapleDelayedAttack(monster, new Point(xs + (is * 250 * (facingleft ? -1 : 1)), -113), Randomizer.rand(80, 130), facingleft));
                            }
                    ...
                    switch (this.skillLevel) {
                        ...
                        case 4:
                            ...
                            if (chr3 != null) {
                                ...
                                Timer.MobTimer.getInstance().schedule(() -> {
                                            chr3.getClient().send(CField.onUserTeleport(chr3 ,Randomizer.rand(-880, 1030), 100));
                    ...
                    switch (this.skillLevel) {
                        ...
                        case 4: { // 1페이즈 : 주시하는 눈
                            ...
                            for (MapleCharacter chr : monster.getMap().getAllCharactersThreadsafe()) {
                                chr.getClient().getSession().writeAndFlush(MobPacket.BossWill.WillSpiderAttack(monster.getId(), skillId, skillLevel, Randomizer.rand(1, 2), null));
                            }
                            ...
                            server.Timer.MobTimer.getInstance().schedule(() -> {
                                ...
                                for (MapleCharacter chr : monster.getMap().getAllCharactersThreadsafe()) {
                                    boolean add = true;
                                    chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showWillEffect(chr, 1, skillId, skillLevel));
                                    chr.getClient().getSession().writeAndFlush(MobPacket.BossWill.WillSpiderAttack(monster.getId(), skillId, skillLevel, 0, null));
                                ...
                                for (MapleCharacter chr : targets) {
                                    if (chr.getBuffedEffect(SecondaryStat.HolyMagicShell) != null) {
                                        if (chr.getHolyMagicShell() > 0) {
                                            ...
                                            Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
                                            statups.put(SecondaryStat.HolyMagicShell, new Pair<>((int) chr.getHolyMagicShell(), (int) chr.getBuffLimit(chr.getBuffSource(SecondaryStat.HolyMagicShell))));
                                        ...
                                        int windWall = (int) Math.max(0, chr.getBuffedValue(SecondaryStat.WindWall) - (100 * chr.getBuffedEffect(SecondaryStat.WindWall).getZ()));
                                        if (windWall > 0) {
                                            chr.setBuffedValue(SecondaryStat.WindWall, windWall);
                                            Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
                                            statups.put(SecondaryStat.WindWall, new Pair<>(windWall, (int) chr.getBuffLimit(400031030)));
                        ...
                        case 5: {//mush
                            ...
                            Timer.MobTimer.getInstance().schedule(() -> {
                                ...
                                for (MapleCharacter chr : targets) {
                                    ...
                                    if (chr.getBuffedEffect(SecondaryStat.TrueSniping) != null || chr.getBuffedEffect(SecondaryStat.Etherealform) != null || chr.getBuffedEffect(SecondaryStat.IndieNotDamaged) != null || chr.getBuffedEffect(SecondaryStat.NotDamaged) != null) {
                                        continue;
                                    }
                                    int reduce = 0;
                                    if (chr.getBuffedEffect(SecondaryStat.IndieDamageReduce) != null) {
                                ...
                                Timer.MobTimer.getInstance().schedule(() -> {
                                    if (monster.getHPPercentDouble() <= 66.6 && monster.getWillHplist().contains(666)) {
                                        MobSkillFactory.getMobSkill(242, 4).applyEffect(player, monster, skill, isFacingLeft);
                                    } else if (monster.getHPPercentDouble() <= 33.3 && monster.getWillHplist().contains(333)) {
                                        MobSkillFactory.getMobSkill(242, 4).applyEffect(player, monster, skill, isFacingLeft);
                                    } else if (monster.getHPPercentDouble() <= 0.3 && monster.getWillHplist().contains(3)) {
                                        MobSkillFactory.getMobSkill(242, 4).applyEffect(player, monster, skill, isFacingLeft);
                                    }
                                }, 1000L);
                            }, 3000L);
                        ...
                        case 8: {
                            ...
                            for (MapleMist mist4 : monster.getMap().getAllMistsThreadsafe()) {
                                if (mist4.getMobSkill().getSkillId() != 242) {
                                    continue;
                                }
                ...
                case 265:
                    ...
                    break;
                case 266: //슬라임 소환
                    for (int slime = 0; slime < 3; slime++) {
                        monster.getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(Randomizer.rand(8880715, 8880719)), monster.getPosition());
                    }
                    break;
            }
