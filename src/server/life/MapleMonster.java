...
import handling.world.guild.MapleGuildSulo;

import maplecontents.Custom.TemaDungeonTimeLimitHuntingMap;
...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public final long getMobMaxHp() {
        return (long) (this.stats.getHp() * bonusHp());
    }
    ...
    public final void damage(MapleCharacter from, long damage, boolean updateAttackTime, int lastSkill) {
        ...
        //[othello] 익스트림 보스
        if (getId() == 8880505) { //8880505 로 1페 클리어 여부
            if (this.getMap().getId() / 1000 == 450017 && (this.getMap().getMobsSize(8880520) > 0 || this.getMap().getMobsSize(8880521) > 0)) { //창조와 파괴의 기사 살아 있을 시 8880505 에 데미지 안들어감.
                return;
            }
        }
        ...
        if (getId() == 8880153) {
            ...
            double hppercent = nowhp * 100.0D / getStats().getHp() / bonusHp();
        ...
        if (this.getId() == 9833377) {
            setHp(Long.MAX_VALUE);
        }

        // [Merry] 이벤트 몬스터
        if (this.getId() == TemaDungeonTimeLimitHuntingMap.GoldReachStorage.getEventMobId()) {
            long expAmount = TemaDungeonTimeLimitHuntingMap.GoldReachStorage.getGainExpAmountByMonster();
            getStats().setExp(expAmount);
        }
        ...
        if (getId() == 8880305) {
            ...
            if (Randomizer.isSuccess(30)) {
                ...
                //System.out.println("주시하는 눈 Obstacle");
                this.map.CreateObstacle(this, obs);
            }
            return;
        }

        MapleGuildSulo sulo = from.getClient().getGuildSulo();
        if (getId() == 9500805) {
            ...
            if (totaldamage >= getStats().getHp() / 1000L * 10L) {
                int addpoint = 0;
                while (totaldamage > 0L) {
                    totaldamage -= getStats().getHp() * 10L / 1000L;
                    addpoint++;
                }
                from.removeSkillCustomInfo(getId());
                int plus = (int) getCustomValue0(getId()) / 1000;
                if (plus <= 0) {
                    plus = 1;
                }
                int totaladdpoint = plus * addpoint;
                long po = sulo.getSuloPoint(); //from.getPlayer().getKeyValue(221121, "point");
                int po1 = Long.valueOf(po).intValue();
                if (totaladdpoint > getCustomValue0(getId())) {
                    totaladdpoint = (int) getCustomValue0(getId());
                }
                addSkillCustomInfo(getId(), -totaladdpoint);
                sulo.setSuloPoint(sulo.getSuloPoint() + Randomizer.rand(25, 58));

                //from.getPlayer().setKeyValue(221121, "point", "" + (from.getPlayer().getKeyValue(221121, "point") + totaladdpoint) + "");
                from.getClient().send(CField.PunchKingPacket(from, 3, new int[]{getObjectId(), Randomizer.rand(25, 58)}));
                from.getClient().send(CField.PunchKingPacket(from, 2, new int[]{po1}));
            }
        }
        if (from.getMapId() == 941000200 && (getId() >= 9500800 && getId() < 9500805)) {
            boolean nextStage = (from.getMap().getAllMonster().size() == 1 && getHp() - damage <= 0L);
            int stage = (int) from.getSkillCustomValue0(941000201);
            long po = sulo.getSuloPoint();
            int po1 = Long.valueOf(po).intValue();
            int CV = 0;
            if (getHp() - damage <= 0L) {
                if (getCustomValue0(getId()) > 0L) {
                    CV = Long.valueOf(Optional.ofNullable(getCustomValue0(getId())).orElse(0L)).intValue();
                    sulo.setSuloPoint(sulo.getSuloPoint() + CV);
                    //from.getPlayer().setKeyValue(221121, "point", "" + (from.getPlayer().getKeyValue(221121, "point") + getCustomValue0(getId())) + "");
                ...
                if (totaldamage >= getStats().getHp() / 100L * 10L) {
                    ...
                    addSkillCustomInfo(getId(), -Randomizer.rand(25, 58));
                    sulo.setSuloPoint(sulo.getSuloPoint() + Randomizer.rand(25, 58));
                    //from.getPlayer().setKeyValue(221121, "point", "" + (from.getPlayer().getKeyValue(221121, "point") + totaladdpoint) + "");
                    from.getClient().send(CField.PunchKingPacket(from, 3, new int[]{getObjectId(), Randomizer.rand(25, 58)}));
                    from.getClient().send(CField.PunchKingPacket(from, 2, new int[]{po1}));
            ...
            if (nextStage) {
                if (stage + 1 > 6) {
                    ...
                    Timer.EventTimer.getInstance().schedule(() -> {
                        if (from.getMapId() == 941000200) {
                            from.warp(941000001);
                        }
                    }, 3000L);
                } else {
                    MapleMonster boss = MapleLifeFactory.getMonster(9500800 + stage);
                    boss.setOwner(from.getId());
                    getMap().spawnMonsterOnGroundBelow(boss, new Point(148, 95));
                    stage++;
                    from.setSkillCustomInfo(941000201, stage, 0L);
        ...
        if (getId() == 8881000) {
            System.err.println("2312312312");
            from.DamageMeter += damage;
            from.dropMessage(-1, "누적 데미지 : " + from.DamageMeter);
            from.getClient().getSession().writeAndFlush(CField.getGameMessage(8, "누적 데미지 : " + from.DamageMeter));
        }

        if (getMobMaxHp() > Integer.MAX_VALUE) {
            if (getId() == 9833376 || getId() == 8881000) {
                from.DamageMeter += damage;
                from.dropMessage(-1, "누적 데미지 : " + from.DamageMeter);
                from.getClient().getSession().writeAndFlush(CField.getGameMessage(8, "누적 데미지 : " + from.DamageMeter));
            }
        }
        if (from.getMapId() == 970072000) {
        ...
        if (this.stats.getSelfD() != -1) {
            ...
            if (this.sponge.get() != null && ((MapleMonster) this.sponge.get()).hp > 0L) {
                ...
                if (((MapleMonster) this.sponge.get()).hp <= 0L) {
                    ...
                    this.map.broadcastMessage(MobPacket.showBossHP(((MapleMonster) this.sponge.get()).getId(), -1L, ((MapleMonster) this.sponge.get()).getMobMaxHp() * getMobMaxHp()));
                    if (send) {
            ...
            if (this.hp > 0L) {
                ...
                if (getStats().isMobZone()) {
                    if (getId() == 8880101 || getId() == 8880111) {
                        ...
                        //[othello] 익스트림 보스
                    } else if (getId() != 8644650 && getId() != 8644655 && getId() != 8880503 && getId() != 8880523) {
                        byte phase;
                        if (getHPPercent() > 75) {
                            phase = 1;
    ...
    public int getHPPercent() {
        ...
        return (int) Math.ceil(this.hp * 100.0D / getStats().getHp() / bonusHp());
    }

    public double getHPPercentDouble() {
        return this.hp * 100.0D / getMobMaxHp() / bonusHp();
    }
    ...
    private final void giveExpToCharacter(MapleCharacter attacker, long exp, boolean highestDamage, int numExpSharers, byte pty, byte Class_Bonus_EXP_PERCENT, byte Premium_Bonus_EXP_PERCENT, int lastskillID) {
        ...
        if (exp > 0L) { // 레벨별 배율
            ...
            if (!attacker.getBuffedValue(80002282) && attacker.getMap().getRuneCurse() > 0 && !GameConstants.보스맵(getMap().getId()) && !GameConstants.isContentsMap(getMap().getId())) {
                ...
                long curse = exp * attacker.getMap().getRuneCurseDecrease() / 100L;
                exp -= curse;
                // attacker.dropMessage(6, "룬의 저주로 인해 경험치 " + exp + " 획득중" );
            }
            /*if (attacker.getLevel() >= 200) {
                int level = 20;
        ...
        if (killer != null && killer.getPosition() != null && (getId() < 8644101 || getId() > 8644112) && getStats().getLevel() >= killer.getLevel() - 20 && getStats().getLevel() <= killer.getLevel() + 20) {
            ...
            if (killer.getBuffedEffect(SecondaryStat.EventSpecialSkill, 80003016) != null) {
                if (killer.getEventKillingMode()) {
                    ...
                    if ((Randomizer.isSuccess(5) || v) && !getStats().isBoss()) {
    ...
    public void applyStatus(MapleClient c, MonsterStatus status, MonsterStatusEffect effect, int value, SecondaryStatEffect eff) {
        ...
        if (c == null || effect == null || eff == null) {
            if (c == null) {
                System.out.println("[MapleMonster-ApplyStatus] c is null");
                return;
            }
            if (effect == null) {
                System.out.println("[MapleMonster-ApplyStatus] effect is null");
                return;

            }
            if (eff == null) {
                System.out.println("[MapleMonster-ApplyStatus] eff is null");
                return;
            }
            return;
        }
    ...
    public void AddSerenTimeHandler(int type, int add) {
        ...
        int[] serens = {8880603, 8880607, 8880609, 8880612, 8880633, 8880637, 8880639, 8880642}; //[othello] - 익스트림 보스  세렌 페에즈 넘어가기
        for (int ids : serens) {
        ...
        if (nowtime <= 0) {
            if (seren != null) {
                ...
                Timer.MapTimer.getInstance().schedule(() -> {
                    ...
                    //[othello] 익스트림 보스
                    if (nextid == 8880603 || nextid == 8880633) {
                        getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880605), pos);
                        MapleMonster totalseren = getMap().getMonsterById(extreme ? 8880632 : 8880602);
                        if (totalseren != null) {
                            totalseren.gainShield((long) (totalseren.getStats().getHp() * bonusHp() * 15L / 100L), !(totalseren.getShield() > 0L), 0);
                        }
                    }
                    getMap().broadcastMessage(MobPacket.BossSeren.SerenTimer(2, new int[]{0}));
                    setCustomInfo(extreme ? 8880633 : 8880603, 0, 0);
