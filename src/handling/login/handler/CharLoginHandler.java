...
public class CharLoginHandler {
    ...
    public static boolean KoreaLogin(String id) {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM KoreaLogin WHERE time > " + (UnixTime() - 300) + " AND id = ?");
            ps.setString(1, id);
            rs = ps.executeQuery();
            if (rs.next()) {
                return true;
    ...
    public static final void login(final LittleEndianAccessor slea, final MapleClient c) {
      ...
      /*  if (!KoreaLogin(login) && !c.isGm()) {
            c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "전용 접속기로만 접속 가능 합니다."));
            c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
            return;
        }*/
    ...
    public static void SessionDCEvent(LittleEndianAccessor OutPacket, MapleClient client) {
        int DisconnectType = OutPacket.readInt();
    ...
    public static final void checkLoadWzData(LittleEndianAccessor slea, final MapleClient c) {
        int type = slea.readInt();
        /*if (type == 1) {
            LoginPacket.sendCRCCheckFileList(c);
        } else if (type == 24) {
    ...
    public static void getLoginRequest(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (webStart == 1) {
            ...
            String login = "";
            String pwd = "";
            if (ServerConstants.cons.get(sp[0]) != null) {
                login = ServerConstants.cons.get(sp[0]).left;
                pwd = ServerConstants.cons.get(sp[0]).mid;
            }
    ...
    public static final void CharlistRequest(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (chars != null && ChannelServer.getInstance(channel) != null) {
            ...
            c.getSession().writeAndFlush(LoginPacket.Activate());
        } else {
            c.getSession().close();
        }
    }
    ...
    public static final void SelectChannelList(MapleClient c, int world) {
        if (ServerConstants.ConnectorSetting && !c.isGm() && !c.getSessionIPAddress().equals("218.156.239.101") && !c.getSessionIPAddress().equals("121.178.253.57") && !c.getSessionIPAddress().equals("175.199.200.108") && !c.getSessionIPAddress().equals("110.11.52.152") && !c.isGm() && !c.getSessionIPAddress().equals("121.186.52.50") && !c.getSessionIPAddress().equals("218.149.25.97") && !c.getSessionIPAddress().equals("220.71.217.94") && !c.getSessionIPAddress().equals("185.213.243.67")) {
            if (c.getAllowed() != 1) {
         ...
         /*   if (!KoreaLogin(c.getAccountName()) && !c.isGm()) {
                c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "전용 접속기로만 접속 가능 합니다."));
                c.getSession().writeAndFlush(LoginPacket.getSelectedChannelFailed((byte) 21, world));
                return;
            }*/
    ...
    public static final void CreateChar(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (job == LoginInformationProvider.JobType.AngelicBuster) {
            newchar.setSecondFace(56260);
            newchar.setSecondHair(68181);
        ...
        if (MapleCharacterUtil.canCreateChar(name, c.isGm()) && (!LoginInformationProvider.getInstance().isForbiddenName(name) || c.isGm()) && (c.isGm() || c.canMakeCharacter(c.getWorld()))) {
            ...
            Byte[] key = {20, 21, 26, 33, 41, 43, 46, 51, 59, 60, 61, 62, 63, 64, 65, 66, 67, 68, 87};
            Byte[] type = {0, 0, 0, 0, 0, 0, 0, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4};
            Integer[] action = {0, 0, 0, 0, 0, 0, 0, 45, 39, 208, 209, 27, 22, 25, 49, 42, 35, 14, 9};
            for (int i = 0; i < key.length; i++) {
                newchar.changeKeybinding(key[i], type[i], action[i]);
            }

            Integer[] quick = {
                42, 82, 71, 73, 29, 83, 79, 81, 2, 3,
                4, 5, 16, 17, 18, 19, 6, 7, 8, 9,
                20, 30, 31, 32, 10, 11, 33, 34, 37, 38,
                49, 50};
            for (int i = 0; i < quick.length; i++) {
                newchar.setKeyValue(333333, "quick" + i, String.valueOf(quick[i]));
            }
        } else {
    ...
    public static final void checkSecondPassword(LittleEndianAccessor rh, MapleClient c) {
        ...
        if (!c.CheckSecondPassword(code) && code.length() != 0) {
            c.getSession().writeAndFlush(LoginPacket.secondPwError((byte) 20));
        } else {
    ...
    public static final void OTPSetting(LittleEndianAccessor slea, MapleClient c) {
        if (c.getSecondPw() == 0) {
            c.setSecondPw(1);
            c.send(LoginPacket.OTPChange(1));
            c.send(CWvsContext.serverNotice(1, "", "2차 비밀번호 입력 기능이 비활성화 되었습니다."));
            //System.err.println(c.getSecondPw());
        } else {
            c.setSecondPw(0);
            c.send(LoginPacket.OTPChange(0));
            c.send(CWvsContext.serverNotice(1, "", "2차 비밀번호 입력 기능이 활성화 되었습니다."));
        }
    }
