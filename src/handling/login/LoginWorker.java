...
public class LoginWorker {
    ...
    public static void registerClient(final MapleClient c, final String id, String pwd) {
        ...
        /*// CONNECTOR 소스 [S]
        if (c.getConnectorClient() == null) {
            c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "전용 접속기로만 접속이 가능합니다."));
            c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
            return;
        }

        if (c.getConnectorClient().isConnectorHeartBeatStop()) {
            c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "전용 접속기가 응답하지 않습니다."));
            c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
            return;
        }
        // CONNECTOR 소스 [E]
        ...
        // CRC 소스 [S]
        if (c.getCRCClient().isCRCHeartBeatStop()) {
            c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "클라이언트가 응답하지 않습니다."));
            c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
            return;
        }

        String systemSerialNumber = c.getCRCClient().getSystemSerialNumber();
        /*
        if (systemSerialNumber.length() < 128) {
            c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "시스템 정보 변조로 인해 로그인에 실패 하였습니다."));
            c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
            return;
        }

        if (CRCServerHandler.checkSystemBan(systemSerialNumber)) {
            c.getSession().writeAndFlush(CWvsContext.serverNotice(1, "", "운영 정책 위반 및 불법 프로그램 이용 등으로 인해 게임 이용이 제한된 상태입니다."));
            c.getSession().writeAndFlush(LoginPacket.getLoginFailed(21));
            return;
        }

        c.getCRCClient().saveSystemSerialNumbers(systemSerialNumber);
        // CRC 소스 [E]*/
