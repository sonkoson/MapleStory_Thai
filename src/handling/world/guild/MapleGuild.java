...
public class MapleGuild implements Comparable<MapleGuild> {
            ...
            if (this.allianceid > 0) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    if (this.id != guildId) {
                        ...
                        if (guild != null) {
                            guild.broadcast(CWvsContext.AlliancePacket.allianceMemberOnline(mgc, this.id, mgc.getId(), online));
                        }
                    }
                }
            }
    ...
    public static final MapleGuildResponse sendInvite(MapleClient c, String targetName) {
        ...
        mc.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.guildInvite(c.getPlayer().getGuildId(), targetName, c.getPlayer()));
        return null;
    }
