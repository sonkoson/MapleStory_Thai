...
public class GuildHandler {
    ...
    public static final void GuildCancelRequest(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        if (slea.available() > 4) { // 가입 요청 거절
            ...
            if (player != null) {
                player.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.RequestDenyMaster(chr.getId(), guild));
            }
            ...
            //guild.broadcast(CWvsContext.GuildPacket.RequestDenyMaster(c.getPlayer().getId(), guild));
            //c.getSession().writeAndFlush(CWvsContext.GuildPacket.RequestListGuild(g));
            //c.getSession().writeAndFlush(CWvsContext.GuildPacket.RecruitmentGuild(c.getPlayer()));
        }
    }
                ...
                if (chr.getGuildRank() <= 2 && g != null) {
                    ...
                    for (ChannelServer cs : ChannelServer.getAllInstances()) {
                        ...
                        if (c != null) {
                            ...
                            c.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.showGuildRanks((byte) 5, c.getClient(), null));
                            ...
                            /*c.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.newGuildInfo2(chr));
                            c.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.newGuildHash());
                            ...
                            c.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.guildLoadAattendance());
                            c.getClient().getSession().writeAndFlush(CWvsContext.GuildPacket.showGuildInfo(c));*/
        ...
        try {
            ...
            switch (action) {
                ...
                case 2: // 길드 생성 1.2.391 OK.
                    ...
                    c.getSession().writeAndFlush(CWvsContext.GuildPacket.updateGP(c.getPlayer().getGuild().getId(), c.getPlayer().getGuild().getGP(), c.getPlayer().getGuild().getLevel())); // 1.2.390 ++
                    c.getSession().writeAndFlush(CWvsContext.GuildPacket.GainGP(c.getPlayer().getGuild(), c.getPlayer(), 500)); // 1.2.390 ++
                ...
                case 31: // 스킬 초기화 1.2.391 OK.
                    ...
                    c.getPlayer().dropMessage(1, "길드 스킬 초기화가 완료 되었습니다.");
                    break;
                ...
                case 50: // 길드 연합 탈퇴 1.2.391 OK.
                    ...
                    if (alliance.getLeaderId() == c.getPlayer().getId() && alliance.getNoGuilds() > 1) {
                        c.getPlayer().dropMessage(1, "연합 마스터는 연합의 모든 길드가 탈퇴하기 전까지 탈퇴할 수 없습니다.");
                        return;
                    }
                    
                    alliance.removeGuild(c.getPlayer().getGuild().getId(), true);
                    c.getPlayer().getGuild().setAllianceId(0);
                    c.getPlayer().getGuild().broadcast(CWvsContext.GuildPacket.exitGuildAlliance(alliance, c.getPlayer().getGuild()));
                    c.getSession().writeAndFlush(CWvsContext.GuildPacket.showGuildInfo(c.getPlayer()));
                    break;
                case 53: // 스킬 쿨타임 초기화
                    if (c.getPlayer().getGuildId() <= 0) {
