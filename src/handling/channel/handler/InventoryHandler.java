        ...
        for (int i = 1; i <= pInv.getSlotLimit(); ++i) {
            ...
            if (item2 == null) {
                ...
                if (nextItem != null) {
                    ...
                    c.getSession().writeAndFlush(CWvsContext.InventoryPacket.updateItemSort(pInvType, pInv.getNextFreeSlot()));
                }
            }
        }
        c.getSession().writeAndFlush((Object) CWvsContext.InventoryPacket.moveInventoryItem(pInvType, updateSlots));
    ...
    public static boolean UseUpgradeScroll(final RecvPacketOpcode header, final short slot, final short dst, final byte ws, final MapleClient c, final MapleCharacter chr) {
        ...
        if (scroll.getItemId() / 100 == 20496) {
            if (toScroll.getOwner().contains("강")) {
                ...
                c.getPlayer().dropMessage(5, "사냥&홍보&후원 강화 된 물품에는 이노센트 류 사용이 불가능합니다.");
                return false;
            }
            if (toScroll2 != null) {
                if (toScroll2.getOwner().contains("강")) {
                    c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
                    c.getPlayer().dropMessage(5, "사냥&홍보&후원 강화 된 물품에는 이노센트 류 사용이 불가능합니다.");
                    return false;
                }
        ...
        if (scrollSuccess == Equip.ScrollResult.CURSE) {
            ...
            c.getSession().writeAndFlush((Object) CWvsContext.InventoryPacket.updateScrollandItem(scroll, toScroll, false)); // 여기
            if (toScroll2 != null) {
        ...
        switch (itemId) {
            ...
            case 5062801:
                ...
                for (InnerSkillValueHolder isvh : c.getPlayer().getInnerSkills().get(preset)) {
                    ...
                    if (ivholder2 == null) {
                        ivholder2 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), true, preset);
                    ...
                    InnerSkillValueHolder ivholder3 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), true, preset);
                    while (isvh.getSkillId() == ivholder3.getSkillId() || ivholder.getSkillId() == ivholder3.getSkillId() || ivholder2.getSkillId() == ivholder3.getSkillId()) {
                        ivholder3 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), true, preset);
                    }
            ...
            case 5062503:
                ...
                if (item8 != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (neq.getPotential4() >= 0) {
                        ...
                        c.getSession().writeAndFlush(CField.getWhiteCubeStart(c.getPlayer(), (Item) neq, bool4, 5062503, slot, c.getPlayer().itemQuantity(toUse.getItemId()) - 1, 1));
            ...
            case 5062010:
                ...
                if (item9 != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (neq.getState() >= 17) {
                        ...
                        c.getPlayer().gainMeso(-GameConstants.getCubeMeso(neq.getItemId()), false);
                        c.getSession().writeAndFlush(CField.getBlackCubeStart(c.getPlayer(), (Item) neq, bool5, 5062010, toUse.getPosition(), c.getPlayer().itemQuantity(toUse.getItemId()) - 1, c.getPlayer().getCubeStack(tt)));
...
//5000930, 5000931, 5000932,5002079,5002080,5002081,5002254,5002255,5002256
        ...
        try {
            ...
            if (mapitem.getItemId() == 2023484 || mapitem.getItemId() == 2023494
                ...
                if (mapitem.getDropper() instanceof MapleMonster) {
                    ...
                    chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 0, (int) mob.getMobExp() * c.getChannelServer().getExpRate() * bonus, 25, 0, 0, (byte) 0, true, null, "", null));
                }
                c.getSession().writeAndFlush(CWvsContext.InventoryPacket.getInventoryStatus(true));
                removeItem(chr, mapitem, ob);
                return;
        ...
        if (item.getItemId() == itemId) {
            ...
            switch (itemId) {
                ...
                case 2702004: { // 카오스 서큘레이터
                    ...
                    int count = 0;
                    for (InnerSkillValueHolder isvh : c.getPlayer().getInnerSkills().get(preset)) {
                        newValues.add(InnerAbillity.getInstance().renewLevel((count == 0 ? nowrank : (nowrank - 1)), isvh.getSkillId(), preset));
                ...
                case 2702006: { // 레전드리 서큘레이터
                    ...
                    for (InnerSkillValueHolder isvh : c.getPlayer().getInnerSkills().get(preset)) {
                        if (ivholder == null) {
                            ...
                            ivholder2 = InnerAbillity.getInstance().renewSkill(ivholder.getRank() == 0 ? 0 : ivholder.getRank(), false, preset);
                            while (isvh.getSkillId() == ivholder2.getSkillId() || ivholder.getSkillId() == ivholder2.getSkillId()) {
                                ivholder2 = InnerAbillity.getInstance().renewSkill(ivholder.getRank() == 0 ? 0 : ivholder.getRank(), false, preset);
                            }
                            newValues.add(ivholder2);
                        } else {
                            InnerSkillValueHolder ivholder3 = InnerAbillity.getInstance().renewSkill(ivholder.getRank() == 0 ? 0 : ivholder.getRank(), false, preset);
                            while (isvh.getSkillId() == ivholder3.getSkillId() || ivholder.getSkillId() == ivholder3.getSkillId() || ivholder2.getSkillId() == ivholder3.getSkillId()) {
                                ivholder3 = InnerAbillity.getInstance().renewSkill(ivholder.getRank() == 0 ? 0 : ivholder.getRank(), false, preset);
                            }
                            newValues.add(ivholder3);
                        }
                        c.getPlayer().changeSkillLevel_Inner(SkillFactory.getSkill(isvh.getSkillId()), (byte) 0, (byte) 0);
    ...
    public static final void MixHair(LittleEndianAccessor slea, MapleClient c) {
        ...
        int newhair = 0;
        if (c.getPlayer().getDressup() || (GameConstants.isZero(c.getPlayer().getJob()) && c.getPlayer().getGender() == 1)) {
            basehair = c.getPlayer().getSecondHair() < 100000 ? c.getPlayer().getSecondHair() : c.getPlayer().getSecondHair() / 1000;
            newhair = (basehair - basehair % 10 + mixColor / 10000) * 1000 + mixColor % 10000 / 1000 * 100 + mixColor % 100;
        ...
        c.getSession().writeAndFlush(CWvsContext.mixLense(itemId, basehair, newhair, false, false, false, c.getPlayer()));
        c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
    }
}
