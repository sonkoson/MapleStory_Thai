...
public class MatrixHandler {
    ...
    public static void UseCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
        ArrayList<Core> cores = new ArrayList<>();

        for (int count = 0; count < value; count++) {
            if (c.getPlayer().getCore().size() >= 500) {
                c.getPlayer().dropMessage(1, "코어는 최대 500개 까지 보유 하실 수 있습니다.");
                c.getSession().writeAndFlush(CWvsContext.AddCore(kind, cores));
                return;
            }
    ...
    public static void UseMirrorCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
        ArrayList<Core> cores = new ArrayList<>();
    ...
    public static void UseEnforcedCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
        ArrayList<Core> cores = new ArrayList<>();
    ...
    public static void UseCraftCoreJamStone(final MapleClient c, final int itemid, final int value, final int kind) {
        ArrayList<Core> cores = new ArrayList<>();
    ...
    public static void updateCore(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (state) {
            ...
            case 8: { // 코어 제작 1.2.390 OK.
                ...
                c.getPlayer().dropMessage(1, "코어 제작이 완료 되었습니다.");
                break;
            }
            case 10: { // 슬롯 강화 1.2.390 OK.
    ...
    public static void gainMatrix(MapleCharacter chr, boolean check) {
        List<VMatrix> matrixs = chr.getMatrixs();
        while (matrixs.size() < 25) {
            ...
            if (core.getState() == 2 && core.getPosition() >= 0) {
                if (core.getPosition() >= 28) {//코어 장착
                    ...
                    if (core.getSkill1() != 0) {
                        ...
                        if (core.getSkill1() == 400000006) {
                            addSkills.put(400001006, core.getLevel() + Math.max(0, matrix2.getLevel()));
                        }
