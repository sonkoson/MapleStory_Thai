...
public class DamageParse {
    ...
    public static void applyDebuff(MapleCharacter player, MapleMonster monster, SecondaryStatEffect effect, ArrayList<Triple<MonsterStatus, MonsterStatusEffect, Long>> statusz, int attackSkillId) {
        switch (attackSkillId) {
            case 15141000: { // 섬멸 VI 구현
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Speed, new MonsterStatusEffect(attackSkillId, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(effect.getX())));
                break;
            }
            ...
            case 23141004: { // 래쓰 오브 엔릴 VI : 스피릿 인챈트 구현
                ...
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_DodgeBodyAttack, new MonsterStatusEffect(attackSkillId, 60000), 1L));
                break;
            }
            ...
            case 31141001: { // 데몬 임팩트 VI : 데몬 체인 구현
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_IndiePdr, new MonsterStatusEffect(attackSkillId, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(-effect.getX())));
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_IndieMdr, new MonsterStatusEffect(attackSkillId, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(-effect.getX())));
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Pad, new MonsterStatusEffect(attackSkillId, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(-effect.getX())));
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Mad, new MonsterStatusEffect(attackSkillId, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(-effect.getX())));
                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Blind, new MonsterStatusEffect(attackSkillId, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(-effect.getZ())));
                break;
            }
            ...
            case 33141502: { // 오리진 네이쳐스 빌리프 구현
                ...
                if (monster.getAnotherByte() == 0) {
                    ...
                    statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_JaguarBleeding, new MonsterStatusEffect(33000036, 15000), (long) monster.getAnotherByte()));
                    break;
                }
    ...
    public static void applyHexaSkill(MapleCharacter player, MapleMonster monster, AttackInfo attack, SecondaryStatEffect effect) {
        // 6차 구현
        switch (attack.skill) {
            case 80003365: { // 오리진 바인드 구현
                ...
                applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Freeze, new MonsterStatusEffect(attack.skill, 10000, 1L)));
                applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Origin_Bind, new MonsterStatusEffect(attack.skill, 10000, 1L)));
            ...
            case 5141001: { // 오리진 리버레이트 넵투누스 구현
                ...
                if (player.getBuffedValue(5141501) && !player.skillisCooling(attackId) && player.getBuffedValue(5101017)) {
                    ...
                    attacks.add(new RangeAttack(attackId + 3, player.getPosition(), player.isFacingLeft() ? 0 : 1, 1860, 0));
                    attacks.add(new RangeAttack(attackId + 2, player.getPosition(), player.isFacingLeft() ? 0 : 1, 1140, 0));
        ...
        if (player.getBuffedValue(1241501)) { // 오리진 세이크리드 바스티온 구현
            ...
            if (!player.skillisCooling(attackId)) {
                ...
                attacks.add(new RangeAttack(attackId, monster.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 0));
                player.getClient().getSession().writeAndFlush(CField.rangeAttack(attackId, attacks));
    ...
    public static void applyAddAttack(MapleCharacter player, MapleMonster monster, AttackInfo attack) {
        ...
        if (player.getSkillLevel(15141000) > 0) { // 섬멸 VI 구현
            if (attack.skill == 15141000) {
                ...
                if (monsters.size() >= 0) {
                    player.getClient().getSession().writeAndFlush(SkillPacket.hexaStackMonster(monsters, 15141000, 90000));
                }
            } else {
                if (attack.skill != 15141001 && attack.skill != 15141002) {
                    ...
                    if (monsters.size() >= 0) {
                        int skillId = attack.skill == 15111022 || attack.skill == 15120003 || attack.skill == 400051016 || attack.skill == 400051060 || attack.skill == 400051096 || attack.skill == 15141500 || attack.skill == 15141501 ? 15141002 : 15141001;

                        player.getClient().getSession().writeAndFlush(SkillPacket.hexaStackAttackMonster(monsters, skillId, attack.skill, 290));
                    }
                }
            }
        }
        ...
        if (player.getBuffedValue(SecondaryStat.Unk_733) != null) { // 오리진 어센던트 셰이드 구현
            if (attack.allDamage.size() > 0) {
                if (!player.skillisCooling(3141502)) {
                    ...
                    player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(3141502, mobList, true, 0, null));
        ...
        if (attack.skill == 4141000) { // 쿼드러플 스로우 VI 구현
            if (attack.allDamage.size() > 0) {
                ...
                EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
                statups.put(SecondaryStat.Quadruple_Throw_Stack, new Pair<Integer, Integer>(player.hexaMasterySkillStack, 0));
        ...
        if (player.getBuffedValue(SecondaryStat.Origin_Forsaken_Relic) != null) { // 오리진 포세이큰 렐릭 구현
            if (!player.skillisCooling(3341503)) {
                if (attack.allDamage.size() > 0) {
                    ...
                    attacks.add(new RangeAttack(3341503, monster.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 180, 1));
                    attacks.add(new RangeAttack(3341503, monster.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 360, 1));
                    attacks.add(new RangeAttack(3341503, monster.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 540, 1));
        ...
        if (player.getBuffedValue(SecondaryStat.Origin_Life_And_Death) != null) { // 오리진 생사여탈 구현
            if (!player.skillisCooling(4141501)) {
                if (attack.allDamage.size() > 0) {
                    ...
                    attacks.add(new RangeAttack(4141501, mobs.isEmpty() ? player.getPosition() : mobs.get(0).getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    attacks.add(new RangeAttack(4141501, mobs.isEmpty() ? player.getPosition() : mobs.get(0).getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    attacks.add(new RangeAttack(4141501, mobs.isEmpty() ? player.getPosition() : mobs.get(0).getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    attacks.add(new RangeAttack(4141501, mobs.isEmpty() ? player.getPosition() : mobs.get(0).getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    attacks.add(new RangeAttack(4141501, mobs.isEmpty() ? player.getPosition() : mobs.get(0).getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
        ...
        if (effKarma != null && attack.skill != 4361503) { // 오리진 카르마 블레이드 구현
            if (attack.allDamage.size() > 0) {
                if (player.hexaMasterySkillStack > 0) {
                    ...
                    int resetTime = (int) (20000 - (System.currentTimeMillis() % 1000000000L - effKarma.getStarttime()));
                    EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
                    statups.put(SecondaryStat.Origin_Karma_Blade_Stack, new Pair<Integer, Integer>(player.hexaMasterySkillStack, resetTime));
                    player.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, effKarma, player));

                    Item weapon = player.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -11);
                    player.getMap().broadcastMessage(CField.finalAttackRequest(1, 4361501, 4341054, ((weapon.getItemId() - 1000000) / 10000), monster));

                    if (player.hexaMasterySkillStack <= 0) {
                        statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
                        statups.put(SecondaryStat.Origin_Karma_Blade_Stack, new Pair<Integer, Integer>(player.hexaMasterySkillStack, 0));
                        ...
                        skills.add(new RangeAttack(4361502, player.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 0));
                ...
                if (GameConstants.isSolaAndLunaSkill(attack.skill)) {
                    if (attack.allDamage.size() > 0) {
                        if (!player.skillisCooling(11141003)) {
                            ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                            skills.add(new RangeAttack(11141000, new Point(0, 0), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
        ...
        if (player.getSkillLevel(21141000) > 0) { // 비욘더 VI 구현
            if (attack.allDamage.size() > 0) {
                if (!player.skillisCooling(21141003)) {
                    ...
                    player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(21141003, mobList, true, 0, null));

                    player.addCoolTime(21141003, player.getBuffedEffect(SecondaryStat.AdrenalinBoost) != null ? 2000 : 4000);
                }
            }
        }
                ...
                if (skill.getSkillList().contains(attack.skill)) {
                    if (skill.getSkillList2().contains(attack.skill)) {
                        ...
                        if (monster.attackCount2 >= 12) {
                            ...
                            player.getClient().getSession().writeAndFlush(SkillPacket.stackMonster(monsters, 23141000, 23141001, 5, 90000));

                            if (monster.stigmaStack < 6) {
                                player.getClient().getSession().writeAndFlush(SkillPacket.stackAttackMonster(attackMonsters, 23141001, 0, 0));
                            } else {
                                monster.stigmaStack = 0;

                                player.getClient().getSession().writeAndFlush(SkillPacket.stackAttackMonster(attackMonsters, 23141002, 5, 0));
                                player.getClient().getSession().writeAndFlush(SkillPacket.stackMonster(monsters, 23141000, 23141001, 5, 90000));
                            }
                        }
                    } else {
        ...
        if (player.getBuffedValue(25141506)) { // 오리진 호신강림 구현
            if (attack.skill == 25141501 || attack.skill == 25141502 || attack.skill == 25141503) {
                if (!player.skillisCooling(400051043)) {
                    for (int i = 0; i < 6; i++) {
                        MapleAtom atom = new MapleAtom(false, player.getId(), 68, true, 25141505, player.getPosition().x, player.getPosition().y);
                        atom.addForceAtom(new ForceAtom(2, Randomizer.rand(10, 20), Randomizer.rand(20, 40), Randomizer.rand(40, 50), 630));
                        player.getMap().spawnMapleAtom(atom);
                    }

                    ArrayList<Triple<Integer, Integer, Integer>> mobList = new ArrayList<>();

                    player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(25141504, mobList, true, 0, null));
                }
            }
        }
        ...
        if (player.getBuffedEffect(SecondaryStat.Origin_Artificial_Evolution) != null) { // 오리진 아티피셜 에볼루션 구현
            if (attack.allDamage.size() > 0) {
                ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                skills.add(new RangeAttack(36141501, player.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 360, 1));
        ...
        if (player.getSkillLevel(37141002) > 0) {
            if (attack.skill == 37141002) { // 릴리즈 파일 벙커 VI 구현
                if (player.hexaMasterySkillStack < 5) {
                    ...
                    EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
                    statups.put(SecondaryStat.Release_Pile_Bunker_Stack, new Pair<Integer, Integer>(player.hexaMasterySkillStack, 0));
        ...
        if (attack.skill == 64141003) { // 체인아크:스트로크 VI 구현
            ...
            EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
            statups.put(SecondaryStat.ChainArts_Stroke_Stack, new Pair<Integer, Integer>(player.getSkillCustomValues(64141000), 0));
            ...
            if (attack.allDamage.size() > 0) {
                ...
                if (player.hexaMasterySkillStack >= 4) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        ...
                        MapleMagicWreck mw = new MapleMagicWreck(player, isBeta ? timePieceId + 1 : timePieceId, new Point(player.getPosition().x + Randomizer.rand(-200, 200), player.getPosition().y - Randomizer.rand(50, 250)), 20000);
            ...
            if (attack.allDamage.size() > 0) {
                if (!player.skillisCooling(22201501)) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                        skills.add(new RangeAttack(22201501, monster.getPosition(), player.isFacingLeft() ? 1 : 0, delays[i], 1));
            ...
            if (player.getBuffedValue(20040217) || player.getBuffedValue(20040219) || player.getBuffedValue(20040220)) {
                if (GameConstants.isDarkSkills(attack.skill) || GameConstants.isEquilibriumSkills(attack.skill)) {
                    if (!player.skillisCooling(27141003)) {
                        ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                        skills.add(new RangeAttack(27141003, monster.getPosition(), player.isFacingLeft() ? 1 : 0, 300, 1));
            ...
            if (GameConstants.isDemonSlash(attack.skill)) {
                if (attack.allDamage.size() > 0) {
                    if (!player.skillisCooling(31141007)) {
                        ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                        skills.add(new RangeAttack(31141006, player.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 90, 1));
            ...
            if (SkillFactory.getSkill(33141501).getSkillList().contains(attack.skill)) {
                if (attack.allDamage.size() > 0) {
                    if (!player.skillisCooling(33141501)) {
                        ...
                        player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(33141501, mobList, true, 780, 0));
            ...
            if (attack.allDamage.size() > 0) {
                if (monster != null && monster.isBuffed(MonsterStatus.MS_JaguarBleeding)) {
                    if (player.hexaMasterySkillStack < 50) {
                        ...
                        SkillFactory.getSkill(33141004).getEffect(player.getSkillLevel(33141000)).applyTo(player, false);
                    } else {
                        player.hexaMasterySkillStack = 0;
                ...
                if (attack.allDamage.size() > 0) {
                    if (monster != null) {
                        if (!player.skillisCooling(63141502)) {
                            ...
                            player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(63141502, mobList, true, 0, new int[]{0, 0}));
        ...
        if (player.getBuffedValue(155141501)) { // 오리진 가장 오래된 심연 구현
            if (attack.allDamage.size() >= 0) {
                if (!player.skillisCooling(155141501)) {
                    ...
                    player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(155141501, mobList, true, 0, null));
            ...
            block859:
            {
                ...
                for (Object oned : attack.allDamage) {
                    ...
                    if (attack.skill == enhanceBatAttackId) {
                        ...
                        player.getClient().getSession().writeAndFlush(CField.rangeAttack(enhanceBatRangeAttackFirstId, skills));
                    }
                    ...
                    if (player.getBuffedValue(14001027) || player.getBuffedValue(14141004)) {
                        ...
                        for (int skill : skills) {
                            if (player.getSkillLevel(skill) > 0) {
                                attackChange += SkillFactory.getSkill(skill).getEffect(player.getSkillLevel(skill)).getProp();
                                limitBatCount += SkillFactory.getSkill(skill).getEffect(player.getSkillLevel(skill)).getY();
                            }
                        }
                        ...
                        // 사출 (쉐도우 배트)
                        if (bat != null && batSummonCount > 0 && attack.skill != batAttackId && attack.skill != enhanceBatAttackId && attack.skill != enhanceBatRangeAttackFirstId && attack.skill != enhanceBatRangeAttackSecondId) {
                            if (Randomizer.isSuccess(attackChange)) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), 15, true, batAttackId, player.getTruePosition().x, player.getTruePosition().y);
                                ForceAtom forceAtom = new ForceAtom(15, 1, 5, Randomizer.rand(45, 90), 500);

                                List<MapleMapObject> objects = player.getMap().getMapObjectsInRange(player.getTruePosition(), 50000.0, Arrays.asList(new MapleMapObjectType[]{MapleMapObjectType.MONSTER}));
                                ...
                                if (!already && objects.size() > 0) {
                                    atom.setDwFirstTargetId(objects.get(Randomizer.rand(0, objects.size() - 1)).getObjectId());
                                }
                        ...
                        // 사출 (강화 쉐도우 배트)
                        if (enhanceBat != null && enhanceBatSummonCount > 0 && attack.skill != batAttackId && attack.skill != enhanceBatAttackId && attack.skill != enhanceBatRangeAttackFirstId && attack.skill != enhanceBatRangeAttackSecondId) {
                            if (Randomizer.isSuccess(attackChange)) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), 15, true, enhanceBatAttackId, player.getTruePosition().x, player.getTruePosition().y);
                                ForceAtom forceAtom = new ForceAtom(15, 1, 5, Randomizer.rand(45, 90), 500);
                        ...
                        // 스폰 (쉐도우 배트)
                        if (batSummonCount < limitBatCount && attack.skill != batAttackId && attack.skill != enhanceBatAttackId && attack.skill != enhanceBatRangeAttackFirstId && attack.skill != enhanceBatRangeAttackSecondId) {
                            ...
                            if (player.attackCountBat >= 3) {
                                ...
                                if (player.spawnCountBat < 5) { // 쉐도우 배트
                                    ...
                                    SecondaryStatEffect effBat = SkillFactory.getSkill(batId).getEffect(player.getSkillLevel(batId));
                                    effBat.applyTo(player, player.getPosition(), false, 60000);
                                } else { // 강화 쉐도우 배트
                                    player.spawnCountBat = 0;

                                    SecondaryStatEffect effBat = SkillFactory.getSkill(enhanceBatId).getEffect(player.getSkillLevel(enhanceBatId));
                                    effBat.applyTo(player, player.getPosition(), false, 60000);
                                }
                            }
                        }
                    ...
                    if (player.getBuffedValue(14121016) || player.getBuffedValue(14141012)) {
                        ...
                        // 사출 (래버너스 배트)
                        if (ravenousBat != null && ravenousBatSummonCount > 0 && attack.skill != ravenousBatAttackId && attack.skill != enhanceRavenousBatAttackId) {
                            if (Randomizer.isSuccess(attackChange)) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), 15, true, ravenousBatAttackId, player.getTruePosition().x, player.getTruePosition().y);
                                ForceAtom forceAtom = new ForceAtom(15, 1, 5, Randomizer.rand(45, 90), 500);
                        ...
                        // 사출 (강화 래버너스 배트)
                        if (enhanceRavenousBat != null && enhanceRavenousBatSummonCount > 0 && attack.skill != ravenousBatAttackId && attack.skill != enhanceRavenousBatAttackId) {
                            if (Randomizer.isSuccess(attackChange)) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), 15, true, enhanceRavenousBatAttackId, player.getTruePosition().x, player.getTruePosition().y);
                                ForceAtom forceAtom = new ForceAtom(15, 1, 5, Randomizer.rand(45, 90), 500);
                                ...
                                player.getMap().broadcastMessage(CField.SummonPacket.updateSummon(enhanceRavenousBat, 99));
                                player.getMap().broadcastMessage(CField.SummonPacket.removeSummon(enhanceRavenousBat, true));
                                player.removeSummon(enhanceRavenousBat);
                            }
                        }

                        // 스폰 (래버너스 배트)
                        if (ravenousBatSummonCount < limitBatCount && attack.skill != ravenousBatAttackId && attack.skill != enhanceRavenousBatAttackId) {
                            player.attackCountBat++;
                            ...
                            if (player.attackCountBat >= 3) {
                                ...
                                if (player.spawnCountBat < 5) { // 래버너스 배트
                                    ...
                                    SecondaryStatEffect effBat = SkillFactory.getSkill(ravenousBatId).getEffect(player.getSkillLevel(ravenousBatId));
                                    effBat.applyTo(player, player.getPosition(), false, 60000);
                                } else { // 강화 래버너스 배트
                                    player.spawnCountBat = 0;

                                    SecondaryStatEffect effBat = SkillFactory.getSkill(enhanceRavenousBatId).getEffect(player.getSkillLevel(enhanceRavenousBatId));
                                    effBat.applyTo(player, player.getPosition(), false, 60000);
                                }
                            }
                        }
                    ...
                    if (player.getBuffedEffect(SecondaryStat.Origin_Nightmare) != null) { // 오리진 나이트메어 구현
                        if (!player.skillisCooling(31141502)) {
                            ...
                            for (AttackPair tempOned : attack.allDamage) {
                                ...
                                player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(31141502, mobList, attack.skill));
                    ...
                    if (player.getBuffedValue(SecondaryStat.BeyondNextAttackProb) != null && attack.skill != attackId && !player.skillisCooling(attackId)) {
                        player.getClient().getSession().writeAndFlush(CField.rangeAttack(attackId, Arrays.asList(new RangeAttack(attackId, player.getTruePosition(), 0, 0, 1))));

                        player.addCoolTime(attackId, 5000);
                    }

                    if (effect != null && effect.getSourceId() == 37121004) {
                        player.getMap().broadcastMessage(CField.RebolvingBunk(player.getId(), monster.getObjectId(), monster.getId(), monster.getPosition()));
                    }
                    ...
                    if (monster != null && player.getBuffedValue(SecondaryStat.PickPocket) != null) {
                        ...
                        switch (attack.skill) {
                            ...
                            case 4241502: {
                                ...
                                int i;
                                int max = SkillFactory.getSkill(skillId).getEffect(player.getSkillLevel(skillId)).getBulletCount();
                    ...
                    if (GameConstants.isBowMaster(player.getJob())) {
                        if (player.getBuffedValue(3111015) && (SkillFactory.getSkill(3111015).getSkillList().contains(attack.skill) || SkillFactory.getSkill(3111015).getSkillList2()
                                .contains(attack.skill))) {
                            ...
                            if (charge) {
                                ...
                                if (player.getSkillCustomValue0(3111015) >= max) {
                                    player.getClient().send(CField.FlashMirage(player.getSkillLevel(3120021) > 0 ? 4 : 3, new ArrayList<>()));
                    ...
                    if (player.getBuffedValue(SecondaryStat.SeaSerpent) != null) {
                        ...
                        if (Arrays.asList(격투스킬).contains(attack.skill)) {
                            ...
                            if (!chr.skillisCooling(skillid) && !chr.getBuffedValue(5110020)) {
                                ...
                                if (isHexa) { // 씨 서펜트 VI 구현
                                    attacks.add(new RangeAttack(5141008, chr.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                                } else {
                                    attacks.add(new RangeAttack(5121023, chr.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                                }
                        ...
                        } else if (attack.skill == 5121007 || attack.skill == 5141000 || attack.skill == 5141001) { // 피스트 인레이지 VI 구현
                            if (!chr.skillisCooling(5121025) && !chr.getBuffedValue(5110020)) {
                                List<RangeAttack> attacks = new ArrayList<>();
                                if (isHexa) { // 씨 서펜트 VI 구현
                                    attacks.add(new RangeAttack(5141006, chr.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                                } else {
                                    attacks.add(new RangeAttack(5121025, chr.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                                }
                                chr.getClient().getSession().writeAndFlush(CField.rangeAttack(5101017, attacks));
                                if (chr.서펜트스톤 < max) {
                                    chr.서펜트스톤++;
                                ...
                                atoms.add(new SecondAtom(57, player.getId(), 0, 5121027, 5000, 0, 15, new Point(monster.getPosition().x, monster.getPosition().y), Arrays.asList(0)));
                    ...
                    if (GameConstants.isMarksMan(player.getJob())) {
                        ...
                        SecondaryStatEffect concentration = SkillFactory.getSkill(3220021).getEffect(player.getSkillLevel(3220021));
                    ...
                    if (GameConstants.isViper(player.getJob())) {
                        if (player.getBuffedValue(5121052)) {
                            SecondaryStatEffect SerpentStone = SkillFactory.getSkill(5121052).getEffect(player.getSkillLevel(5121052));
                            SkillFactory.getSkill(5121055).getEffect(player.getSkillLevel(5121055)).applyTo(player, player.getPosition());
                        }
                        //[othello] - 바이퍼 하울링 피스트 무적
                        if (attack.skill == 400051070) {
                            SkillFactory.getSkill(400051071).getEffect(player.getSkillLevel(400051070)).applyTo(player, false, false);
                        }
                    }
                    //메기도 플레임 재생성
                    ...
                    if (player.getSkillLevel(400051015) > 0 && GameConstants.isViper(player.getJob())) {
                        if (monster != null) {
                            if (player.서펜트스크류 == 1 && !player.skillisCooling(400051015) && !player.getBuffedValue(400051015)) {
                                ...
                                statups.put(SecondaryStat.SerpentScrew, new Pair<Integer, Integer>((int) player.서펜트스크류, 0));
                            }
                            if (monster.getStats().isBoss()) {
                                if (monster.isAlive()) {
                            ...
                            if (player.Serpent > 300) {
                                ...
                                if (player.skillisCooling(400051015)) {
                                    ...
                                    player.addCooldown(400051015, System.currentTimeMillis(), SkillFactory.getSkill(400051015).getEffect(player.getSkillLevel(400051015)).getCooldown(player));
                                    player.getClient().getSession().writeAndFlush((Object) CField.skillCooldown(400051015, SkillFactory.getSkill(400051015).getEffect(player.getSkillLevel(400051015)).getCooldown(player)));
                                }
                        ...
                        if (player.getBuffedValue(SecondaryStat.Reincarnation) != null && monster != null) {
                            if (player.getReinCarnation() > 0 && monster.getStats().isBoss()) {
                                ...
                                if (player.getReinCarnation() == 0) {
                                    SecondaryStatEffect eff = SkillFactory.getSkill(1320016).getEffect(player.getSkillLevel(1320016));
                                    int reduce = 0;
                                    ...
                                    player.getClient().send(CField.EffectPacket.showEffect(player, 0, 1320016, 10, 0, 0, (byte) (player.isFacingLeft() ? 1 : 0), true, null, null, null));
                                    player.getMap().broadcastMessage(player, CField.EffectPacket.showEffect(player, 0, 1320016, 10, 0, 0, (byte) (player.isFacingLeft() ? 1 : 0), false, null, null, null), false);
                                }
                                ...
                                // Buff (2)
                                Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<>();
                                statups.put(SecondaryStat.Reincarnation, new Pair<>(player.getReinCarnation(), (int) player.getBuffLimit(SecondaryStat.Reincarnation, 1320019)));
                                player.getClient().send(CWvsContext.BuffPacket.giveBuff(statups, player.getBuffedEffect(SecondaryStat.Reincarnation), player, buffSkillid));
                            }
                        }
                    } else if (GameConstants.isCannon(player.getJob())) {
                        if ((attack.skill == 5321000 || attack.skill == 5321012 || attack.skill == 5341000 || attack.skill == 5341001) && monster != null && attack.targets > 0) { // 캐논 버스터 VI 구현 / 캐논 바주카 VI 구현
                            boolean isHexa = player.getSkillLevel(5341002) > 0;
                            long count = player.getSkillCustomValue(isHexa ? 5341002 : 5311013);
                            if (player.isAutoSkill(isHexa ? 5341002 : 5311013) && !player.skillisCooling(isHexa ? 5341002 : 5311013) && count > 0) {
                                ...
                                player.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, effect, player));
                                SecondaryStatEffect eff = SkillFactory.getSkill(isHexa ? 5341002 : 5311013).getEffect(player.getSkillLevel(isHexa ? 5341002 : 5311013));
                                player.getClient().send(CField.skillCooldown(isHexa ? 5341002 : 5311013, eff.getCooldown(player)));
                                player.addCooldown(isHexa ? 5341002 : 5311013, System.currentTimeMillis(), eff.getCooldown(player));
                            }
                        }
                    }
                    ...
                    if (GameConstants.isMarksMan(player.getPlayer().getJob())) {
                        if (attack.skill == 400031010) {
                            SkillFactory.getSkill(400031010).getEffect(player.getPlayer().getSkillLevel(400031010)).applyTo(player.getPlayer(), false, false);
                        }
                    }
                    ...
                    if (GameConstants.isMechanic(player.getPlayer().getJob())) {
                        if (attack.skill == 400051041) {
                            SkillFactory.getSkill(400051094).getEffect(player.getPlayer().getSkillLevel(400051041)).applyTo(player.getPlayer(), false, false);
                        }
                    }
                    //엔버 소울 레조넌스 무적
                    ...
                    if (GameConstants.isSoulMaster(player.getJob())) {
                        if (player.isAutoSkill(11121018) && player.getCosmicCount() >= 5) {
                            player.getClient().getSession().writeAndFlush(CField.rangeAttack(11121018, Arrays.asList(new RangeAttack(11121018, player.getTruePosition(), 1, 330, 1))));
                        }
                    }
                    ...
                    if (player.getSkillLevel(400051015) > 0 && attack.skill == 400051015 && attack.targets > 0 && player.getBuffedValue(SecondaryStat.SerpentScrew) != null) {
                        SecondaryStatEffect eff = SkillFactory.getSkill(400051015).getEffect(player.getSkillLevel(400051015));
                    ...
                    if (attack.skill == 1221009 || attack.skill == 1241000 || attack.skill == 1241502) { // 신성 낙인 구현, 오리진 세이크리드 바스티온 구현
                        monster.stigmaStack++;
                        ...
                        if (monster.stigmaStack >= 5) {
                            ...
                            player.getMap().broadcastMessage(SkillPacket.stackAttackMonster(attackMonsters, 1221023, 5, 202));
                        }
                        ...
                        player.getMap().broadcastMessage(SkillPacket.stackMonster(monsters, 1220022, 1221023, 5, 30000));
                    }
                    ...
                    if (player.getBuffedValue(SecondaryStat.QuiverCatridge) != null && attack.skill != 400031021 && attack.skill != 95001000 && attack.skill != 3111013 && attack.skill != 3100010 && attack.skill != 400031000) {
                        ...
                        SecondaryStatEffect adq = SkillFactory.getSkill(3120022).getEffect(player.getSkillLevel(3120022));
                        ...
                        if (quiverFoolburst) {
                            ...
                            if (Randomizer.isSuccess(quiverEff.getW()) && player.getBuffedEffect(SecondaryStat.DebuffIncHp) == null) {
                                player.addHP((int) ((double) player.getStat().getCurrentMaxHp() * ((double) adq.getX() / 100.0)));
                            }
                    ...
                    if (player.getJob() == 112) {
                        if ((attack.skill == 1101011
                                || attack.skill == 1111010
                                || attack.skill == 1121008
                                || attack.skill == 1121015) && attack.targets > 0 && player.getBuffedValue(1121054) && player.발할라검격 >= 1) {
                            player.발할라검격--;

                            MapleSummon summon4 = new MapleSummon(player, 1121055, monster.getPosition(), SummonMovementType.STATIONARY, (byte) 0, 30000);
                    ...
                    if (markOf != null && nk != null) {
                        ...
                        if (attack.skill != 400041038 && attack.skill != 4100012 && attack.skill != 4120019 && attack.skill != 4141004) {
                            if (monster.isBuffed(markOf.getSourceId()) || !monster.isAlive() && Randomizer.isSuccess(markOf.getProp())) {
                                if (attack.skill != 400041020 || attack.skill == 400041020 && Randomizer.isSuccess(SkillFactory.getSkill(400041020).getEffect(player.getSkillLevel(400041020)).getW())) {
                                    ...
                                    MapleAtom atom;
                                    if (player.getSkillLevel(4141002) > 0) {
                                        atom = new MapleAtom(true, monster.getObjectId(), 11, true, monster.isBuffed(4141003) ? 4141004 : 4100012, monster.getTruePosition().x, monster.getTruePosition().y);
                                    } else {
                                        atom = new MapleAtom(true, monster.getObjectId(), 11, true, monster.isBuffed(4120018) ? 4120019 : 4100012, monster.getTruePosition().x, monster.getTruePosition().y);
                                    }
                                    atom.setDwUserOwner(player.getId());
                                    atom.setDwTargets(monsters);
                        ...
                        switch (attack.skill) {
                            ...
                            case 1201015: {
                                if (!monster.getStats().isBoss() && Randomizer.isSuccess(90)) {
                                    statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Stun, new MonsterStatusEffect(attack.skill, effect.getSubTime() > 0 ? effect.getSubTime() : 6000), 1L));
                                }
                            ...
                            case 1241004: {
                                ...
                                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Blind, new MonsterStatusEffect(attack.skill, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(effect.getDuration())));
                                break;
                            }
                            ...
                            case 51141001: { // 샤이닝 크로스-어썰트 VI 구현
                                ...
                                if (Randomizer.isSuccess(30)) {
                                    statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Blind, new MonsterStatusEffect(attack.skill, 10000), 10000L));
                                }
                            ...
                            // 작업 - 보마 튕김
                            case 3121052: {
                                // statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_sl, new MonsterStatusEffect(attack.skill, effect.getSubTime() > 0 ? effect.getSubTime() : effect.getDuration()), Long.valueOf(effect.getS())));
                                break;
                            }
                            case 3201008: {
                            ...
                            case 25121007: {
                                if (monster.getStats().getCategory() == 1 || monster.getId() == 8880502 || monster.getId() == 8880522 || monster.getId() == 8644650 || monster.getId() == 8644655 || monster.getId() == 8880342 || monster.getId() == 8880302) {
                            ...
                            case 400041007: {
                                ...
                                statusz.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Indie_Unk4, new MonsterStatusEffect(36110005, effect2.getDuration()), Long.valueOf(monster.getAirFrame())));
                            ...
                            default: {
                                ...
                                if (player.getBuffedValue(1111003)) {
                                    applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Blind, new MonsterStatusEffect(1111003, 20000, 1)));
                                    applys.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_Pad, new MonsterStatusEffect(1111003, 20000, 1)));
                                    break;
                                }
                        ...
                        if (player.getBuffedValue(SecondaryStat.BleedingToxin) != null) {
                            ...
                            if (SkillFactory.getSkill(63110011).getSkillList().contains(attack.skill) && monster.isAlive()) {
                                ...
                                if (cast) {
                                    ...
                                    if (monster.getCustomValue0(63110011) >= (player.getBuffedValue(63141503) ? 20 : 15)) {
                                        monster.setCustomInfo(63110011, (player.getBuffedValue(63141503) ? 20 : 15), 90000);
                                    }
                                ...
                                if (monster.isAlive() && monster.getCustomValue0(63110011) > 0L) {
                                    ...
                                    player.getClient().send(CField.getDeathBlessAttack(moblist, player.getBuffedValue(63141503) ? 63141505 : 63111012));
                                    moblist.clear();
                                    for (MapleMonster mob : player.getMap().getAllMonster()) {
                                        if (mob.getCustomValue0(63110011) <= 0L) {
                                            continue;
                ...
                if (player.getKeyValue(21770, "500001000") == 500001001 && !player.skillisCooling(500001001)) { // HEXA 솔 야누스 구현
                    ArrayList<RangeAttack> attacks = new ArrayList<>();

                    attacks.add(new RangeAttack(500001001, player.getPosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                ...
                if (attack.skill == 35121016) { // 매시브 파이어 : IRON-B 구현
                    if (monster != null) {
                        ...
                        dif = (dif < 0 ? -dif : dif);
                        player.getClient().getSession().writeAndFlush((Object) CField.bonusAttackRequest(35121019, mobList, true, 0, (dif * 3), monster.getPosition().x, monster.getPosition().y));
                    }
                }
                ...
                if (attack.skill == 35141001) { // 매시브 파이어 : IRON-B 구현
                    if (monster != null) {
                        ...
                        int dif = (attack.position.x - monster.getPosition().x);
                        dif = (dif < 0 ? -dif : dif);
                        player.getClient().getSession().writeAndFlush((Object) CField.bonusAttackRequest(35141002, mobList, true, 0, dif, monster.getPosition().x, monster.getPosition().y));
                    }
                }
                ...
                if (player.getSkillLevel(155141000) > 0) { // 깨어난 심연 구현
                    ...
                    if (skill.getSkillList().contains(attack.skill)) {
                        if (player.hexaMasterySkillStack > 0) {
                            ...
                            SecondaryStatEffect eff = SkillFactory.getSkill(155141001).getEffect(player.getSkillLevel(155141000));
                            player.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, eff, player));
                            ...
                            player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(155141001, monsters, true, attack.skill));
                        }
                    }
                }
                ...
                if (attack.skill == 5121013 || attack.skill == 5221013 || attack.skill == 400051040) {
                    if (player.getSkillLevel(5121013) > 0 && attack.skill == 5121013 && player.getSkillLevel(400051040) > 0 && player.getCooldownLimit(400051040) <= 8000L) {
                        ...
                        int[] array2;
                        int[] reduceSkills = array2 = new int[]{5210015, 5210016, 5210017, 5210018, 5220014, 5211007, 5221022, 5220023, 5220024, 5220025, 5241002}; // 배틀쉽 봄버 VI 구현
                ...
                if (totDamage > 0L) {
                    ...
                    if (monster != null && player.getBuffedValue(isHexa ? 65141002 : 65121011) && attack.skill != 65120011 && attack.skill != 65111007 && attack.skill != 65141003 && attack.skill != 65141004 && attack.skill != 65141502 && attack.skill != 60011216 && attack.skill < 400000000) {
                        ...
                        if (Randomizer.isSuccess(prop)) {
                            for (int i = 0; i < 2; ++i) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), isHexaBuff ? 93 : 25, true, isHexa ? 65141004 : 65120011, player.getTruePosition().x, player.getTruePosition().y);
                                ArrayList<Integer> monsters = new ArrayList<Integer>();
                                monsters.add(0);
                ...
                if (player.isAutoSkill(400051044)) { //[othello] 뇌신창격 오토모드 이펙트 처리
                    ...
                    if (player.striker3rdStack >= 8 && !player.skillisCooling(400051044)) {
                        ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                        skills.add(new RangeAttack(400051044, player.getPosition(), player.isFacingLeft() ? -1 : 0, 0, 0));
                        skills.add(new RangeAttack(400051045, player.getPosition(), player.isFacingLeft() ? -1 : 0, 0, 0));
                        skills.add(new RangeAttack(400051045, player.getPosition(), player.isFacingLeft() ? -1 : 0, 0, 0));
                        skills.add(new RangeAttack(400051045, player.getPosition(), player.isFacingLeft() ? -1 : 0, 0, 0));
                        skills.add(new RangeAttack(400051045, player.getPosition(), player.isFacingLeft() ? -1 : 0, 0, 0));
                        player.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(player, 0, 400051096, 1, 0, 0, (byte) (player.isFacingLeft() ? 1 : 0), true, player.getTruePosition(), null, null));
                        player.getMap().broadcastMessage(player, CField.EffectPacket.showEffect(player, 0, 400051096, 1, 0, 0, (byte) (player.isFacingLeft() ? 1 : 0), false, player.getTruePosition(), null, null), false);
                ...
                //921170011
                if (!GameConstants.사출기(attack.skill) || attack.targets > 0 && player.getMapId() == 921170011 || attack.targets > 0 && player.getMapId() == 921170004 || attack.targets > 0 && player.getMapId() == 921170012) {
                    if (attack.targets > 0 && player.getKeyValue(99999, "tripling") > 0
                            ...
                            && attack.skill != 500001001
                            || attack.targets > 0 && player.getMapId() == 921170012 || attack.targets > 0 && player.getMapId() == 921170011 || attack.targets > 0 && player.getMapId() == 921170004 || attack.targets > 0 && attack.skill != 1311020 && player.getBuffedEffect(SecondaryStat.TryflingWarm) != null && !GameConstants.isTryFling(attack.skill) && attack.skill != 400031031 && attack.skill != 400031001 && attack.skill != 13111020 && attack.skill != 13121054 && attack.skill != 13101022 && attack.skill != 13110022 && attack.skill != 13120003 && attack.skill != 13111020 && attack.skill != 400001018 && attack.targets > 0) {
                        int skillid = 0;
                        if (player.getSkillLevel(13141001) > 0) { // 트라이플링 윔 VI 구현
                            skillid = 13141001;
                        } else if (player.getKeyValue(99999, "tripling") > 0 || player.getMapId() == 921170004 || player.getMapId() == 921170012 || player.getMapId() == 921170011 || player.getKeyValue(1234, "ww_whim") > 0 || player.getSkillLevel(SkillFactory.getSkill(13120003)) > 0) {
                            skillid = 13120003;
                                ...
                                if (Randomizer.isSuccess(trychance)) {
                                    ...
                                    for (int i = 0; i < Randomizer.rand(1 + (int) (player.getKeyValue(99999, "triplingBonus") > 0 ? (player.getKeyValue(99999, "triplingBonus") * 1) : 0), maxcount); ++i) {
                                        ...
                                        if (skillid == 13141001) { // 트라이플링 윔 VI 구현
                                            atom.addForceAtom(new ForceAtom(upgrade ? 4 : 3, Randomizer.rand(41, 49), Randomizer.rand(4, 8), Randomizer.nextBoolean() ? Randomizer.rand(171, 174) : Randomizer.rand(6, 9), (short) Randomizer.rand(42, 47)));
                                        } else {
                                            atom.addForceAtom(new ForceAtom(upgrade ? 2 : 1, Randomizer.rand(41, 49), Randomizer.rand(4, 8), Randomizer.nextBoolean() ? Randomizer.rand(171, 174) : Randomizer.rand(6, 9), (short) Randomizer.rand(42, 47)));
                                        }
                                    }
                                    atom.setDwTargets(monsters);
                                    player.getMap().spawnMapleAtom(atom);
...
//                }
        ...
        if (player.getSkillLevel(400011048) > 0 && !player.skillisCooling(400011048) && attack.targets > 0 && SkillFactory.getSkill(400011048).getSkillList().contains(attack.skill)) {
            ...
            player.getClient().getSession().writeAndFlush(CField.rangeAttack(400011048, Arrays.asList(new RangeAttack(400011049, attack.position, 1, 0, 1))));

            int ViperSkilltype = ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1;
            player.getClient().getSession().writeAndFlush(CField.rangeAttack(attackid, Arrays.asList(new RangeAttack(attackid, attack.position, ViperSkilltype, 0, 1))));

            SkillFactory.getSkill(400011048).getEffect(player.getSkillLevel(400011048)).applyTo(player, player.getPosition());
        ...
        if (player.getSkillLevel(400041075) > 0 && player.getCooldownLimit(400041075) == 0L && (attack.skill == 4341004 || attack.skill == 4341009 || attack.skill == 4361000)) { //사라졌?
        ...
        if ((attack.skill == 400011056 || attack.skill == 11121055 || attack.skill == 400011056) && player.getSkillLevel(400011048) > 0 && player.getCooldownLimit(400011048) > 0L) {
        ...
        if (GameConstants.isKhali(player.getJob())) {
            ...
            if (player.getSkillLevel(154110005) > 0 && player.getSkillCustomValue(154110005) == null) {
                ...
                player.getClient().getSession().writeAndFlush((Object) CField.bonusAttackRequest(154110005, new ArrayList<Triple<Integer, Integer, Integer>>(), true, 2, new int[0]));
            }
        }

        //칼리
        if (player.getBuffedValue(SecondaryStat.SummonChakri) != null && player.getBuffedValue(SecondaryStat.Ultimatum) == null) { //서먼차크리만 갖고 있을때
        ...
        if (effect != null && player.getBuffedValue(SecondaryStat.OverloadMana) != null && !GameConstants.is_forceAtom_attack_skill(attack.skill) && !effect.isMist()) {
            if (GameConstants.isKinesis(player.getJob())) {
                player.addHP((int) (-(player.getStat().getCurrentMaxHp() * (long) player.getBuffedEffect(SecondaryStat.OverloadMana).getY() / 1000L)));
            } else {
                player.addMP((int) (-(player.getStat().getCurrentMaxMp(player) * (long) player.getBuffedEffect(SecondaryStat.OverloadMana).getX() / 1000L)));
            }
        }
        ...
        if (GameConstants.isAdel(player.getJob())) {
            ...
            if (player.getBuffedValue(isHexa ? 151141003 : 151101013) && player.getSkillCustomValue(isHexa ? 151141003 : 151101013) == null && attack.targets > 0 && (attack.skill == 151101000 || attack.skill == 151111000 || attack.skill == 151121000 || attack.skill == 151121002 || attack.skill == 151141000 || attack.skill == 151141001 || attack.skill == 151141005)) {
            ...
            if (player.getSkillLevel(151100017) > 0 && attack.targets > 0 && (attack.skill == 151101000 || attack.skill == 151111000 || attack.skill == 151121000 || attack.skill == 151121002 || attack.skill == 151141000 || attack.skill == 151141001 || attack.skill == 151141005)) {
                player.에테르핸들러(player, 12, attack.skill, false);
            }
            if (player.getBuffedValue(151101006) && player.에테르소드 > 0 && player.getSkillCustomValue(151101016) == null && attack.targets > 0 && (attack.skill == 151101000 || attack.skill == 151111000 || attack.skill == 151121000 || attack.skill == 151121002 || attack.skill == 151141000 || attack.skill == 151141001 || attack.skill == 151141005)) {
            ...
            if (attack.targets > 0) {
                ...
                if (attack.skill == 63001000 || attack.skill == 63101003 || attack.skill == 63111002 || attack.skill == 63141007) {
                    RangeAttack rand = new RangeAttack(attack.skill == 63141007 ? 63141008 : 63001001, attack.position, player.isFacingLeft() ? 1 : 0, 0, 1);
                    ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                    skills.add(rand);
                    player.getClient().getSession().writeAndFlush((Object) CField.rangeAttack(attack.skill, skills));
                }
        ...
        if (GameConstants.isHoyeong(player.getJob())) {
            ...
            if (player.getBuffedValue(SecondaryStat.ButterflyDream) != null && SkillFactory.getSkill(164121007).getSkillList().contains(attack.skill) && player.getSkillCustomValue(164121007) == null) {
                ...
                if (Randomizer.isSuccess(prop)) {
                    for (int i = 0; i < 5; ++i) {
                        MapleAtom atom = new MapleAtom(false, player.getId(), 61, true, 164120007, player.getTruePosition().x, player.getTruePosition().y);
                        ArrayList<Integer> monsters = new ArrayList<Integer>();
                        monsters.add(0);
                        atom.addForceAtom(new ForceAtom(i, Randomizer.rand(15, 146), Randomizer.rand(27, 304), Randomizer.rand(31, 360), 500));
                        atom.addForceAtom(new ForceAtom(1 + i, Randomizer.rand(15, 100), Randomizer.rand(27, 340), Randomizer.rand(31, 306), 500));
                        atom.addForceAtom(new ForceAtom(2 + i, Randomizer.rand(15, 99), Randomizer.rand(27, 100), Randomizer.rand(31, 236), 500));
                        atom.setDwTargets(monsters);
                        player.getMap().spawnMapleAtom(atom);
                    }
                }
            ...
            if (attack.skill == 155141004 || attack.skill == 155141005) { // 스칼렛 차지드라이브 VI 구현
                ...
                player.getClient().getSession().writeAndFlush((Object) CField.rangeAttack(attack.skill, Arrays.asList(new RangeAttack(155141007, attack.position, 0, 0, 1))));
            } else if (attack.skill == 155141006) { // 스칼렛 차지드라이브 VI 구현
                isLinkAttack = true;

                player.getClient().getSession().writeAndFlush((Object) CField.rangeAttack(attack.skill, Arrays.asList(new RangeAttack(155141008, attack.position, 0, 0, 1))));
            } else if (attack.skill == 155141016) { // 어비스 차지드라이브 VI 구현
                isLinkAttack = true;

                player.getClient().getSession().writeAndFlush((Object) CField.rangeAttack(attack.skill, Arrays.asList(new RangeAttack(155141017, attack.position, 0, 0, 1))));
            }
        ...
        if (player.getSkillLevel(400041063) > 0 && attack.targets > 0 && !player.skillisCooling(400041067)) {
            ...
            if (Arrays.asList(skills).contains(attack.skill)) {
                ...
                if (attack.targets > 0 && player.getBuffedEffect(SecondaryStat.SageElementalClone) != null && System.currentTimeMillis() - player.lastSungiAttackTime >= (long) (sungi.getV2() * 1000)) {
                ...
                } else if (attack.targets > 0 && player.getBuffedEffect(SecondaryStat.SageElementalClone) == null && System.currentTimeMillis() - player.lastSungiAttackTime >= (long) (sungi.getQ() * 1000)) {
                    int i = 0;
                    if (!sungi_skills.isEmpty()) {
                        player.lastSungiAttackTime = System.currentTimeMillis();
        ...
        if (GameConstants.isNightLord(player.getJob()) && !GameConstants.is_forceAtom_attack_skill(attack.skill) && attack.skill < 400001001 && Randomizer.isSuccess(SkillFactory.getSkill(4110012).
                getEffect(player.getSkillLevel(4110012)).
                getProp())) {
    ...
    public static final void applyAttackMagic(AttackInfo attack, Skill theSkill, MapleCharacter player, SecondaryStatEffect effect, double maxDamagePerHit) {
        ...
        for (AttackPair oned : attack.allDamage) {
            ...
            if (attack.skill == 2121011 || attack.skill == 2141003) { // 플레임 헤이즈, 플레임 헤이즈 VI 구현
                ...
                MapleMist mist = new MapleMist(eff.calculateBoundingBox(monster.getPosition(), player.isFacingLeft()), player, eff, 15000, (byte) 0);
                player.getMap().spawnMist(mist, false);
            }
            ...
            if (attack.skill == 2241000) { // 체인 라이트닝 VI 구현
                ...
                if (count < 4) {
                    if (!player.skillisCooling(skillId)) {
                        ...
                        MapleMonster mob = player.getMap().getAllMonster().get(Randomizer.rand(0, (player.getMap().getAllMonster().size() - 1)));

                        MapleMist mist = new MapleMist(eff.calculateBoundingBox(mob.getPosition(), player.isFacingLeft()), player, eff, 4000, (byte) 0);
            ...
            /*       if (player.isExtremeMode()) {
                int reducePercent = MapleExtremeBoss.getDamReduceByMap(player.getMapId());
                long originDamage = totDamageToOneMonster;
                long reduceDamage = (totDamageToOneMonster / reducePercent);
                totDamageToOneMonster -= reduceDamage;
                //  System.out.println(player.getName() + " | 기존 데미지 : " + player.StringToComma(originDamage) + " | 데미지 감소율("+reducePercent+"%, " +player.StringToComma(reduceDamage) + ") | 감소율 적용 데미지 : " + player.StringToComma(originDamage-reduceDamage));
            }*/
            monster.damage(player, totDamageToOneMonster, true, attack.skill);
            applyAddAttack(player, monster, attack);
            ...
            if (!GameConstants.사출기(attack.skill) || player.getMapId() == 921170004 || player.getMapId() == 921170011 || player.getMapId() == 921170012) {
                if (attack.targets > 0 && player.getKeyValue(99999, "tripling") > 0
                        && attack.skill != 1311020
                        && !GameConstants.isTryFling(attack.skill)
                        && attack.skill != 400031031
                        ...
                        && attack.skill != 400001018
                        && attack.skill != 500001001
                        || player.getMapId() == 921170004 || player.getMapId() == 921170011 || player.getMapId() == 921170012 || attack.targets > 0 && attack.skill != 1311020 && player.getBuffedEffect(SecondaryStat.TryflingWarm) != null && !GameConstants.isTryFling(attack.skill) && attack.skill != 400031031 && attack.skill != 400031001 && attack.skill != 13111020 && attack.skill != 13121054 && attack.skill != 13101022 && attack.skill != 13110022 && attack.skill != 13120003 && attack.skill != 13111020 && attack.skill != 400001018 && attack.targets > 0) {
                    int skillid = 0;
                    if (player.getSkillLevel(13141001) > 0) { // 트라이플링 윔 VI 구현
                        skillid = 13141001;
                    } else if (player.getKeyValue(99999, "tripling") > 0 || player.getSkillLevel(SkillFactory.getSkill(13120003)) > 0) {
                        skillid = 13120003;
                        if (player.getSkillLevel(SkillFactory.getSkill(13120003)) < 30) {
                            player.teachSkill(13120003, 30);
                        }
                    ...
                    if (skillid != 0) {
                        ...
                        if (objs.size() > 0) {
                            ...
                            if (Randomizer.isSuccess(trychance) || player.getMapId() == 921170004 || player.getMapId() == 921170011 || player.getMapId() == 921170012) {
                                MapleAtom atom = new MapleAtom(false, player.getId(), 7, true, skillid, player.getTruePosition().x, player.getTruePosition().y);
                                ArrayList<Integer> monsters = new ArrayList<Integer>();
            ...
            /*if (attack.targets > 0 && attack.skill != 1311020 && player.getSkillLevel(13100022) > 0 && !GameConstants.isTryFling(attack.skill) && attack.skill != 400031031 && attack.skill != 400031001 && attack.skill != 13111020 && attack.skill != 13121054 && attack.skill != 13101022 && attack.skill != 13110022 && attack.skill != 13120003 && attack.skill != 13111020 && attack.skill != 400001018) {
                int skillid = 0;
                if (player.getSkillLevel(SkillFactory.getSkill(13120003)) > 0) {
                    skillid = 13120003;
                } else if (player.getSkillLevel(SkillFactory.getSkill(13110022)) > 0) {
                    skillid = 13110022;
                } else if (player.getSkillLevel(SkillFactory.getSkill(13120010)) > 0) {
                    skillid = 13120010;
                }
                if (skillid != 0) {
                    Skill trskill = SkillFactory.getSkill(skillid);
...
//                        break;
        ...
        if (attack.skill == 2121011 || attack.skill == 2141003) { // 플레임 헤이즈, 플레임 헤이즈 VI 구현
            if (attack.allDamage.size() == 0) {
                ...
                MapleMist mist = new MapleMist(eff.calculateBoundingBox(player.getPosition(), player.isFacingLeft()), player, eff, 15000, (byte) 0);
                player.getMap().spawnMist(mist, false);
            }
        }
        ...
        if (attack.skill == 2341000) { // 엔젤레이 VI 구현
            if (attack.allDamage.size() > 0) {
                ...
                if (player.hexaMasterySkillStack >= 12) {
                    ...
                    player.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(2341001, mobList, true, 0, null));
                }
            }
        }
        ...
        if (totDamage > 0L) {
            ...
            if (attack.targets > 0 && player.getBuffedEffect(SecondaryStat.Triumph) != null && attack.skill != 2311017 && attack.skill != 2341005 && System.currentTimeMillis() - player.TriumphTime >= 2000) {
                List<MapleMapObject> objs = player.getMap().getMapObjectsInRange(player.getTruePosition(), 500000.0, Arrays.asList(new MapleMapObjectType[]{MapleMapObjectType.MONSTER}));
                if (objs.size() > 0) {
                    final List<SecondAtom> atoms = new ArrayList<SecondAtom>();
                    atoms.add(new SecondAtom(0x25, player.getId(), monster.getId(), player.getSkillLevel(2341004) > 0 ? 2341005 : 2311017, 4000, 0, 1, new Point((int) player.getTruePosition().getX(), (int) player.getTruePosition().getY() - 120), Arrays.asList(0)));
                    atoms.add(new SecondAtom(0x25, player.getId(), monster.getId(), player.getSkillLevel(2341004) > 0 ? 2341005 : 2311017, 4000, 0, 1, new Point((int) player.getTruePosition().getX() + 20, (int) player.getTruePosition().getY() - 116), Arrays.asList(0)));
                    atoms.add(new SecondAtom(0x25, player.getId(), monster.getId(), player.getSkillLevel(2341004) > 0 ? 2341005 : 2311017, 4000, 0, 1, new Point((int) player.getTruePosition().getX() - 20, (int) player.getTruePosition().getY() - 120), Arrays.asList(0)));
                    atoms.add(new SecondAtom(0x25, player.getId(), monster.getId(), player.getSkillLevel(2341004) > 0 ? 2341005 : 2311017, 4000, 0, 1, new Point((int) player.getTruePosition().getX() + 10, (int) player.getTruePosition().getY() - 116), Arrays.asList(0)));
                    atoms.add(new SecondAtom(0x25, player.getId(), monster.getId(), player.getSkillLevel(2341004) > 0 ? 2341005 : 2311017, 4000, 0, 1, new Point((int) player.getTruePosition().getX() - 10, (int) player.getTruePosition().getY() - 120), Arrays.asList(0)));
                    player.spawnSecondAtom(atoms);
    ...
    public static final AttackInfo parseDmgB(LittleEndianAccessor lea, MapleCharacter chr) {
        ...
        try {
            ...
            //|| ret.skill == 400031004
            if (GameConstants.sub_883680(ret.skill) || ret.skill == 5300007 || ret.skill == 27120211 || ret.skill == 400031003 || ret.skill == 64101008 || ret.skill == 400031067) {
                lea.skip(4);
            }
        ...
        try {
            ...
            switch (ret.skill) {
                ...
                case 101141015:
                    ...
                    break;
                default:
                    if (SkillFactory.getSkill(ret.skill) != null && SkillFactory.getSkill(ret.skill).isFinalAttack() && ret.skill != 154110005) {
                        lea.skip(1);
                    }
    ...
    public static void WFinalAttackRequest(MapleCharacter chr, int skillid, MapleMonster monster) {
        if (SkillFactory.getSkill(skillid) != null) {
            if (chr.getJob() == 512 && chr.skillisCooling(5121013)) {
                //하울링 피스트 노틸러스 소환 fix
                if (skillid == 5001002 || skillid == 5101004 || skillid == 5111009 || skillid == 5121007 || skillid == 5121020 || skillid == 5141000 || skillid == 5141001 || skillid == 400051071 || skillid == 400051070) { // 피스트 인레이지 VI 구현
                    ...
                    // 5120011
                    SkillFactory.getSkill(chr.getSkillLevel(5141009) > 0 ? 5140010 : 5120021).getEffect(chr.getSkillLevel(5141009) > 0 ? chr.getSkillLevel(5141009) : chr.getSkillLevel(5121013)).applyTo(chr);
                    chr.setSkillCustomInfo(chr.getSkillLevel(5141009) > 0 ? 5140010 : 5120021, 0L, 1000L);
                    chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, 0, chr.getSkillLevel(5141009) > 0 ? 5140010 : 5120021, 1, 0, 0, (byte) (monster.isFacingLeft() ? 1 : 0), false, monster.getTruePosition(), null, null), false);
                    chr.getClient().getSession().writeAndFlush(CField.rangeAttack(chr.getSkillLevel(5141009) > 0 ? 5140010 : 5120021, Arrays.asList(new RangeAttack(skillid, chr.getPosition(), (monster.isFacingLeft() ? 1 : 0), 0, 1))));
