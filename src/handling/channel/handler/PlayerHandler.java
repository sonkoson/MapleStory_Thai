...
public class PlayerHandler {
    ...
    public static void selectSolJanus(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (skillId == 500001001) { // HEXA 솔 야누스 구현
            for (MapleSummon summon : c.getPlayer().getSummons()) {
                if (summon.getSkill() == 500001002 || summon.getSkill() == 500001003 || summon.getSkill() == 500001004) {
    ...
    public static final void UseChair(MapleClient c, MapleCharacter chr, LittleEndianAccessor slea) {
        ...
        if (chr.getMapId() == ServerConstants.warpMap) {
            chr.getClient().getSession().writeAndFlush(SLFCGPacket.BlackLabel("#fn나눔고딕 ExtraBold##fs32##b#e잠수포인트 적립을 시작합니다.", 100, 1000, 4, 0, 0, 1, 4));
    ...
    public static final void CancelChair(short id, MapleClient c, MapleCharacter chr) {
        ...
        if (chr.getMapId() == ServerConstants.warpMap) {
            chr.getClient().getSession().writeAndFlush(SLFCGPacket.BlackLabel("#fn나눔고딕 ExtraBold##fs32##b#e잠수포인트 적립을 그만둡니다.", 100, 1000, 4, 0, 0, 1, 4));
        ...
        if (type > -2) {
            ...
            if (chr.getBuffedEffect(SecondaryStat.RoyalGuardPrepare) != null) {
                ...
                if (chr.getRoyalStack() >= 3 && chr.getSkillLevel(51110009) > 0) {
                    ...
                    SkillFactory.getSkill(isHexa ? 51141002 : 51001005).getEffect(chr.getSkillLevel(isHexa ? 51141002 : 51001005)).applyTo(chr);
                    ...
                    switch (attacker.getId()) {
                        ...
                        case 8880633: {
                            ...
                            if (extremeSeren != null && extremeSerenDawn == null) {
                                extremeSeren.gainShield(extremeSeren.getStats().getHp() / 100L, extremeSeren.getShield() <= 0L, 0);
                            }
                            if (extremeSerenDawn != null) {
                                extremeSerenDawn.getMap().broadcastMessage(MobPacket.BossSeren.SerenChangePhase("Mob/8880603.img/info/shield", 2, extremeSerenDawn));
                            }
                            break;
                        }
                        case 8880634: {
                ...
                if (chr.getBuffedValue(SecondaryStat.MagicGaurd) != null || chr.getSkillLevel(27000003) > 0 || chr.getSkillLevel(12000024) > 0) {
                    ...
                    if ((long) damage < chr.getStat().getCurrentMaxHp()) {
                        if (chr.getBarrier() < damage) {
                            ...
                            chr.setBarrier(chr.getBarrier() - damage);
                            chr.getBuffedEffect(SecondaryStat.DamAbsorbShield).applyTo(chr, false, (int) chr.getBuffLimit(chr.getBuffSource(SecondaryStat.DamAbsorbShield)));
                        }
                        chr.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(chr, 0, 3210013, 10, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), true, chr.getPosition(), null, null));
                        chr.getMap().broadcastMessage(chr, CField.EffectPacket.showEffect(chr, 0, 3210013, 10, 0, 0, (byte) (chr.isFacingLeft() ? 1 : 0), false, chr.getPosition(), null, null), false);
    ...
    public static final void CancelBuffHandler(LittleEndianAccessor slea, MapleCharacter chr) {
        ...
        if (sourceid == 1241502) {
            if (chr.getPoisonStack() > 5) {
                ...
                HashMap<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<SecondaryStat, Pair<Integer, Integer>>();
                statups.put(SecondaryStat.DotBasedBuff, new Pair<Integer, Integer>(Integer.valueOf(chr.getPoisonStack()), 0));
                drain.setDuration(0);
    ...
    public static final void SpecialMove(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (skillid == 5311013 || skillid == 5341002) { // 미니 캐논볼 VI 구현
            ...
            chr.getClient().getSession().writeAndFlush(CField.rangeAttack(skillid, Arrays.asList(new RangeAttack(skillid + 1, chr.getTruePosition(), 1, 330, 1))));
        }
        ...
        if (linkEffect.getCooldown(chr) > 0 && effect.getSourceId() != 35111002 && effect.getSourceId() != 11141002 && effect.getSourceId() != 151100002 && effect.getSourceId() != 20031205 && !effect.ignoreCooldown(chr) && (linkEffect.getSourceId() < 400041003 || linkEffect.getSourceId() > 400041005) && linkcool) {
        ...
        Label_14276:
        {
            switch (skillid) {
                ...
                case 23111008: {  //엘리멘탈 나이트
                    ...
                    SecondaryStatEffect eff = SkillFactory.getSkill(elementSummons).getEffect(c.getPlayer().getSkillLevel(elementSummons));
                    MapleSummon tosummon = new MapleSummon(chr, elementSummons, chr.getTruePosition(), SummonMovementType.BIRD_FOLLOW, (byte) 0, eff.getDuration());
                    chr.addSummon(tosummon);
                        ...
                        for (MaplePartyCharacter pc : chr.getParty().getMembers()) {
                            if (!pc.getPlayer().getBuffedValue(5121010)) {
                                for (MapleCoolDownValueHolder i : pc.getPlayer().getCooldowns()) {
                                    if (!Arrays.asList(bufflist).contains(i.skillId) && !SkillFactory.getSkill(i.skillId).isHyper() && i.skillId / 10000 <= pc.getPlayer().getJob()) { //오류
                ...
                case 3341004: // 카디널 디스차지 VI
                {
                    ...
                    MapleAtom atom = new MapleAtom(false, chr.getId(), success ? 87 : 57, true, success ? 3341005 : 3310004, chr.getTruePosition().x, chr.getTruePosition().y);
                    ...
                    if (chr.문양 == 2 && chr.getSkillLevel(3310004) > 0 && !objectIds.isEmpty()) {
                        ...
                        if (check) {
                            for (byte i = 0; i < bullet; i++) {
                                ...
                                atom.addForceAtom(new ForceAtom(2, 0x2A, 4, 0, 0x3C, mob != null ? mob.getTruePosition() : chr.getTruePosition()));
                            }
                            atom.setDwTargets(monsters);
                            chr.getMap().spawnMapleAtom(atom);
                        }
                ...
                case 31241000: { // 실드 체이싱 VI 구현
                    pos = slea.readPos();
                    chr.setPosition(pos);
                    final MapleAtom atom4 = new MapleAtom(false, chr.getId(), 84, true, 31241001, chr.getTruePosition().x, chr.getTruePosition().y);
                    final List<Integer> monsters4 = new ArrayList<Integer>();
                ...
                case 33121255: {
                    pos = slea.readPos();
                    chr.setPosition(pos);
                    if (skillid == 33001016 || skillid == 33111015 || skillid == 33141010 || skillid == 33101115 || skillid == 33141008) {
                        if (skillid == 33101115 || skillid == 33141008) {
                            c.getSession().writeAndFlush((Object) CField.skillCooldown(skillid == 33141008 ? 33141009 : 33101215, 7000));
                        ...
                        if (!objs.isEmpty()) {
                            ...
                            for (final MapleMapObject mobs2 : skill2) {
                                ...
                                datas.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_JaguarProvoke, new MonsterStatusEffect(skillid, mon.getStats().isBoss() ? (effect.getDuration() / 2) : effect.getDuration(), 1L)));
                                datas.add(new Pair<MonsterStatus, MonsterStatusEffect>(MonsterStatus.MS_DodgeBodyAttack, new MonsterStatusEffect(skillid, mon.getStats().isBoss() ? (effect.getDuration() / 2) : effect.getDuration(), 1L)));
                ...
                case 35120017: {
                    ...
                    final MapleAtom atom5 = new MapleAtom(false, chr.getId(), isHexa ? 97 : 20, true, skillid, chr.getTruePosition().x, chr.getTruePosition().y);
                ...
                case 65111100: { // 오리진 그랜드 피날레 구현
                    ...
                    MapleAtom atom = new MapleAtom(false, chr.getId(), isHexa ? 93 : 3, true, isHexa ? 65141502 : 65111007, chr.getTruePosition().x, chr.getTruePosition().y);
                ...
                case 65141001: { // 소울 시커 VI 구현
                    ...
                    chr.setPosition(pos);
                    byte count = slea.readByte();
                    MapleAtom atom = new MapleAtom(false, chr.getId(), isHexa ? 93 : 3, true, 65141003, chr.getTruePosition().x, chr.getTruePosition().y);
                    ArrayList<Integer> monsters = new ArrayList<Integer>();
                    int mobid = 0;
                    if (count == 1) {
                        mobid = slea.readInt();
                ...
                case 400021001: { // 도트 퍼니셔
                    ...
                    for (int i = 0; i < 15 + addball; i++) {
                        monsters.add(0);
                        ForceAtom atoms = new ForceAtom(moid, Randomizer.rand(41, 44), Randomizer.rand(3, 4), Randomizer.rand(10, 360), (short) 720, new Point(Randomizer.rand(xMin, xMax), Randomizer.rand(yMin, yMax)));
                    ...
                    for (MapleMonster monster : chr.getMap().getAllMonster()) {
                        if (monster != null) {
                            if (monster.getBuff(MonsterStatus.MS_Burned) != null) {
                                ...
                                realduration2 = ((monster.getBuff(MonsterStatus.MS_Burned).getStartTime() + monster.getBuff(MonsterStatus.MS_Burned).getDuration()) - System.currentTimeMillis());
                                ...
                                chr.getMap().broadcastMessage(MobPacket.NujukDamage(monster, chr, totaldamage, 400041000, 6));
                                chr.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(400041030, new ArrayList<>(), false, 0, new int[0]));
                                if (monster.getBuff(MonsterStatus.MS_Burned) != null) {
                ...
                case 400041058: {
                    ...
                    for (int i = 0; i < 30; i++) {
                        AdelProjectile sa = new AdelProjectile(9, chr.getId(), chr.getObjectId(), 400041058, 3600, 0, 1, chr.getPosition(), new ArrayList<>());
                    ...
                    if (skillid == 131001025) {
                        ...
                        if (effect.isMagicDoor()) {
                            ...
                            if (skillid == 13141501) { // 오리진 미스트랄 스프링 구현
                                ...
                                for (int i = 0; i < 13; i++) {
                                    atoms.add(new SecondAtom(47, chr.getId(), 0, 13141502, 3000, 0, 1, new Point(pos.x + Randomizer.rand(-400, 400), pos.y + Randomizer.rand(-550, -50)), Arrays.asList(0)));
                                }

                                for (int i = 0; i < 5; i++) {
                                    atoms.add(new SecondAtom(48, chr.getId(), 0, 13141503, 3000, 0, 1, new Point(pos.x + Randomizer.rand(-400, 400), pos.y + Randomizer.rand(-550, -50)), Arrays.asList(0)));
                                }

                                for (int i = 0; i < 3; i++) {
                                    atoms.add(new SecondAtom(49, chr.getId(), 0, 13141504, 3000, 0, 1, new Point(pos.x + Randomizer.rand(-400, 400), pos.y + Randomizer.rand(-550, -50)), Arrays.asList(0)));
                                }
                            ...
                            if (skillid == 25141003) { // 오리진 호신강림 구현
                                ...
                                SecondaryStatValueHolder mbsvh = chr.checkBuffStatValueHolder(SecondaryStat.IndieSummon, 25141506);
                                mbsvh.value = 0;

                                SecondaryStatEffect eff = chr.getBuffedEffect(25141506);

                                int resetTime = (int) (30000 - (System.currentTimeMillis() % 1000000000L - eff.getStarttime()));
    ...
    public static final void closeRangeAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr, boolean energy) {
        ...
        if (attack.skill != 0) {
            ...
            if (GameConstants.isPathFinder(chr.getJob())) {
                ...
                if (chr.getBuffedValue(25101009) && SkillFactory.getSkill(25100009).getSkillList().contains(Integer.valueOf(attack.skill)) && attack.targets > 0) {
                    ...
                    int suc = isHexa ? 40 : (chr.getSkillLevel(25120154) > 0) ? 35 : ((chr.getSkillLevel(25120115) > 0) ? 25 : 15);
                    if (attack.skill == 400051079) {
                        MapleAtom atom = new MapleAtom(false, chr.getId(), isHexa ? 68 : 13, true, isHexa ? 25141505 : 25120115, (chr.getTruePosition()).x, (chr.getTruePosition()).y);
                        ...
                        MapleAtom atom = new MapleAtom(false, chr.getId(), isHexa ? 68 : 13, true, isHexa ? 25141505 : (skillid_ == 25100009) ? 25100010 : 25120115, (chr.getTruePosition()).x, (chr.getTruePosition()).y);
                ...
                if (!chr.skillisCooling(400011032) && (attack.skill == 400011032 || attack.skill == 400011033 || attack.skill == 400011034 || attack.skill == 400011035 || attack.skill == 400011036 || attack.skill == 400011037 || attack.skill == 400011067)) {
                    ...
                    if (chr.getBuffedValue(SecondaryStat.RoyalGuardState) != null) {
                        ...
                        SkillFactory.getSkill(isHexa ? 51141002 : 51001005).getEffect(chr.getSkillLevel(isHexa ? 51141002 : 51001005)).applyTo(chr, false, (int) duration);
                    }
                }
            } else if (GameConstants.isZero(chr.getJob())) {
            ...
            if (GameConstants.isKhali(chr.getJob())) {
                ...
                if (MapleSkillManager.isKhaliAttackSkills(attack.skill) && !c.getPlayer().getBuffedValue(400041087)) {
                    ...
                    skills.add(new RangeAttack(id, chr.getTruePosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 8));
                ...
                //디시빙 블레이드
                if ((MapleSkillManager.isKhaliAttackSkills(attack.skill) || MapleSkillManager.isKhaliHexSkills(attack.skill)) && attack.targets > 0 && !c.getPlayer().getBuffedValue(154110005)) {
                    SecondaryStatEffect blade = SkillFactory.getSkill(154110005).getEffect(chr.getSkillLevel(154110005));
            ...
            if (GameConstants.isKhali(chr.getJob())) {
                ...
                if ((chr.getBuffedValue(154111000) && attack.targets > 0) && MapleSkillManager.isKhaliVoydSkills(attack.skill)) { // 서먼 차크리
        ...
        if (GameConstants.isBattleMage(chr.getJob())) {
            ...
            if (attack.skill == 37000011 || attack.skill == 37000012 || attack.skill == 37000013 || attack.skill == 37001002 || attack.skill == 37141002) { // 릴리즈 파일 벙커 VI 구현
                skills.clear();
                ...
                if (chr.getSkillLevel(37141002) > 0) { // 릴리즈 파일 벙커 VI 구현
                    skills.add(new RangeAttack(37141003, chr.getTruePosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    skills.add(new RangeAttack(37141004, chr.getTruePosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    skills.add(new RangeAttack(37141005, chr.getTruePosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                    skills.add(new RangeAttack(37141006, chr.getTruePosition(), ((attack.facingleft >>> 4 & 0xF) == 0) ? 0 : 1, 0, 1));
                }
                chr.getClient().send(CField.rangeAttack(attack.skill, skills));
            }
    ...
    public static final void BuffAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (attack.skill != 0) {
            ...
            if (effect.getCooldown(chr) > 0 && !effect.ignoreCooldown(chr) && !GameConstants.isExceptionSkill(chr.getJob(), attack.skill) && attack.skill != 162111006) {
    ...
    public static final void rangedAttack(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) { // 원거리 공격
        ...
        if (attack.skill != 0) {
            skill = SkillFactory.getSkill(GameConstants.getLinkedSkill(attack.skill));
            if (skill == null || (GameConstants.isAngel(attack.skill) && (chr.getStat()).equippedSummon % 10000 != attack.skill % 10000) && (attack.skill != 3141004)) {
                c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
            ...
            if (effect.getCooldown(chr) > 0 && !effect.ignoreCooldown(chr) && !GameConstants.isExceptionSkill(chr.getJob(), attack.skill) && linkcool) {
    ...
    public static final void MagicDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr, boolean chilling, boolean orbital) {
        ...
        if (GameConstants.isKinesis(chr.getJob())) {
            if (attack.skill != 142120003 && attack.skill != 142111002 && attack.skill != 142110003 && attack.skill != 142001002 && attack.skill != 142141000 && attack.skill != 142111007 && attack.skill != 142120000 && attack.skill != 142120001 && attack.skill != 142120002 && attack.skill != 400021048 && attack.skill != 400021008) { // 얼티메이트-메테리얼 VI 구현
                chr.givePPoint(attack.skill);
        ...
        if (effect.getCooldown(chr) > 0 && !effect.ignoreCooldown(chr) && !GameConstants.isExceptionSkill(chr.getJob(), attack.skill) && attack.skill != 142101009 && attack.skill != 12120011 && attack.skill != 2220014 && attack.skill != 2120013 && attack.skill != 400021004) {
    ...
    public static final void ChangeMap(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        // 6차 구현
        if (chr.getLevel() >= 260) {
            for (MapleSummon summon : chr.getSummons()) { // 맵 이동 시 6차 소환수 삭제
                int originSkillId = GameConstants.getHexaOriginFirst(chr.getJob());

                if ((((summon.getSkill() / originSkillId) == 1) && ((summon.getSkill() % originSkillId) < 10)) || summon.getSkill() == 500001002 || summon.getSkill() == 500001003 || summon.getSkill() == 500001004) {
...
// 보스 포탈입장 데쿠
    ...
    public static final void InnerPortal(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if (portal.getName().equals("east01") || portal.getName().equals("west00") || portal.getName().equals("pt00")) {
        ...
        if (c.getPlayer().getMapId() == 211042300 && portal.getName().equals("ps00")) { // 자쿰
            bossNumber = 1;
        } else if (c.getPlayer().getMapId() == 262030000 && portal.getName().equals("in00")) { // 힐라
            bossNumber = 3;
        } else if (c.getPlayer().getMapId() == 105200000 && portal.getName().equals("boss00")) { // 피에르
            bossNumber = 4;
        } else if (c.getPlayer().getMapId() == 105200000 && portal.getName().equals("boss01")) { // 반반
            bossNumber = 5;
        } else if (c.getPlayer().getMapId() == 105200000 && portal.getName().equals("boss02")) { // 블러디퀸
            bossNumber = 6;
        } else if (c.getPlayer().getMapId() == 105200000 && portal.getName().equals("boss03")) { // 벨룸
            bossNumber = 7;
        } else if (c.getPlayer().getMapId() == 211070000 && portal.getName().equals("lionCastleenter")) { // 반 레온
            bossNumber = 8;
        } else if (c.getPlayer().getMapId() == 272020110 && portal.getName().equals("AkayrumEnter")) { // 아카이럼
            bossNumber = 9;
        } else if (c.getPlayer().getMapId() == 401060000 && portal.getName().equals("enter_magnusDoor")) { // 매그너스
            bossNumber = 10;
        } else if (c.getPlayer().getMapId() == 271040000 && portal.getName().equals("in00")) { // 시그너스
            bossNumber = 12;
        } else if (c.getPlayer().getMapId() == 350060300 && portal.getName().equals("bossIn00")) { // 스우
            bossNumber = 13;
        } else if (c.getPlayer().getMapId() == 105300303 && portal.getName().equals("pt_enter")) { // 데미안
            bossNumber = 15;
        } else if (c.getPlayer().getMapId() == 220080000 && portal.getName().equals("in00")) { // 파풀라투스
            bossNumber = 22;
        } else if (c.getPlayer().getMapId() == 450011990 && portal.getName().equals("inMaze")) { // 진 힐라
            bossNumber = 24;
        } else if (c.getPlayer().getMapId() == 450012500 && portal.getName().equals("in_boss")) { // 검은 마법사
            bossNumber = 25;
        } else if (c.getPlayer().getMapId() == 450009301 && portal.getName().equals("in00")) { // 더스크
            bossNumber = 26;
        } else if (c.getPlayer().getMapId() == 450012200 && portal.getName().equals("in00")) { // 듄켈
            bossNumber = 27;
        }
    ...
    public static final void ChangeInner(LittleEndianAccessor slea, MapleClient c) {
        ...
        int a = (((InnerSkillValueHolder) c.getPlayer().getInnerSkills().get(preset).get(0)).getRank() == 0) ? 100 : ((((InnerSkillValueHolder) c.getPlayer().getInnerSkills().get(preset).get(0)).getRank() == 1) ? 200 : ((((InnerSkillValueHolder) c.getPlayer().getInnerSkills().get(preset).get(0)).getRank() == 2) ? 1500 : 8000));
        int plus = (((InnerSkillValueHolder) c.getPlayer().getInnerSkills().get(preset).get(0)).getRank() == 2) ? 1500 : 3000;
        int plus2 = (((InnerSkillValueHolder) c.getPlayer().getInnerSkills().get(preset).get(0)).getRank() == 2) ? 4000 : 8000;
        ...
        for (InnerSkillValueHolder isvh : c.getPlayer().getInnerSkills().get(preset)) {
            ...
            if (check_rock) {
                ...
                while (!breakout) {
                    if (count != 0) {
                        if (count == 1) {
                            //  c.getPlayer().dropMessage(5, "" + c.getPlayer().getInnerSkills().get(preset).size() + " line : " + line);
                            if (ivholder.getSkillId() == c.getPlayer().getInnerSkills().get(preset).get(line - 1).getSkillId()) {
                                ivholder = InnerAbillity.getInstance().renewSkill(nowrank, false, preset);
                                continue;
                            }
                            breakout = true;
                        ...
                        if (ivholder.getSkillId() == c.getPlayer().getInnerSkills().get(preset).get(line - 1).getSkillId() || ivholder.getSkillId() == c.getPlayer().getInnerSkills().get(preset).get(line2 - 1).getSkillId()) {
                            ivholder = InnerAbillity.getInstance().renewSkill(nowrank, false, preset);
                            continue;
                        }
                        breakout = true;
                ...
                ivholder2 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), false, preset);
                boolean breakout = false;
                while (!breakout) {
                    breakout = true;
                    if (count != 0) {
                        if (count == 1) {
                            if (ivholder2.getSkillId() == c.getPlayer().getInnerSkills().get(preset).get(line - 1).getSkillId()) {
                                ivholder2 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), false, preset);
                                breakout = false;
                            }
                        } else if (ivholder2.getSkillId() == c.getPlayer().getInnerSkills().get(preset).get(line - 1).getSkillId() || ivholder2.getSkillId() == c.getPlayer().getInnerSkills().get(preset).get(line - 1).getSkillId()) {
                            ivholder2 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), false, preset);
                            breakout = false;
                        }
                    }
                ...
                InnerSkillValueHolder ivholder3 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), false, preset);
                while (ivholder.getSkillId() == ivholder3.getSkillId() || ivholder2.getSkillId() == ivholder3.getSkillId()) {
                    ivholder3 = InnerAbillity.getInstance().renewSkill((ivholder.getRank() == 0) ? 0 : (ivholder.getRank() - 1), false, preset);
                }
        ...
        c.getPlayer().getClient().getSession().writeAndFlush(CField.updateInnerAbility(c.getPlayer().getInnerSkills().get(preset), preset));
        ...
        for (int i = 0; i < wsize; ++i) {
            ...
            switch (skillId) {
                case 0: {
                    ...
                    if (c.getPlayer().getStat().getMp() >= c.getPlayer().getStat().getCurrentMaxMp(c.getPlayer())) {
                        ...
                        if (effNightmare != null) {
                            ...
                            int resetTime = (int) (20000 - (System.currentTimeMillis() % 1000000000L - effNightmare.getStarttime()));

                            EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class
                            );
                            statups.put(SecondaryStat.Origin_Nightmare, new Pair<Integer, Integer>(c.getPlayer().hexaMasterySkillStack, resetTime));
                            SecondaryStatEffect eff = SkillFactory.getSkill(31141501).getEffect(c.getPlayer().getSkillLevel(31141501));
                            c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, eff, c.getPlayer()));
                            break;
                        }
                    }
                ...
                case 400001036: // 에르다 샤워
                    int cooldown = SkillFactory.getSkill(400001036).getEffect(c.getPlayer().getSkillLevel(400001036)).getCooldown(c.getPlayer());
            ...
            switch (skillId) {
                ...
                case 25141505: { // 오리진 호신강림 구현
                    ...
                    atom.setDwUserOwner(c.getPlayer().getId());
                    forceAtom = new ForceAtom(4, Randomizer.rand(41, 42), Randomizer.rand(4, 4), Randomizer.rand(5, 300), 0);
                    forceAtom.setnAttackCount(attackCount + 1);
                    atom.setDwFirstTargetId(nextObjectId);
                    atom.addForceAtom(forceAtom);
                    c.getPlayer().getMap().spawnMapleAtom(atom);
                ...
                case 14000029: {
                    ...
                    for (int skill : skills) {
                        if (c.getPlayer().getSkillLevel(skill) > 0) {
                            attackLimit += SkillFactory.getSkill(skill).getEffect(c.getPlayer().getSkillLevel(skill)).getY();
                        }
                    }
    ...
    public static void absorbingSword(LittleEndianAccessor slea, MapleCharacter chr) {//mush will
        ...
        MapleAtom atom = new MapleAtom(false, chr.getId(), skill == 61141006 ? 101 : skill == 400011058 || skill == 400011059 ? 32 : 2, true, skill, chr.getTruePosition().x, chr.getTruePosition().y);
        ArrayList<Integer> monsters = new ArrayList<Integer>();
        for (int i = 0; i < mobSize; ++i) {
            monsters.add(slea.readInt());
            atom.addForceAtom(new ForceAtom(chr.getBuffedValue(61141006) ? 1 : chr.getBuffedValue(61121217) ? 4 : 2, chr.getBuffedValue(61141006) ? 17 : 18, Randomizer.rand(20, 40), 0, (short) Randomizer.rand(1000, 1500)));
        }
        while (atom.getForceAtoms().size() < (chr.getBuffedValue(61141006) ? 1 : chr.getBuffedValue(61120007) || chr.getBuffedValue(61121217) ? 5 : 3)) {
            atom.addForceAtom(new ForceAtom(chr.getBuffedValue(61141006) ? 1 : chr.getBuffedValue(61121217) ? 4 : 2, chr.getBuffedValue(61141006) ? 17 : 18, Randomizer.rand(20, 40), 0, (short) Randomizer.rand(1000, 1500)));
        }
        if (skill != 0) {
            chr.cancelEffectFromBuffStat(SecondaryStat.StopForceAtominfo);
    ...
    public static void MatrixSkill(LittleEndianAccessor slea, MapleClient c) {
        ...
        slea.skip(4);
        if (skillid == 3321004
                || skillid == 3321038 || skillid == 400051016
                || skillid == 400021028 || skillid == 400051003 || skillid == 27121201 || skillid == 3001004
                || skillid == 63111003 || skillid == 63111103 || skillid == 64111004 || skillid == 3301008 || skillid == 400041018
                || skillid == 37111006 || skillid == 152120003 || skillid == 400011081 || skillid == 400011082 || skillid == 400021070
                || skillid == 152121004 || skillid == 400011004
                || skillid == 400041043 || skillid == 400041020 || skillid == 63101004 || skillid == 63101100
                || skillid == 12120022 || skillid == 12120023 || skillid == 400031026 || skillid == 400031058) { // 에인션트 아스트라
            slea.skip(3);
        ...
        switch (skillid) {
            ...
            /*          case 400031056: {
                if (chr.getBuffedEffect(SecondaryStat.RepeatingCrossbowCatridge) != null) {
                    System.err.println("chr.repeatingCrossbowCatridge " + chr.repeatingCrossbowCatridge);
                    if (chr.repeatingCrossbowCatridge > 1) {
                        chr.repeatingCrossbowCatridge--;
                        Map<SecondaryStat, Pair<Integer, Integer>> map = new HashMap<>();
                        map.put(SecondaryStat.RepeatingCrossbowCatridge, new Pair<>(Integer.valueOf(chr.repeatingCrossbowCatridge), Integer.valueOf((int) chr.getBuffLimit(chr.getBuffSource(SecondaryStat.RepeatingCrossbowCatridge)))));
    ...
    public static void UpdateSymbol(LittleEndianAccessor slea, MapleClient c, int plus) {
        try {
            ...
            long l = needmeso = ArcneSymbol ? GameConstants.needArcSymbolMeso(item.getItemId(), item.getArcLevel()) : GameConstants.needAutSymbolMeso(item.getItemId(), item.getArcLevel());
    ...
    public static void PlayerWeaponMotionChange(LittleEndianAccessor slea, MapleClient c) {
        ...
        character.updateInfoQuest(27042, "use=" + character.getV("Weapon_motion") + ";use=" + character.getV("Weapon_motion"));
    }
    ...
    public static void keydownSkillManagement(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (skillid >= 3321034 && skillid <= 3321040) {
            ...
            chr.getClient().getSession().writeAndFlush(CField.rangeAttack(skillid, Arrays.asList(new RangeAttack(3321039, chr.getTruePosition(), 1, 330, 1))));
    ...
    public static void eventUIResult(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (type) {
            ...
            case 41: {
                ...
                // 지금 사용하는거
                if (idx == 1100) { // 윗줄 라인
                    NPCScriptManager.getInstance().start(c, 9062875, "9062875"); // 이벤트
                } else if (idx == 1101) {
                    NPCScriptManager.getInstance().start(c, 9062277, "9062277"); // 상점
                } else if (idx == 1102) {
                    NPCScriptManager.getInstance().start(c, 9063145, "9063145"); // 이동
                }
                if (idx == 1103) { // 윗줄 라인
                    NPCScriptManager.getInstance().start(c, 1530042, "1530042"); // 코디
                } else if (idx == 1104) {
                    NPCScriptManager.getInstance().start(c, 9062871, "1530050"); // 편의
                } else if (idx == 1105) {
                    NPCScriptManager.getInstance().start(c, 9062872, "9062872"); // 컨텐츠
                }
                ...
                public void run() {
                    if (c.getPlayer().getBuffedValue(respawnSkillId)) {
                        for (SecondAtom sa : removeSecondAtomList) {
                            ...
                            sa.setPoint(new Point(point.x + Randomizer.rand(-400, 400), point.y + Randomizer.rand(-550, -50)));
                        }
    ...
    public static void Vmatrixstackbuff(MapleClient c, boolean use, LittleEndianAccessor slea, int count1) {
        ...
        if (GameConstants.isPinkBean(c.getPlayer().getJob())) {
            ...
            SecondaryStatEffect flashMirazu = SkillFactory.getSkill(3111015).getEffect(c.getPlayer().getSkillLevel(3111015));
            long count = 0L;
            ...
            if (c.getPlayer().getJob() == 532) {
                ...
                if (c.getPlayer().getSkillCustomValue(400051008) != 3 && c.getPlayer().getSkillCustomValue(isHexa ? 5341002 : 5311013) >= 3) {
                    if (use && counts > 0L) {
                ...
                statups.put(SecondaryStat.VMatrixStackBuff, new Pair<Integer, Integer>((int) c.getPlayer().getBHGCCount(), 0));
                c.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, yoyo, c.getPlayer()));
            }
            //하울링 게일 스택체크
            ...
            c.getSession().writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(statups, InfinityFlameCircle, c.getPlayer()));
    ...
    public static void LiftBreak(MapleCharacter player, LittleEndianAccessor slea) {
        ...
        if (skillid == 400051078 || skillid == 63121008 || skillid == 64121013) {
            ...
            localstatups.clear();
            localstatups.put(SecondaryStat.IndieNotDamaged, new Pair<>(Integer.valueOf(player.getId()), Integer.valueOf((int) player.getBuffLimit(400051070))));
            player.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(localstatups, effect, player));
            ...
            if (skillid == 154121011) { //[othello] 보이드 블리츠 다크사이트
                Map<SecondaryStat, Pair<Integer, Integer>> localstatups = new HashMap<>();
                SecondaryStatEffect effect = SkillFactory.getSkill(154121011).getEffect(player.getSkillLevel(154121011));
                localstatups.clear();
                localstatups.put(SecondaryStat.DarkSight, new Pair<>(Integer.valueOf(player.getId()), 3000));
                player.getClient().getSession().writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(localstatups, effect, player));
    ...
    public static void EnterTrueRoom(MapleClient c) {
        ...
        int warptimed = (count == 1) ? 60 : ((count == 2) ? 180 : ((count == 3) ? 1800 : ((count == 4) ? 3600 : ((count == 5) ? 10800 : ((count == 6) ? 21600 : ((count == 7) ? 43200 : 86400))))));
        c.setKeyValue("LIE_DETECTOR_WARP", warptimed + "");
        c.getPlayer().warp(993073000);
        c.getPlayer().getWorldGMMsg(c.getPlayer(), "진실의 방에 입장. 계정번호 : " + c.getAccID());
        FileoutputUtil.log(FileoutputUtil.진실의방로그, "[입장] 계정 번호 : " + c.getAccID() + " | 캐릭 번호 : " + c.getPlayer().getId() + " | 캐릭터 닉네임 : " + c.getPlayer().getName() + "이 진실의방에 입장");
            ...
            case 4:
                ...
                if (give) {
                    ...
                    if (((MapleCabinet) list1.get(get - 1)).getPlayerid() > 0) {
                        /*if (((MapleCabinet) list1.get(get - 1)).getPlayerid() != c.getPlayer().getId()) { // 임시 처리
    ...
    public static void ItemSortLock(MapleClient c, LittleEndianAccessor slea) {
        ...
        switch (InvType) {
            case 1: // Equip
                c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.InventoryPacket.ItemSortLockEquip(type, (short) slot, item.getItemId(), item.getQuantity(), lock));
                break;
            case 2: // Consume
                c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.InventoryPacket.ItemSortLockConsume(type, (short) slot, item.getItemId(), item.getQuantity(), lock));
                break;
            case 3: // Install
                c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.InventoryPacket.ItemSortLockInstall(type, (short) slot, item.getItemId(), item.getQuantity(), lock));
                break;
            case 4: // Etc
                c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.InventoryPacket.ItemSortLockEtc(type, (short) slot, item.getItemId(), item.getQuantity(), lock));
                break;
            case 5: // Cash
                c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.InventoryPacket.ItemSortLockCash(type, (short) slot, item.getItemId(), item.getQuantity(), lock));
                break;
            case 6: // Cody
                c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.InventoryPacket.ItemSortLockCody(type, (short) slot, item.getItemId(), item.getQuantity(), lock));
                break;
        }
        c.getPlayer().getClient().getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
    }
    ...
    public static void FlashMirage(LittleEndianAccessor slea, MapleClient c) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            c.send(CField.EffectPacket.showEffect(c.getPlayer(), 0, 3111015, 84, rotate, finally_x, direction, true, new Point(x, y), null, null));
            c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.EffectPacket.showEffect(c.getPlayer(), 0, 3111015, 84, rotate, finally_x, direction, false, new Point(x, y), null, null), false);
    ...
    public static void CodyPresetApply(LittleEndianAccessor slea, MapleClient c) {
        ...
        c.getSession().writeAndFlush(CWvsContext.enableActions(c.getPlayer()));
        c.getPlayer().dropMessage(1, "프리셋이 변경되었습니다.");

        chr.setKeyValue(913, "preset", preset + "");
        chr.updateInfoQuest(27043, "preset0=" + chr.getKeyValue(913, "preset") + ";preset1=" + chr.getKeyValue(913, "preset") + ";preset2=" + chr.getKeyValue(913, "preset") + ";preset_new=" + chr.getKeyValue(913, "preset"));
    ...
    public static void dotgerecv(LittleEndianAccessor slea, MapleClient c) {
        ...
        slea.skip(4);
        SecondaryStatEffect effect;
        if (c.getPlayer().getSkillLevel(skillid) > 0 && (effect = SkillFactory.getSkill(skillid).getEffect(c.getPlayer().getSkillLevel(skillid))).makeChanceResult()) {
    ...
    public static void CurseEnchant(LittleEndianAccessor slea, MapleClient c) {
        int skillid = slea.readInt();
        Map<SecondaryStat, Pair<Integer, Integer>> localstatups = new HashMap<>();
        SecondaryStatEffect effect = SkillFactory.getSkill(skillid).getEffect(c.getPlayer().getSkillLevel(c.getPlayer().getSkillLevel(skillid)));
        localstatups.clear();
        ...
        if (c.getPlayer().getBuffedValue(skillid)) {
            ...
            localstatups.put(SecondaryStat.CurseEnchant, new Pair<>(Integer.valueOf(c.getPlayer().getId()), Integer.valueOf((int) c.getPlayer().getBuffLimit(skillid))));
            c.send(CWvsContext.BuffPacket.giveBuff(localstatups, effect, c.getPlayer()));
            SkillFactory.getSkill(skillid).getEffect(c.getPlayer().getSkillLevel(skillid)).applyTo(c.getPlayer(), c.getPlayer(), false, c.getPlayer().getPosition(), (int) c.getPlayer().getBuffLimit(skillid), (byte) 0, false);
        }
    }
    ...
    public static void CurseEnchantSelect(LittleEndianAccessor slea, MapleClient c) {
        int type = slea.readInt();
        Map<SecondaryStat, Pair<Integer, Integer>> localstatups = new HashMap<>();
        SecondaryStatEffect effect = SkillFactory.getSkill(3321042).getEffect(c.getPlayer().getSkillLevel(c.getPlayer().getSkillLevel(3321042)));
        localstatups.clear();
        localstatups.put(SecondaryStat.CurseEnchant, new Pair<>(type, Integer.valueOf((int) c.getPlayer().getBuffLimit(3321042))));
        c.send(CWvsContext.BuffPacket.giveBuff(localstatups, effect, c.getPlayer()));
        SkillFactory.getSkill(3321042).getEffect(c.getPlayer().getSkillLevel(3321042)).applyTo(c.getPlayer(), c.getPlayer(), false, c.getPlayer().getPosition(), (int) c.getPlayer().getBuffLimit(3321042), (byte) 0, false);
    ...
    public static void bless5th(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (skill == 400001050) {
            ...
            Map<SecondaryStat, Pair<Integer, Integer>> localstatups = new HashMap<>();
            localstatups.put(SecondaryStat.Bless5th, new Pair<>(stack, Integer.valueOf((int) chr.getBuffLimit(400001050))));
    ...
    public static void LieDetecTer(MapleClient c) {
        ...
        if (c.getPlayer().getLevel() >= 220 && Randomizer.isSuccess(10, 1000) && !c.getPlayer().getMap().isTown() && c.getPlayer().getChair() == 0 && c.getPlayer().getEventInstance() == null && c.getPlayer().getMap().getId() != ServerConstants.warpMap && !GameConstants.isContentsMap(c.getPlayer().getMapId())) {
            if (c.getKeyValue("LIE_DETECT") == null) {
                c.sendLieDetoctor(c.getPlayer(), false);
            } else {
                if ((time - Long.parseLong(c.getKeyValue("LIE_DETECT")) >= lietime && c.getPlayer().getSkillCustomValue(179875) == null) || c.getKeyValue("LIE_DETECT") == null) {
                    c.sendLieDetoctor(c.getPlayer(), false);
                }
            }
    ...
    public static void IceAura(LittleEndianAccessor slea, MapleClient c) { //Mist
        ...
        for (MapleMist mists : c.getPlayer().getMap().getAllMistsThreadsafe()) {
            if (mists.getSourceSkill() != null && c.getPlayer().getId() == mists.getOwnerId() && mists.getSourceSkill().getId() == skillid) {
                mist = mists;
                break;
            }
        }
            ...
            if (skillid == 2221055) {
                if (c.getPlayer().getParty() != null) {
                    for (MaplePartyCharacter pchr : c.getPlayer().getParty().getMembers()) {
                        MapleCharacter other = c.getChannelServer().getPlayerStorage().getCharacterByName(pchr.getName());
                        if (other != null && mist.getBox().contains(other.getPosition()) && !other.getBuffedValue(2221054)) {
                            SkillFactory.getSkill(2221054).getEffect(c.getPlayer().getSkillLevel(2221054)).applyTo(mist.getOwner(), other);
                        }
                    }
                } else if (!c.getPlayer().getBuffedValue(2221054)) {
                    SkillFactory.getSkill(2221054).getEffect(c.getPlayer().getSkillLevel(2221054)).applyTo(c.getPlayer());
                }

                for (MapleMonster mob : c.getPlayer().getMap().getAllMonster()) {
                    if (mist.getBox().contains(mob.getPosition()) && mob.isAlive() && mob.getBuff(MonsterStatus.MS_Speed) == null && mob.getFreezingOverlap() > 0) {
