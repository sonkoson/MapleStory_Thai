...
import static constants.GameConstants.jobOrders;
    ...
    public static final void Loggedin(int playerid, MapleClient c) {
        try {
            ...
            if (player.getInnerSkills().get(0).size() == 0) {
                player.getInnerSkills().get(0).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 0));
                player.getInnerSkills().get(0).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 0));
                player.getInnerSkills().get(0).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 0));
            }

            if (player.getInnerSkills().get(1).size() == 0) {
                player.getInnerSkills().get(1).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 1));
                player.getInnerSkills().get(1).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 1));
                player.getInnerSkills().get(1).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 1));
            }

            if (player.getInnerSkills().get(2).size() == 0) {
                player.getInnerSkills().get(2).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 2));
                player.getInnerSkills().get(2).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 2));
                player.getInnerSkills().get(2).add(new InnerSkillValueHolder(70000015, (byte) 1, (byte) 1, (byte) 0, (byte) 2));
            }
            ...
            if (c.getPlayer().getLevel() >= 260) {
                if (c.getPlayer().getQuestStatus(1488) != 2) {
                    ...
                    c.getPlayer().changeSkillLevel(GameConstants.getHexaOriginFirst(c.getPlayer().getJob()), (byte) 1, (byte) 1);
                }
            ...
            int[] bossquests = {38214, 36013,
                33565, 31851, 31833, 3496, 3470, 30007, 3170, 31179, 3521, 31152,
                34015, 33294, 34330, 34585, 35632, 35731, 35815, 34478, 34331, 34478,
                100114, 16013, 34120, 34218, 34330, 34331, 34478, 34269, 34272, 34585,
                34586, 6500, 1465, 1466, 26607, 1484, 39921, 16059, 16015, 16013,
                100114, 34128, 39013, 34772, 39034, 34269, 34271, 34272, 34243, 39204,
                15417, 34377, 34477, 34450, 38401, 38404};
            ...
            if (player.choicepotential != null && player.memorialcube != null) {
                c.getSession().writeAndFlush(CField.getBlackCubeStart(player, (Item) player.choicepotential, false, player.memorialcube.getItemId(), player.memorialcube.getPosition(), player.getItemQuantity(5062010, false), 1));
            }
            ...
            if (!towerchair.equals("")) {
                ...
                c.getSession().writeAndFlush(CField.getGameMessage(5, "다양한 이벤트를 진행중입니다. 팬시 디스코드를 확인해주세요."));
                c.getSession().writeAndFlush(CField.getGameMessage(8, "팬시에서 현재 1.5배 경험치 피버 타임이 진행 중입니다!"));
                c.getSession().writeAndFlush(CField.getGameMessage(6, "팬시의 이용규칙을 준수하여 플레이해주시기 바랍니다. 매너있는 플레이 부탁드립니다."));
            
            // 이 패킷 쓰면 맵에 리미트 걸림
            //c.getSession().writeAndFlush(SLFCGPacket.StarDustUI(501661, "UI/UIWindowEvent.img/starDust_chuchu", player.getKeyValue(501661, "e"), c.getPlayer().getKeyValue(501661, "point")));
