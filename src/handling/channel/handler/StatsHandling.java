...
public class StatsHandling {
    ...
    public static final void DistributeSP(int skillid, int count, MapleClient c, MapleCharacter chr) {
        ...
        for (Pair<String, Integer> ski : skill.getRequiredSkills()) {
            int left;
            if (!(((String) ski.left).equals("level") ? chr.getLevel() < (Integer) ski.right : chr.getSkillLevel(SkillFactory.getSkill(left = Integer.parseInt((String) ski.left))) < (Integer) ski.right)) {
                continue;
            }
            return;
        }
