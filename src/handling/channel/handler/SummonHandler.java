...
public class SummonHandler {
    ...
    public static void applyAddAttack(MapleCharacter player, MapleMonster monster, int attackSkillId) {
        if (player.getBuffedValue(32141000)) { // 데스 VI 구현
            if (attackSkillId == 32141000) {
                if (monster.getStats().isBoss()) {
                    if (player.hexaMasterySkillStack < 48) {
                        player.hexaMasterySkillStack += 12;
                    }
                } else if (!monster.isAlive()) {
                    player.deathKillCount++;

                    if (player.deathKillCount >= 4) {
                        if (player.hexaMasterySkillStack < 48) {
                            player.hexaMasterySkillStack += 12;
                        }

                        player.deathKillCount = 0;
                    }
                }
            }
    ...
    public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        MapleSummon tempSummon = c.getPlayer().getSummon(500001002);
        ...
        if (ServerConstants.DEBUG_RECEIVE) {
            switch (skillid) {
                default:
                    System.err.println("[SummonAttack A] " + skillid + " / " + slea);
                    break;
            }
        }

        slea.skip(11); // 1.2.390 ++
        ...
        if (ServerConstants.DEBUG_RECEIVE) {
            switch (skillid) {
                default:
                    System.err.println("[SummonAttack B] " + skillid + " / " + slea);
                    break;
            }
        }
        ...
        for (final Pair<Integer, List<Long>> attackEntry : allDamage) {
            ...
            switch (skillid2) {
                ...
                case 33001016: {
                    ...
                    chr.addHP(chr.getStat().getCurrentMaxHp() * SkillFactory.getSkill(skillid2).getEffect(chr.getSkillLevel(skillid2)).getQ() / 100L);
                ...
                case 33121017: {
                    statusz2.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Freeze, new MonsterStatusEffect(skillid2, 10000), 1L));
                    statusz2.add(new Triple<MonsterStatus, MonsterStatusEffect, Long>(MonsterStatus.MS_Smite, new MonsterStatusEffect(skillid2, 10000), 1L));
                    if (mob.getBuff(MonsterStatus.MS_JaguarBleeding) == null && mob.getAnotherByte() > 0) {
                        mob.setAnotherByte(0);
                    }
                    if (mob.getAnotherByte() < 3) {
            ...
            applyAddAttack(chr, mob, skillid);
            ...
            if (mob.isBuffed(summonSkill.getId()) && GameConstants.isBattleMage(chr.getJob())) {
                ...
                if (size < 10 && skillid != 32001014 && skillid != 32141000 && skillid != 2111010 && skillid != 32100010 && skillid != 32110017 && skillid != 32120019 && !summon.isNoapply()) { // 데스 VI 구현
    ...
    public static final void RemoveSummon(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        //  System.err.println(summon.getSkill());
        ...
        if (summon.getSkill() == 400021047 || summon.getSkill() == 400021063 || summon.getSkill() == 400031047 || summon.getSkill() == 400041033 || summon.getSkill() == 400041034) {
            final int unk = slea.readInt(); // [othello] 372 ++
    ...
    public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        switch (sum.getSkill()) {
            ...
            case 1301013: { //todo more
                if (chr.hasSkill(1310016)) {
                    final ArrayList<MapleCharacter> partyMembers = chr.getPartyMembers();
                    if (!partyMembers.isEmpty()) {
                        for (MapleCharacter partyMember : partyMembers) {
                            chr.getSkillEffect(1310016).applyTo(partyMember);
                            partyMember.getMap().broadcastMessage(CField.SummonPacket.summonSkill(chr.getId(), sum.getObjectId(), 14));
                        }
                    } else {
                        chr.getSkillEffect(1310016).applyTo(chr);
                        chr.getMap().broadcastMessage(CField.SummonPacket.summonSkill(chr.getId(), sum.getObjectId(), 14));
                    }
                }
