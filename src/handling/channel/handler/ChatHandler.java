            ...
            if (c.getChatBlockedTime() == 0L) {
                if (chr.isHidden()) {
                    if (chr.isIntern() && !chr.isSuperGM() && unk == 0) {
                        chr.getMap().broadcastGMMessage(chr, CField.getChatText(chr, text, false, 1, item, type, emoticon), true);
                        if (unk == 0) {
                        ...
                        chr.getMap().broadcastGMMessage(chr, CField.getChatText(chr, text, c.getPlayer().isSuperGM(), unk, item, type, emoticon), true);
                    }
                    ...
                    if (unk == 0 || text.startsWith("~")) {
                        ...
                        chr.getMap().broadcastMessage(CField.getChatText(chr, text, false, 1, item, type, emoticon), c.getPlayer().getTruePosition());
                    }
                    ...
                    AdminTool.addMessage(0, "[" + chr.getClient().getChannel() + "채널] " + chr.getName() + " : " + text.replaceAll("~", ""));
                    FileoutputUtil.log(FileoutputUtil.일반채팅로그, "[일반] [" + chr.getClient().getChannel() + "채널] " + chr.getName() + " : " + text);
                    chr.getMap().broadcastMessage(CField.getChatText(chr, text, c.getPlayer().isSuperGM(), unk, item, type, emoticon), c.getPlayer().getTruePosition());
                }
    ...
    public static final void Whisper_Find(final LittleEndianAccessor slea, final MapleClient c, final RecvPacketOpcode recv) {
        ...
        switch (mode) {
            ...
            case 6: { // 귓말 , 귓말 아이템
                ...
                player2.getClient().getSession().writeAndFlush((Object) CField.getWhisper(c.getPlayer().getName(), c.getChannel(), text, item, c));
