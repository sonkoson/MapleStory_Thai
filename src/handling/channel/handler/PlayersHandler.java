...
public class PlayersHandler {
    ...
    public static void LinkPreset(LittleEndianAccessor slea, MapleClient c) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            int skillchar = slea.readInt();
            list.add(new Pair<>(Integer.valueOf(skillid), Integer.valueOf(skillchar)));
    ...
    public static final void TouchRune(LittleEndianAccessor slea, MapleCharacter chr) {
        ...
        if (GameConstants.isYeti(chr.getJob()) || GameConstants.isPinkBean(chr.getJob())) {
            ...
            switch (chr.getTouchedRune()) {
                // 파멸, 천둥, 거인, 어둠, 초월, 정화, 광선, 전이
                case 0: // 파멸의 룬 80001431
                    effect = SkillFactory.getSkill(80001431).getEffect(1);
                    effect.applyTo(chr, false);
                    break;
                case 1: // 천둥의 룬 80001752
                    effect = SkillFactory.getSkill(80001752).getEffect(1);
                    effect.applyTo(chr, false);
                    break;
                case 2: // 거인의 룬 80001753
                    // 9999020 리액터 작업 필요함
                    break;
                case 3: // 어둠의 룬 80001754
