...
public class HEXAHandler {
    ...
    public static ArrayList<HEXASkill> hexaSkillData(MapleClient c) {
        ...
        int enhanceFirstSkill = (GameConstants.getHexaEnhanceFirst(jobOrder) - GameConstants.getDifferencePatchNumber(jobId));
        int enhanceFirstSkillLevel = c.getPlayer().getSkillLevel(enhanceFirstSkill);

        hexas.add(new HEXASkill((30000000 + (jobOrder * 4) - GameConstants.getDifferencePatchNumber(jobId)), enhanceFirstSkillLevel));

        int enhanceSecondSkill = (GameConstants.getHexaEnhanceSecond(jobOrder) - GameConstants.getDifferencePatchNumber(jobId));
        int enhanceSecondSkillLevel = c.getPlayer().getSkillLevel(enhanceSecondSkill);

        hexas.add(new HEXASkill((30000001 + (jobOrder * 4) - GameConstants.getDifferencePatchNumber(jobId)), enhanceSecondSkillLevel));

        int enhanceThirdSkill = (GameConstants.getHexaEnhanceThird(jobOrder) - GameConstants.getDifferencePatchNumber(jobId));
        int enhanceThirdSkillLevel = c.getPlayer().getSkillLevel(enhanceThirdSkill);

        hexas.add(new HEXASkill((30000002 + (jobOrder * 4) - GameConstants.getDifferencePatchNumber(jobId)), enhanceThirdSkillLevel));

        int enhanceFourthSkill = (GameConstants.getHexaEnhanceFourth(jobOrder) - GameConstants.getDifferencePatchNumber(jobId));
        if (GameConstants.isXenon(c.getPlayer().getJob()) && GameConstants.getHexaEnhanceFourth(jobOrder) == 500004123) {
        ...
        if (enhanceFourthSkill != 500004127 && enhanceFourthSkill != 500004126) {
            hexas.add(new HEXASkill((30000003 + (jobOrder * 4) - GameConstants.getDifferencePatchNumber(jobId)), enhanceFourthSkillLevel));
        }
        ...
        for (int i = 0; i < hexaStatList.length; i++) {
            if (c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat M S") != -1) {
                hexas.add(
                        new HEXAStat(
                                hexaStatList[i],
                                new Pair<Integer, Integer>((int) c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat M S"), (int) c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat M L")),
                                new Pair<Integer, Integer>((int) c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat A1 S"), (int) c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat A1 L")),
                                new Pair<Integer, Integer>((int) c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat A2 S"), (int) c.getPlayer().getKeyValue(hexaStatList[i], "HEXA Stat A2 L"))
                        )
                );
            }
        }
        ...
        if (commonCoreId >= 40000000) {
            ...
            if (GameConstants.isXenon(c.getPlayer().getJob()) && GameConstants.getHexaEnhanceFourth(jobOrder) == 500004123) {
                if (commonCoreId == 30000007) {
                    c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFourth(jobOrder) + 4), (byte) 1, (byte) 1);
                } else {
                    c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFourth(jobOrder) + 3), (byte) 1, (byte) 1);
                }

            } else {
                c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFourth(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) 1, (byte) 1);
            }
            ...
            c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceThird(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) 1, (byte) 1);
        } else if ((commonCoreId + GameConstants.getDifferencePatchNumber(jobId)) >= 30000001) {
            c.getPlayer().gainSolErda(-4);

            MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, 4009547, 75, false, true);

            c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceSecond(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) 1, (byte) 1);
        } else if ((commonCoreId + GameConstants.getDifferencePatchNumber(jobId)) >= 30000000) {
            c.getPlayer().gainSolErda(-4);

            MapleInventoryManipulator.removeById(c, MapleInventoryType.ETC, 4009547, 75, false, true);

            c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFirst(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) 1, (byte) 1);
    ...
    public static void enhanceHexaSkill(LittleEndianAccessor slea, MapleClient c, int type) {
        ...
        if (commonCoreId >= 40000000) {
            ...
            if (GameConstants.isXenon(c.getPlayer().getJob()) && GameConstants.getHexaEnhanceFourth(jobOrder) == 500004123) {
                if (commonCoreId == 30000007) {
                    c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFourth(jobOrder) + 4), (byte) afterLevel, (byte) afterLevel);
                } else {
                    c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFourth(jobOrder) + 3), (byte) afterLevel, (byte) afterLevel);
                }
            } else {
                c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFourth(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) afterLevel, (byte) afterLevel);
            }
        } else if ((commonCoreId + GameConstants.getDifferencePatchNumber(jobId)) >= 30000002) {
            c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceThird(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) afterLevel, (byte) afterLevel);
        } else if ((commonCoreId + GameConstants.getDifferencePatchNumber(jobId)) >= 30000001) {
            c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceSecond(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) afterLevel, (byte) afterLevel);
        } else if ((commonCoreId + GameConstants.getDifferencePatchNumber(jobId)) >= 30000000) {
            c.getPlayer().changeSkillLevel((GameConstants.getHexaEnhanceFirst(jobOrder) - GameConstants.getDifferencePatchNumber(jobId)), (byte) afterLevel, (byte) afterLevel);
        } else if (commonCoreId >= 20000050) {
            int count = 0;
            int skillId = 0;
            while (skillId != -1) {
            ...
            c.getPlayer().changeSkillLevel(GameConstants.getHexaActiveFirst(jobId), (byte) afterLevel, (byte) afterLevel);
            ...
            c.getPlayer().changeSkillLevel(GameConstants.getHexaOriginFirst(jobId), (byte) afterLevel, (byte) afterLevel);
        }

        c.getSession().writeAndFlush(CField.updateHexaCore(c));
        c.getSession().writeAndFlush(CField.messageHexaCore(type, beforeLevel, new HEXASkill(coreId, afterLevel), true));
    }
    ...
    public static void activeHexaStat(LittleEndianAccessor slea, MapleClient c, int type) {
        ...
        HEXAStat hexa = new HEXAStat(
                coreId,
                new Pair<Integer, Integer>((int) c.getPlayer().getKeyValue(coreId, "HEXA Stat M S"), (int) c.getPlayer().getKeyValue(coreId, "HEXA Stat M L")),
                new Pair<Integer, Integer>((int) c.getPlayer().getKeyValue(coreId, "HEXA Stat A1 S"), (int) c.getPlayer().getKeyValue(coreId, "HEXA Stat A1 L")),
                new Pair<Integer, Integer>((int) c.getPlayer().getKeyValue(coreId, "HEXA Stat A2 S"), (int) c.getPlayer().getKeyValue(coreId, "HEXA Stat A2 L"))
        );
    ...
    public static void enhanceHexaStat(LittleEndianAccessor slea, MapleClient c, int type) {
        ...
        if (Randomizer.isSuccess(chance)) {
            c.getPlayer().setKeyValue(coreId, "HEXA Stat M L", (int) (c.getPlayer().getKeyValue(coreId, "HEXA Stat M L") + 1) + "");
        } else {
            if (Randomizer.isSuccess(50)) {
                if (c.getPlayer().getKeyValue(coreId, "HEXA Stat A1 L") >= 10) {
                    c.getPlayer().setKeyValue(coreId, "HEXA Stat A2 L", (int) (c.getPlayer().getKeyValue(coreId, "HEXA Stat A2 L") + 1) + "");
                } else {
                    c.getPlayer().setKeyValue(coreId, "HEXA Stat A1 L", (int) (c.getPlayer().getKeyValue(coreId, "HEXA Stat A1 L") + 1) + "");
                }
            } else {
    ...
    public static void saveHexaStat(LittleEndianAccessor slea, MapleClient c, int type) {
        ...
        c.getPlayer().setKeyValue((coreId + 666666), "HEXA Stat M S", c.getPlayer().getKeyValue(coreId, "HEXA Stat M S") + "");
        c.getPlayer().setKeyValue((coreId + 666666), "HEXA Stat M L", c.getPlayer().getKeyValue(coreId, "HEXA Stat M L") + "");
        c.getPlayer().setKeyValue((coreId + 666666), "HEXA Stat A1 S", c.getPlayer().getKeyValue(coreId, "HEXA Stat A1 S") + "");
        c.getPlayer().setKeyValue((coreId + 666666), "HEXA Stat A1 L", c.getPlayer().getKeyValue(coreId, "HEXA Stat A1 L") + "");
        c.getPlayer().setKeyValue((coreId + 666666), "HEXA Stat A2 S", c.getPlayer().getKeyValue(coreId, "HEXA Stat A2 S") + "");
        c.getPlayer().setKeyValue((coreId + 666666), "HEXA Stat A2 L", c.getPlayer().getKeyValue(coreId, "HEXA Stat A2 L") + "");
