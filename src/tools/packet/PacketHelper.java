...
public class PacketHelper {
    ...
    public static final void addSkillInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        for (Triple<Skill, SkillEntry, Integer> linkskill : linkskills) {
            addLinkSkillInfo(mplew, ((Skill) linkskill.getLeft()).getId(), ((Integer) linkskill.getRight()).intValue(), (chr.getSkillLevel(((Skill) linkskill.getLeft()).getId()) == 0) ? ((Integer) linkskill.getRight()).intValue() : chr.getId(), ((SkillEntry) linkskill.getMid()).skillevel);
        }
    ...
    public static void addInventoryInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr, boolean change_load_type) {
    ...
    public static final void addCharStats(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        if (GameConstants.isDemonSlayer(chr.getJob()) || GameConstants.isXenon(chr.getJob()) || GameConstants.isDemonAvenger(chr.getJob()) || GameConstants.isArk(chr.getJob()) || GameConstants.isHoyeong(chr.getJob())) {
    ...
    public static void addEquipSpecialStats(MaplePacketLittleEndianWriter mplew, Equip equip) {
        ...
        if (head != 0) {
            ...
            if (equip.getSpecialStats().contains(EquipSpecialStat.ITEM_LEVEL)) {
                //System.out.println("5 : " + Math.max(equip.getBaseLevel(), equip.getEquipLevel()));
            ...
            if (equip.getSpecialStats().contains(EquipSpecialStat.EQUIPMENT_TYPE)) {
                //System.out.println("22 : " + ((equip.getEquipmentType() == 1) ? 306 : ((equip.getEquipmentType() == 2) ? 344 : ((equip.getEquipmentType() == 3) ? 282 : equip.getEquipmentType()))));
    ...
    public static void addHexaStatInfo(MaplePacketLittleEndianWriter mplew, MapleClient c) {
        ...
        for (HEXAStat hexa : hexas) {
            ...
            for (int i = 0; i < 2; i++) {
                ...
                mplew.writeInt((i == 0 ? 1 : (c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat M S") == -1) ? 0 : 1)); // 저장 여부
                    ...
                    if (i == 0) {
                        ...
                        switch (j) {
                            case 0:
                                ...
                                mplew.writeInt(((c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat M L") != -1) ? c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat M L") : 0));
                                break;
                            case 1:
                                mplew.writeInt(c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat A1 S"));
                                mplew.writeInt(((c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat A1 L") != -1) ? c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat A1 L") : 0));
                                break;
                            case 2:
                                mplew.writeInt(c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat A2 S"));
                                mplew.writeInt(((c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat A2 L") != -1) ? c.getPlayer().getKeyValue((hexa.getCoreId() + 666666), "HEXA Stat A2 L") : 0));
                                break;
                        }
    ...
    public static void addShopInfo(MaplePacketLittleEndianWriter mplew, MapleShop shop, MapleClient c) {
        ...
        for (Item item : c.getPlayer().getRebuy()) {
            i++;
            addShopItemInfo(mplew, new MapleShopItem(0, (short) 1000, item.getItemId(), (int) ii.getPrice(item.getItemId()), 0, (byte) 0, (short) item.getQuantity(), 0, i, 0, 0, 0, 0, 0, 0, 0), shop, ii, item, c.getPlayer());
        }
    ...
    public static void encodeForRemote(MaplePacketLittleEndianWriter mplew, Map<SecondaryStat, Pair<Integer, Integer>> statups, MapleCharacter chr) {
        ...
        if (statups.containsKey(SecondaryStat.StopForceAtominfo)) { // 1.2.391 ok.
            mplew.writeInt((chr.getBuffSource(SecondaryStat.StopForceAtominfo) == 61121217) ? 4 : ((chr.getBuffSource(SecondaryStat.StopForceAtominfo) == 61110211) ? 3 : ((chr.getBuffSource(SecondaryStat.StopForceAtominfo) != 61101002 && chr.getBuffSource(SecondaryStat.StopForceAtominfo) != 61110211 && chr.getBuffSource(SecondaryStat.StopForceAtominfo) != 61141006) ? 2 : 1)));
    ...
    public static void encodeForLocal(MaplePacketLittleEndianWriter mplew, SecondaryStatEffect effect, Map<SecondaryStat, Pair<Integer, Integer>> statups, MapleCharacter chr, List<Pair<SecondaryStat, Pair<Integer, Integer>>> newstatups) {
        ...
        if (statups.containsKey(SecondaryStat.StopForceAtominfo)) { // 1.2.391 ok.
            mplew.writeInt((effect.getSourceId() == 61121217) ? 4 : ((effect.getSourceId() == 61110211) ? 3 : ((effect.getSourceId() != 61101002 && effect.getSourceId() != 61110211 && effect.getSourceId() != 61141006) ? 2 : 1)));
        ...
        for (Pair<SecondaryStat, Pair<Integer, Integer>> stat : newstatups) { // 1.2.391 ok.
            if (!((SecondaryStat) stat.left).canStack() && ((SecondaryStat) stat.left).isSpecialBuff()) {
                ...
                mplew.writeInt((stat.left == SecondaryStat.UnkBuffStat707) ? 0 : chr.getBuffSource((SecondaryStat) stat.left));
