...
public class MobPacket {
    ...
    public static byte[] bloodQueenAttack1(int oid, int type, boolean left) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.BLOOD_QUEEN_ATTACK1.getValue());
    ...
    public static byte[] bloodQueenAttack2(int oid, int type, boolean left) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.BLOOD_QUEEN_ATTACK2.getValue());
    ...
    public static byte[] killMonster(int oid, int animation) {
        ...
        mplew.writeInt(oid);
        mplew.writeShort(animation); // 1.2.390 Changed. (Byte -> Short)
        mplew.writeInt(0);
        mplew.writeInt(0);
        if (animation == 9) {
            mplew.writeInt(-1);
    ...
    public static byte[] showBossHP(MapleMonster mob) {
        ...
        mplew.writeLong(mob.isEliteboss() ? (long) (mob.getHp() * mob.bonusHp()) : (mob.getHp() + mob.getBarrier()));
        mplew.writeLong(mob.isEliteboss() ? mob.getElitehp() : (long) (mob.getStats().getHp() * mob.bonusHp()));
    ...
    public static byte[] spawnMonster(MapleMonster life, int spawnType, int link, int control) {
        ...
        if (life.getId() == 8881000) {
            ...
            for (Triple<String, Integer, Integer> skeleton : life.getStats().getSkeleton()) {
                ...
                mplew.writeShort(((Integer) skeleton.mid).intValue());
            }
    ...
    public static byte[] applyMonsterStatus(MapleMonster mons, List<Pair<MonsterStatus, MonsterStatusEffect>> mse, boolean fromMonster, SecondaryStatEffect effect) {
        ...
        mplew.writeShort(70);
        mplew.write(getMse(mse, MonsterStatus.MS_Origin_Bind) || getMse(mse, MonsterStatus.MS_PowerImmune) ? 3
                : getMse(mse, MonsterStatus.MS_Freeze) ? 2
                : 0);
    ...
    private static void encodeTemporary(MaplePacketLittleEndianWriter mplew, MapleMonster mob, List<Pair<MonsterStatus, MonsterStatusEffect>> mse, SecondaryStatEffect effect) {
        ...
        PacketHelper.writeMonsterMaskT(mplew, newstatups); // 1.2.391 ok.
        ...
        for (Pair<MonsterStatus, MonsterStatusEffect> ms : newstatups) { // 1.2.391 ok.
        ...
        if (getMse(mse, MonsterStatus.MS_Pdr)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_Mdr)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_Speed)) { // 1.2.391 ok.
            mplew.write(Math.min(5, mob.getFreezingOverlap()));
        }
        if (getMse(mse, MonsterStatus.MS_Freeze)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_Freeze).getChr().getId());
        }
        if (getMse(mse, MonsterStatus.MS_390_UNK16)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_PCounter)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_MCounter)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_PCounter) || getMse(mse, MonsterStatus.MS_MCounter)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.write(0);
            mplew.writeInt(1);
        }
        if (getMse(mse, MonsterStatus.MS_Puriaus)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_Puriaus).getChr().getId());
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_AdddamParty)) { // 1.2.391 ok.
        ...
        if (getMse(mse, MonsterStatus.MS_FixdamRBuff)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_ElementDarkness)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_DeadlyCharge)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_Incizing)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_Incizing).getChr().getId());
            mplew.writeInt(10);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_BMageDebuff)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_BMageDebuff).getChr().getId());
        }
        if (getMse(mse, MonsterStatus.MS_PvPHelenaMark)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_MultiPMDR)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_MultiPMDR).getValue() / 10L);
        }
        if (getMse(mse, MonsterStatus.MS_PsychicGroundMark)) { // 1.2.391 ok.
            MonsterStatusEffect eff = null;
            for (Pair<MonsterStatus, MonsterStatusEffect> stat : mse) {
                if (stat.getLeft() == MonsterStatus.MS_PsychicGroundMark) {
                    eff = stat.getRight();
                    break;
                }
            }
        ...
        if (getMse(mse, MonsterStatus.MS_ElementResetBySummon)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_DragonStrike)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_DragonStrike).getChr().getId());
        }
        if (getMse(mse, MonsterStatus.MS_CurseMark)) { // 1.2.391 ok.
            mplew.writeInt(4);
            mplew.writeInt(0);
            mplew.writeInt(152120013);
        }
        if (getMse(mse, MonsterStatus.MS_PopulatusTimer)) { // 1.2.391 ok.
            mplew.writeInt(1);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk80)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk81)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk92)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_MultiDamSkill)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_RWChoppingHammer)) { // 1.2.391 ok.
            mplew.writeInt(mob.getBuff(MonsterStatus.MS_RWChoppingHammer).getChr().getId());
        }
        if (getMse(mse, MonsterStatus.MS_TimeBomb)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_Explosion)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk102)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_Burned)) { // 1.2.391 ok.
            int count = 0;
        ...
        if (getMse(mse, MonsterStatus.MS_BalogDisable)) { // 1.2.391 ok.
            mplew.write(0);
            mplew.write(0);
        }
        if (getMse(mse, MonsterStatus.MS_ExchangeAttack)) { // 1.2.391 ok.
            mplew.write(1);
        }
        if (getMse(mse, MonsterStatus.MS_AddBuffStat)) { // 1.2.391 ok.
        ...
        if (getMse(mse, MonsterStatus.MS_LinkTeam)) { // 1.2.391 ok.
            mplew.writeMapleAsciiString("");
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk119)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk120)) { // 1.2.391 ok.
            mplew.writeLong(0L);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk121)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk122)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk124)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk125)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_Origin_Bind)) { // 1.2.391 ok.
            mplew.writeShort(22);
        }
        if (getMse(mse, MonsterStatus.MS_SoulExplosion)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_SeperateSoulP)) { // 1.2.391 ok.
            mplew.writeInt(50);
            mplew.writeInt(mob.getObjectId());
        ...
        if (getMse(mse, MonsterStatus.MS_SeperateSoulC)) { // 1.2.391 ok.
            mplew.writeInt(50);
            mplew.writeInt(mob.getSeperateSoul());
            mplew.writeShort(17425);
            mplew.writeInt(43);
        }
        if (getMse(mse, MonsterStatus.MS_TrueSight)) { // 1.2.391 ok.
        ...
        if (getMse(mse, MonsterStatus.MS_Laser)) { // 1.2.391 ok.
        ...
        if (getMse(mse, MonsterStatus.MS_390_Unk118)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk126)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk127)) { // 1.2.391 ok.
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk129)) { // 1.2.391 ok.
            mplew.writeLong(0);
        }
        if (getMse(mse, MonsterStatus.MS_390_Unk98)) { // 1.2.391 ok.
            mplew.writeInt(0);
        }
        if (getMse(mse, MonsterStatus.MS_IndieUNK)) { // 1.2.391 ok.
    ...
    public static byte[] 서펜트마크(int skillid, int mobid) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.MONSTER_STACK.getValue());
        mplew.writeInt(skillid);
        mplew.write(false);
    ...
    public static class BossLucid {
        ...
        public static byte[] createButterfly(int initId, List<Butterfly> butterflies) {
            ...
            mplew.writeInt(butterflies.size()); // ok
    ...
    public static byte[] monsterResistOriginBind(MapleMonster monster, MapleCharacter player, int time, int skill) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.MONSTER_RESIST.getValue());
    ...
    public static byte[] DemianTranscendenTalSet(MapleMonster demian, MapleMonster TranscendenMob) {
        ...
        mplew.writeInt(5); // V4
        mplew.writeInt(TranscendenMob.getObjectId()); // V5
        return mplew.getPacket();
    }
    ...
    //[othello] 익스트림 보스
    public static byte[] useFieldSkillExtremeSeren(FieldSkill fieldSkill) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.FIELD_SKILL.getValue());
        mplew.writeInt(fieldSkill.getSkillId());
        mplew.writeInt(fieldSkill.getSkillLevel());
    ...
    public static class BossSlime {

        public static byte[] useFieldSkillSlime(FieldSkill fieldSkill) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.FIELD_SKILL.getValue());
            mplew.writeInt(fieldSkill.getSkillId());
            mplew.writeInt(fieldSkill.getSkillLevel());
            switch (fieldSkill.getSkillId()) {
                ...
                case 100023: {
                    ...
                    mplew.writeInt(210);
                    mplew.writeInt(1);
                    int radomcount = Randomizer.rand(3, 6);
                    for (int i = 0; i < radomcount; i++) {
                        mplew.writeShort(Randomizer.rand(36, 1375));
                        mplew.writeShort(Randomizer.rand(-1649, -1102));
                        mplew.writeShort(Randomizer.rand(-23, 1442));
                        mplew.writeShort(Randomizer.rand(-2555, -1438));
        ...
        public static byte[] SlimePacket(int type, MapleMap map) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.SLIME_PACKET.getValue());
            mplew.writeInt(type);
            switch (type) {
                case 1:
                    mplew.writeInt(4);
                    mplew.writeInt(6734693);
                    mplew.write(1);
                    break;
                case 2://지하 보여주기
                    mplew.write(HexTool.getByteArrayFromHexString("CD 02 00 00 F9 FA FF FF CD 02 00 00 8A FF FF FF F4 01 00 00 2C 01 00 00 A0 0F 00 00"));
                    break;
                case 3:
                    mplew.write(HexTool.getByteArrayFromHexString("20 01 00 00 05 00 68 6F 6C 65 31 01 05 00 68 6F 6C 65 32 01 05 00 68 6F 6C 65 33 01 07 00 62 6F 74 74 6F 6D 31 01 07 00 62 6F 74 74 6F 6D 32 01 07 00 62 6F 74 74 6F 6D 34 01 08 00 62 6F 74 74 6F 6D 31 35 01 08 00 62 6F 74 74 6F 6D 31 34 01 08 00 62 6F 74 74 6F 6D 31 32 01 08 00 62 6F 74 74 6F 6D 31 33 01 08 00 62 6F 74 74 6F 6D 31 31 01 07 00 62 6F 74 74 6F 6D 39 01 08 00 62 6F 74 74 6F 6D 31 30 01 07 00 62 6F 74 74 6F 6D 38 01 07 00 62 6F 74 74 6F 6D 36 01 07 00 62 6F 74 74 6F 6D 37 01 07 00 62 6F 74 74 6F 6D 35 01 07 00 62 6F 74 74 6F 6D 33 01 05 00 61 73 65 74 32 00 05 00 61 73 65 74 33 00 05 00 61 73 65 74 35 00 05 00 61 73 65 74 34 00 06 00 61 73 65 74 31 38 00 06 00 61 73 65 74 31 36 00 06 00 61 73 65 74 31 37 00 06 00 61 73 65 74 31 35 00 06 00 61 73 65 74 31 34 00 05 00 61 73 65 74 36 00 05 00 61 73 65 74 31 00 06 00 61 73 65 74 32 39 00 06 00 61 73 65 74 32 38 00 06 00 61 73 65 74 32 35 00 06 00 61 73 65 74 32 36 00 06 00 61 73 65 74 32 37 00 05 00 61 73 65 74 37 00 06 00 61 73 65 74 31 32 00 05 00 61 73 65 74 38 00 05 00 61 73 65 74 39 00 06 00 61 73 65 74 31 31 00 06 00 61 73 65 74 31 30 00 06 00 61 73 65 74 31 39 00 06 00 61 73 65 74 32 30 00 06 00 61 73 65 74 32 31 00 06 00 61 73 65 74 32 33 00 06 00 61 73 65 74 32 32 00 06 00 61 73 65 74 33 30 00 06 00 61 73 65 74 33 34 00 06 00 61 73 65 74 33 31 00 06 00 61 73 65 74 33 32 00 06 00 61 73 65 74 33 33 00 06 00 61 73 65 74 33 35 00 06 00 61 73 65 74 35 35 00 06 00 61
38 00 06 00 64 73 65 74 33 30 00 06 00 64 73 65 74 34 31 00 06 00 64 73 65 74 34 33 00 06 00 64 73 65 74 34 32 00 06 00 64 73 65 74 34 37 00 06 00 64 73 65 74 34 35 00 06 00 64 73 65 74 34 36 00 06 00 64 73 65 74 34 34 00 06 00 64 73 65 74 38 38 00 05 00 65 73 65 74 34 00 05 00 65 73 65 74 32 00 05 00 65 73 65 74 31 00 05 00 65 73 65 74 35 00 05 00 65 73 65 74 33 00 05 00 65 73 65 74 39 00 06 00 65 73 65 74 31 30 00 05 00 65 73 65 74 38 00 05 00 65 73 65 74 37 00 06 00 65 73 65 74 31 31 00 05 00 65 73 65 74 36 00 06 00 65 73 65 74 31 35 00 06 00 65 73 65 74 31 36 00 06 00 65 73 65 74 31 34 00 06 00 65 73 65 74 31 32 00 06 00 65 73 65 74 31 37 00 06 00 65 73 65 74 31 33 00 06 00 65 73 65 74 32 30 00 06 00 65 73 65 74 32 31 00 06 00 65 73 65 74 31 39 00 06 00 65 73 65 74 31 38 00 06 00 65 73 65 74 32 34 00 06 00 65 73 65 74 32 35 00 06 00 65 73 65 74 32 32 00 06 00 65 73 65 74 32 33 00 06 00 65 73 65 74 33 33 00 06 00 65 73 65 74 33 32 00 06 00 65 73 65 74 33 31 00 06 00 65 73 65 74 32 36 00 06 00 65 73 65 74 33 34 00 06 00 65 73 65 74 32 39 00 06 00 65 73 65 74 32 38 00 06 00 65 73 65 74 32 37 00 06 00 65 73 65 74 33 30 00 06 00 65 73 65 74 33 38 00 06 00 65 73 65 74 33 39 00 06 00 65 73 65 74 33 37 00 06 00 65 73 65 74 33 36 00 06 00 65 73 65 74 34 32 00 06 00 65 73 65 74 33 35 00 06 00 65 73 65 74 34 30 00 06 00 65 73 65 74 34 31 00 06 00 65 73 65 74 39 30 00 06 00 65 73 65 74 39 31 00"));
                    break;
                /*int subtype = 1;
                    long gague = 1;
                    mplew.writeInt(map.getNodez().getEnvironments().size());
                    for (MapleNodes.Environment env : map.getNodez().getEnvironments()) {
                        mplew.writeMapleAsciiString(env.getName());
                        switch (subtype) {
                            case 1: {
                                if ((gague == 0 && env.getName().contains("aset")) || (gague == 1 && env.getName().contains("bset")) || (gague == 2 && env.getName().contains("cset")) || (gague == 3 && env.getName().contains("dset")) || (gague == 4 && env.getName().contains("eset")) || env.getName().equals("bottom1") || env.getName().equals("bottom2") || env.getName().equals("bottom15") || env.getName().equals("bottom4") || env.getName().equals("bottom14") || env.getName().equals("bottom12") || env.getName().equals("bottom13") || env.getName().equals("bottom11") || env.getName().equals("bottom9") || env.getName().equals("bottom10") || env.getName().equals("bottom8") || env.getName().equals("bottom6") || env.getName().equals("bottom7") || env.getName().equals("bottom5") || env.getName().equals("bottom3")) {
                                    mplew.write(0);
                                } else {
                                    mplew.write(0);
                                }
                            ...
                            default: {
                                if (env.getName().equals("hole1") || env.getName().equals("hole2") || env.getName().equals("hole3") || env.getName().equals("bottom11") || env.getName().equals("bottom9") || env.getName().equals("bottom10") || env.getName().equals("bottom8") || env.getName().equals("bottom6") || env.getName().equals("bottom7") || env.getName().equals("bottom5") || env.getName().equals("bottom3")) {
                                    mplew.write(0);
                                } else {
                                    mplew.write(0);
                                }
                ...
                case 7://마그마 소환
                    mplew.write(HexTool.getByteArrayFromHexString("01 00 00 00 04 00 00 00 01 01 00 00 00 FB 01 00 00 BB F7 FF FF AF 02 00 00 6F F8 FF FF 0F 02 00 00 15 F8 FF FF 9B 02 00 00 6F F8 FF FF BA FF FF FF A6 FF FF FF 46 00 00 00 00 00 00 00 2D 02 00 00 1F F8 FF FF 7D 02 00 00 6F F8 FF FF D8 FF FF FF B0 FF FF FF 28 00 00 00 00 00 00 00 55 02 00 00 6F F8 FF FF 01 00 00 00 02 00 00 00 38 04 00 00 00 00 00 00 49 82 87 00 00 00 00 00 49 82 87 00 03 00 00 00 01 01 00 00 00 46 03 00 00 E6 F8 FF FF FA 03 00 00 9A F9 FF FF 5A 03 00 00 40 F9 FF FF E6 03 00 00 9A F9 FF FF BA FF FF FF A6 FF FF FF 46 00 00 00 00 00 00 00 78 03 00 00 4A F9 FF FF C8 03 00 00 9A F9 FF FF D8 FF FF FF B0 FF FF FF 28 00 00 00 00 00 00 00 A0 03 00 00 9A F9 FF FF 02 00 00 00 03 00 00 00 38 04 00 00 00 00 00 00 49 82 87 00 00 00 00 00 49 82 87 00 03 00 00 00 01 01 00 00 00 4E 01 00 00 0B FB FF FF 02 02 00 00 BF FB FF FF 62 01 00 00 65 FB FF FF EE 01 00 00 BF FB FF FF BA FF FF FF A6 FF FF FF 46 00 00 00 00 00 00 00 80 01 00 00 6F FB FF FF D0 01 00 00 BF FB FF FF D8 FF FF FF B0 FF FF FF 28 00 00 00 00 00 00 00 A8 01 00 00 BF FB FF FF 03 00 00 00 05 00 00 00 38 04 00 00 00 00 00 00 49 82 87 00 00 00 00 00 49 82 87 00 03 00 00 00 01 01 00 00 00 00 03 00 00 0B FB FF FF B4 03 00 00 BF FB FF FF 14 03 00 00 65 FB FF FF A0 03 00 00 BF FB FF FF BA FF FF FF A6 FF FF FF 46 00 00 00 00 00 00 00 32 03 00 00 6F FB FF FF 82 03 00 00 BF FB FF FF D8 FF FF FF B0 FF FF FF 28 00 00 00 00 00 00 00 5A 03 00 00 BF FB FF FF 04 00 00 00 07 00 00 00 38 04 00 00 00 00 00 00 49 82 87 00 00 00 00 00 49 82 87 00 03 00 00 00"));
                    break;
                case 8://문 열기
                    mplew.write(HexTool.getByteArrayFromHexString("00 00 00 00 03 00 00 00 08 00 00 00 24 00 00 00 99 F9 FF FF 5F 05 00 00 99 F9 FF FF 24 00 00 00 BC FB FF FF 5F 05 00 00 BC FB FF FF 24 00 00 00 E0 FE FF FF 71 05 00 00 E0 FE FF FF 24 00 00 00 37 00 00 00 71 05 00 00 37 00 00 00 02 00 00 00 02 00 00 00 02 00 00 00 02 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 01 00 00 00 00 00 00 00 01 00 00 00 02 00 00 00 03 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 01 00 01 00 01 00 01"));
                    break;
                case 9://슬라임 게이지 충전
                    mplew.writeInt(100);//충전량
                    mplew.writeInt(100);
                    mplew.writeInt(100);
                    break;
                case 11:
                    ...
                    break;
                case 12: //지하막기
                    mplew.write(HexTool.getByteArrayFromHexString("42 82 87 00 0C 00 00 00 C2 FB FF FF"));
                    break;
                case 13:
                    mplew.writeInt(10);
                    mplew.writeInt(3);
