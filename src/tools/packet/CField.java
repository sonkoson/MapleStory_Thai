...
public class CField {
    ...
    public static class SummonPacket {
        ...
        public static byte[] DeathAttack(MapleSummon summon, int skillvalue) {
            ...
            // 데스 VI 구현 [S]
            mplew.writeInt(summon.getSkill() == 32141000 && summon.getOwner().hexaMasterySkillStack >= 48 ? 9 : skillvalue);
            ...
            if (summon.getOwner().hexaMasterySkillStack >= 48) {
                ...
                statups.put(SecondaryStat.BMageDeath, new Pair<>(Integer.valueOf(summon.getOwner().getDeath()), Integer.valueOf(0)));
                summon.getOwner().getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, summon.getOwner().getBuffedEffect(SecondaryStat.BMageDeath), summon.getOwner()));
            }
            // 데스 VI 구현 [E]
    ...
    public static byte[] getCharInfo(MapleCharacter chr) {
        ...
        if (sundayMaple) {
            ...
            mplew.writeMapleAsciiString("#fn나눔고딕 ExtraBold##fs15##fc0xFFB7EC00#" + currentDate + " " + currentDayOfWeek.getDisplayName(TextStyle.FULL, Locale.KOREAN));
            mplew.writeInt(60);
            mplew.writeInt(220);
        }

        int damageSkinCode = (int) (chr.getKeyValue(7293, "damage_skin") == -1 ? 0 : chr.getKeyValue(7293, "damage_skin"));
...
// [SEX] 멀티 채팅패킷 전체수정
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        int damageSkinCode = (int) (chr.getKeyValue(7293, "damage_skin") == -1 ? 0 : chr.getKeyValue(7293, "damage_skin")); // 데미지 스킨
        mplew.writeInt(damageSkinCode);
    ...
    public static byte[] getChatText(MapleCharacter chr, String text, boolean whiteBG, int show, Item item, int type, int emoticon) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort((item == null && show != 11) ? SendPacketOpcode.CHATTEXT.getValue() : SendPacketOpcode.CHATTEXTITEM.getValue());
    ...
    public static byte[] getWhiteCubeStart(MapleCharacter chr, Item item, boolean up, int cubeId, int cubePosition, int remainCount, int cudestack) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getBlackCubeStart(MapleCharacter chr, Item item, boolean up, int cubeId, int cubePosition, int remainCount, int cudestack) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] skillCooldown(int sid, int time) {
        ...
        mplew.writeInt((sid == 25121133 || sid == 400041051 || sid == 152110004 || sid == 400011135 || sid == 400001063 || sid == 2221055 || sid == 2211015 || sid == 2221055) ? sid : GameConstants.getLinkedSkill(sid));
        mplew.writeInt(time);
...
//        mplew.write(-1); //274 ++
    ...
    public static class AuctionPacket {
        ...
        public static void auctionItem(MaplePacketLittleEndianWriter mplew, AuctionItem item) {
            ...
            if (item.getState() == 3 || GameConstants.getInventoryType(item.getItem().getItemId()) == MapleInventoryType.CASH && item.getItem().getQuantity() > 1) {
                mplew.writeLong(item.getDirectPrice()); // nDirectPrice
            } else {
                mplew.writeLong(item.getItem().getQuantity() > 1 ? item.getPrice() * item.getItem().getQuantity() : item.getDirectPrice()); // nDirectPrice
            }
    ...
    public static byte[] RuneAction(int type, int time) {
        ...
        if (type == 8 || type == 9) {
            ...
            long[] action = {172471030436L, -295930472239813559L, -235836247967617121L, -4528850249916986908L, -7978072759301675534L, -8535685939159527094L};
    ...
    public static byte[] bonusAttackRequest(int bonusAttackSkillId, List<Triple<Integer, Integer, Integer>> mobList, int attackSkillId) {
        MaplePacketLittleEndianWriter pw = new MaplePacketLittleEndianWriter();
        pw.writeShort(SendPacketOpcode.BONUS_ATTACK_REQUEST.getValue());
    ...
    public static byte[] bonusAttackRequest(int skillId, ArrayList<MapleMonster> monsters, boolean unk, int attackSkillId) {
        MaplePacketLittleEndianWriter pw = new MaplePacketLittleEndianWriter();
        pw.writeShort(SendPacketOpcode.BONUS_ATTACK_REQUEST.getValue());
    ...
    public static byte[] unionArtipactSet() {
        ...
        for (int i = 0; i < artipactSettingsLength; i++) {
            mplew.writeInt(MapleUnionArtipact.settings[i][0]); // 아티팩트 포인트 최대 보유량
            mplew.writeInt(MapleUnionArtipact.settings[i][1]); // 필요 경험치
            mplew.writeInt(MapleUnionArtipact.settings[i][2]); // 보유 아티팩트 AP
        }
    ...
    public static byte[] unionArtipactQuestMsg(int type, int exp, int point, int index, long time) {
        ...
        switch (type) {
            ...
            case 3: {
                ...
                String packet = (data[4] + " " + data[5] + " " + data[6] + " " + data[7] + " " + data[0] + " " + data[1] + " " + data[2] + " " + data[3]);
    ...
    public static byte[] fishing(int type) {
        ...
        switch (type) {
            ...
            case 2:
                //임시 처리, sub_120E8B0
                /*
                00 00 00 40 0B 16 40 40 //fDistance
                00 //nState
                00 00 00 00 00 00 2E 40 //fDamageMin
                00 00 00 00 00 80 41 40 //fDamageMax
                ...
                00 00 00 00 //nState
                F4 01 00 00 //tTimeMin
                00 00 00 00 //tTimeRange
                00 00 00 00 00 00 00 00 //fDistanceMin
                00 00 00 00 00 00 00 00 //fDistanceMax

                01 00 00 00
                B8 0B 00 00
                B8 0B 00 00 //3000
                00 00 00 A0 99 99 B9 3F
                00 00 00 A0 99 99 C9 3F

                02 00 00 00
                E8 03 00 00
                D0 07 00 00 //2000
                00 00 00 A0 99 99 A9 BF
    ...
    public static byte[] spawnSecondAtoms(int cid, List<SecondAtom> tiles, int spawnType) {
        ...
        for (SecondAtom tile : tiles) {
            ...
            mplew.writeInt((tile.getSkillId() == 5201017) ? 450 + ((i - 1) * 120) : (tile.getSkillId() == 3111016) ? 480 : (tile.getSkillId() == 2121052) ? 480 : (tile.getSkillId() == 4121020) ? 810 + ((i - 1) * 60) : (tile.getSkillId() == 162121010) ? 1110 : ((tile.getSkillId() == 162111005) ? 600 : ((tile.getSkillId() == 400051069) || (tile.getSkillId() == 2311017) ? (i * 120) : ((tile.getSkillId() == 400041058) ? 150 : 0))));
            mplew.writeInt((tile.getSkillId() == 13141502) ? 270 : (tile.getSkillId() == 13141503 || tile.getSkillId() == 13141504) ? 120 : (tile.getSkillId() == 162121010) ? (20 + (i - 1) * 60) : (tile.getSkillId() == 3111016) ? 30 : 0);
            ...
            mplew.writeInt(0);
            mplew.writeInt((tile.getSkillId() == 5201017) ? 10 : (tile.getSkillId() == 4121020) ? 30 : (tile.getSkillId() == 162111002) ? 20 : ((tile.getSkillId() == 400051069 || tile.getSkillId() == 400021092 || tile.getSkillId() == 400051069 || tile.getSkillId() == 162101000 || tile.getSkillId() == 2121052 || tile.getSkillId() == 13141502 || tile.getSkillId() == 13141503 || tile.getSkillId() == 13141504) ? 1 : 0));
