...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void MapiaStart(final MapleCharacter player, int time, final int morningmap, final int citizenmap1, final int citizenmap2, final int citizenmap3, final int citizenmap4, final int citizenmap5, final int citizenmap6, final int mapiamap, final int policemap, final int drmap, final int after, final int night, final int vote, int bating) {
        String[] job = {"미남", "미녀", "장미칼", "짜잔", "유리멘탈", "부처맨탈", "주인공", "[Happy oz!] 히로인", "[Happy oz!] 엑스트라", "[Happy oz!] 신", "[Happy oz!] 어그로", "[Happy oz!] 프라이데이", "[Happy oz!] 동정", "[Happy oz!] 히키코모리", "[Happy oz!] 간지인", "[Happy oz!] 멋쟁이", "[Happy oz!]야생의"};
        String name = "";
        String mapia = "";
        String police = "";
    ...
    public void EnterFairy(boolean black, boolean effect, boolean blackflame) {
        ...
        if (effect) {
            Timer.EtcTimer.getInstance().schedule(() -> this.c.getPlayer().getClient().getSession().writeAndFlush(CField.showSpineScreen(false, false, true, "Back/fairyAcademy.img/back/5", "animation", 0, false, "")), 1000L);
        }
        ...
        Timer.EtcTimer.getInstance().schedule(() -> {
            if (effect) { //이미지 여기서 다 수정
                this.c.getPlayer().getClient().getSession().writeAndFlush(CField.showSpineScreen(false, false, true, "Back/fairyAcademy.img/back/5", "animation", 0, false, ""));
            }
            this.c.send(SLFCGPacket.InGameDirectionEvent("", 1, 1000));
