...
public class PinkbeanHapplyNewWeek {
    ...
    public void incCoinCount(MapleClient client, Integer amount) {
        ...
        client.getSession().writeAndFlush(CWvsContext.InfoPacket.updateClientInfoQuest(this.getPrimaryQuestInfoKey(),
        "rewardEx=0;" +
                "start=1;" +
                "summonCnt=0;"+
                this.rewardCount(character) +
                "coinCount=" + (getCoinCount(character) + amount)
        ));
    }
