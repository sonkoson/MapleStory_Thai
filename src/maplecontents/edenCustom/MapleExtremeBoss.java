        ...
        if (isExtremeMode) {
            if (mapCheck) {
                if (!alreadyCharacterHasExtremeBuff) {
                    ...
                    character.dropMessage(5, "익스트림 난이도 보스에 입장하여 입히는 데미지가 " + getDamReduceByMap(characterMapID) + "% 감소합니다.");
                }
            } else {
                character.setExtremeMode(false);
                if (alreadyCharacterHasExtremeBuff) character.cancelEffect(character.getBuffedEffect(damageReduceBuff));
