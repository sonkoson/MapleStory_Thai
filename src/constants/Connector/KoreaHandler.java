...
public class KoreaHandler {
    ...
    public static void LoginRequest(String[] msg, ChannelHandlerContext ctx) {
        ...
        try {
            ...
            if (rs.next()) {
                System.out.println("[알림] " + ctx.channel().remoteAddress().toString().split(":")[0] + " 에서 인게임 서버에 접속했습니다.");
                sendPacket(ctx, "로그인,성공");
            } else {
                System.out.println("[알림] " + ctx.channel().remoteAddress().toString().split(":")[0] + " 에서 인게임 서버에 접속에 실패했습니다.");
                sendPacket(ctx, "로그인,실패");
            }
    ...
    public static void GameStart(String[] header, ChannelHandlerContext ctx) {
        delLogin(header[1]);
        ServerConstants.cons.put(ctx.channel().remoteAddress().toString().split(":")[0], new Triple<>(header[1], header[2], ctx.channel().remoteAddress().toString().split(":")[0]));
        ...
        System.out.println("[알림] " + ctx.channel().remoteAddress().toString().split(":")[0] + " 에서 인게임에 접속했습니다.");
    }
