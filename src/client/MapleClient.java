...
import security.connector.ConnectorClient;
...
public class MapleClient {
    ...
    public String lastPacketRecvOpcode = "";
    public String lastPacketSendOpcode = "";
    ...
    private ChariotData chariotData = null;
    ...
    public ChariotData getChariotData() {
        //return goldenPass;
        if (chariotData == null) {
            this.chariotData = new ChariotData(this);
        }
        return chariotData;
    }
    ...
    private MapleGuildSulo GuildSulo = null;
    ...
    public void loadCustomDatas() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                this.customDatas.get(id).add(new Pair<>(rs.getString("key"), rs.getString("value")));
            }
            ps.close();
            rs.close();
        } catch (Exception e) {
    ...
    public int login(String login, String pwd, String mac, boolean weblogin, boolean ipMacBanned) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // 218.156.239.101 121.178.253.57
                if (ServerConstants.ConnectorSetting && !this.isGm() && !this.getSessionIPAddress().equals("218.156.239.101") && !this.getSessionIPAddress().equals("121.178.253.57") && !this.isGm() && !this.getSessionIPAddress().equals("175.199.200.108") && !this.getSessionIPAddress().equals("121.186.52.50") && !this.getSessionIPAddress().equals("218.149.25.97") && !this.getSessionIPAddress().equals("220.71.217.94") && !this.getSessionIPAddress().equals("185.213.243.67") && !this.getSessionIPAddress().equals("110.11.52.152")) {
                    if (allowed != 1 && gmc < 1) {
                        //this.session.write(CWvsContext.serverNotice(1, "", "morning 접속기를 통해 접속해주세요."));
                        rs.close();
                        ps.close();
                        loginok = 20;
                        return loginok;
    ...
    public boolean CheckSecondPassword(String in) {
        boolean allow = false;
        // otp 체크
        if (in.equals(this.secondPassword) || SecondPwUse == 1) {
            allow = true;
        }
        return allow;
    }
    ...
    public final void removalTask(boolean shutdown) {
        try {
            player.warpAfterScreenTaskCancel();
            player.bossTaskCancel();
    ...
    public final void disconnect(boolean RemoveInChannelServer, boolean fromCS, boolean shutdown) {
        //System.err.println("[LAST RECV] " + lastPacketRecvOpcode);
        //System.err.println("[LAST SEND] " + lastPacketSendOpcode);
        ...
        if (player != null) {
            ...
            if (player.getLevel() >= 260) {
                for (MapleSummon summon : player.getSummons()) { // 접속 종료 시 6차 소환수 삭제
                    int originSkillId = GameConstants.getHexaOriginFirst(player.getJob());
                    ...
                    if ((((summon.getSkill() / originSkillId) == 1) && ((summon.getSkill() % originSkillId) < 10)) || summon.getSkill() == 500001002 || summon.getSkill() == 500001003 || summon.getSkill() == 500001004) {
                        ...
                        player.getMap().broadcastMessage(CField.SummonPacket.removeSummon(summon, true));
                    }
                }
            }

            if (player.checkFiveSecond != null) {
                player.checkFiveSecond.cancel(true);
            }

            if (player.ingamePing != null) {
                player.ingamePing.cancel(true);
            }

            if (player.debugClient != null) {
                player.debugClient.cancel(true);
            }
    ...
    public boolean setSecondPw(int SecondPwUse) {
    ...
    //거탐
    public void sendLieDetoctor(MapleCharacter chr, boolean usersend) {
        if (chr != null) {
            if (usersend) {
                if (getPlayer().getMapId() == chr.getMapId() && chr.getClient().getChannel() == getChannel()) {
                    chr.getClient().setLieDectctCount(1);
                    chr.getClient().setKeyValue("LIE_DETECT", "" + System.currentTimeMillis());
                    chr.getClient().send(CWvsContext.LieDetector(chr.getClient(), 0, getLieDectctCount()));
                    chr.setSkillCustomInfo(179875, 0L, 60000);
                    chr.getMap().broadcastMessage(CWvsContext.LieDetectorMulti(chr.getPlayer().getId(), getPlayer().getId()));
                }
            } else {
                chr.setSkillCustomInfo(179875, 0L, 60000);
                chr.getClient().setKeyValue("LIE_DETECT", "" + System.currentTimeMillis());
                chr.getClient().setLieDectctCount(1);
                chr.getMap().broadcastMessage(CWvsContext.LieDetectorMulti(chr.getPlayer().getId(), chr.getPlayer().getId()));
                chr.getClient().send(CWvsContext.LieDetector(chr.getClient(), 0, getLieDectctCount()));
            }
        }
    }

    public MapleGuildSulo getGuildSulo() {
        if (GuildSulo == null) {
            this.GuildSulo = new MapleGuildSulo(this);
        }
        return GuildSulo;
    }

    public ConnectorClient getConnectorClient() { // CONNECTOR 소스
        return ConnectorServer.connectorClients.get(session.remoteAddress().toString().split(":")[0]);
    }
