...
public class Skill {
    ...
    public static final Skill loadFromData(final int id, final MapleData data, final MapleData delayData) {
        try {
            ...
            MapleData actions = data.getChildByPath("action");
            if (actions != null) {
                for (final MapleData dat : actions.getChildren()) {
                    ret.Skill6th = dat.getData().toString().startsWith("6th");
                }
            }
            ...
            if (level != null) {
                ...
                ret.ndTime = MapleDataTool.getInt("ndTime", level, 0);
                ret.psdDamR = MapleDataTool.getString("damR", level, "");
                ret.trueMax = ret.maxLevel + (ret.combatOrders ? 2 : 0);
                for (int i = 1; i <= ret.trueMax; ++i) {
                    ret.getEffects().add(SecondaryStatEffect.loadSkillEffectFromData(level, id, isBuff, i, "x")); // 여기
                }
            } else {
