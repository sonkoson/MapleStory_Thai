...
public class PlayerStats
            ...
            if (oldHp > this.hp && !chra.isAlive()) {
                if (!igskill) {
                    if (!chra.skillisCooling(1320016) && !chra.skillisCooling(1320019) && chra.getBuffedValue(1321020) && chra.getSkillLevel(1320016) > 0) {
                        ...
                        } else if (chra.getJob() == 1412) { //다크니스 어센션
                            chra.getClient().getSession().writeAndFlush((Object) CField.EffectPacket.showEffect(chra, 3000, 14110030, 63, 0, 0, (byte) (chra.isFacingLeft() ? 1 : 0), true, chra.getTruePosition(), null, null));
                            chra.getMap().broadcastMessage(chra, CField.EffectPacket.showEffect(chra, 3000, 14110030, 63, 0, 0, (byte) (chra.isFacingLeft() ? 1 : 0), false, chra.getTruePosition(), null, null), false);
                        }
                        this.hp = chra.getStat().getCurrentMaxHp() / 2L;
                        chra.updateSingleStat(MapleStat.HP, this.hp);
    ...
    public final long getCurrentMaxHp(MapleCharacter chr) {
        return this.localmaxhp;
    }
...
//        this.reduceCooltime = new ArrayList<Integer>();
    ...
    private Pair<Long, Long> handlePassiveSkills(MapleCharacter chra) {
        ...
        for (int j = 0; j < skill2; ++j) {
            ...
            if ((bof += chra.getTotalSkillLevel(skill3)) <= 0) {
                continue;
            }
            eff = bx.getEffect(bof);
            switch (skill3) {
        ...
        switch (chra.getJob()) {
            ...
            case 232: {
                ...
                if ((bof = chra.getTotalSkillLevel(bx = SkillFactory.getSkill(2320047))) <= 0) {
                    break;
                }
                this.ASR += 10;
                this.TER += 10;
...
//                }
            ...
            case 522: {
                ...
                if (GameConstants.isViper(chra.getJob())) {
                    ...
                    if (bof > 0) {
                        eff = bx.getEffect(bof);
                        if (chra.getBuffedEffect(SecondaryStat.UnkBuffStat707) != null) {
            ...
            case 1111:
            case 1112: {
                bx = SkillFactory.getSkill(11000021); //소울 가디언
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.def += eff.getMdef();
                    localmaxhp_ += (long) eff.getMhpX();
                }
            ...
            case 5112: {
                ...
                //othello
                bx = SkillFactory.getSkill(51120003); //어드밴스드 소울 실드
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                ...
                bx = SkillFactory.getSkill(50000001); //소울 실드
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    this.percent_hp += eff.getX();
    ...
    private Pair<Long, Long> handleBuffStats(MapleCharacter chra) {
        ...
        if (chra.getBuffedEffect(SecondaryStat.DarkSight) != null) {
            ...
            this.watk += chra.getBuffedEffect(SecondaryStat.UnkBuffStat707) != null ? effect.getWatk() : effect.getWatk() / 2;
        ...
        block145:
        for (Pair<SecondaryStat, SecondaryStatValueHolder> buffs : chra.getEffects()) {
            String buffname;
            switch (buffname = "" + buffs.getLeft()) {
                ...
                case "MesoUp": {
                    if (buffname.equals("MesoUp")) {
                        if (buffs.getRight().effect.getSourceId() == 8001535) {
                            break;
                        }
   ...
   public final void connectData(MaplePacketLittleEndianWriter mplew) {
    ...
    public Pair<Long, Long> handleInnerSkills(MapleCharacter chra) {
        long localmaxhp_ = 0L;
        long localmaxmp_ = 0L;
        for (InnerSkillValueHolder ISVH : chra.getInnerSkills().get(chra.getInnerSkillsPreset())) {
