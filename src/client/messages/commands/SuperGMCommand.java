...
public class SuperGMCommand {
    ...
    public static class 거짓말탐지기 extends CommandExecute {

        public int execute(MapleClient c, String[] splitted) {
            for (ChannelServer cserv : ChannelServer.getAllInstances()) {
                MapleCharacter player = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
                if (player != null) {
                    c.getPlayer().dropMessage(5, "[알림] " + splitted[1] + "에게 거짓말탐지기를 보냈습니다.");
                    c.sendLieDetoctor(player, false);
                } else if (player == null) {
                    c.getPlayer().dropMessage(5, "[알림] " + splitted[1] + "에게 거짓말탐지기를 보낼 수 없습니다.");
                } else {
                    c.getPlayer().dropMessage(5, "[알림] 알수 없는 오류");
                }
    ...
    public static class 몬스터소환개체 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "남아있는 몹 수 : " + map.getMapObjectsInRange(c.getPlayer().getPosition(), range, Arrays.asList(new MapleMapObjectType[]{MapleMapObjectType.MONSTER})).size());
            for (MapleMapObject monstermo : map.getMapObjectsInRange(c.getPlayer().getPosition(), range, Arrays.asList(new MapleMapObjectType[]{MapleMapObjectType.MONSTER}))) {
                MapleMonster mob = (MapleMonster) monstermo;
