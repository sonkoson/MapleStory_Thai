...
public class AvatarLook {
    ...
    public static final void encodeAvatarLook(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr, final boolean mega, boolean second, boolean update) {
        // -> 1.2.390 Changed.
        ...
        if (GameConstants.isAngelicBuster(chr.getJob()) && second) { // 엔젤릭 버스터 리마스터 드레스 업 구현
            Item default1 = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -1301);
            Item default2 = chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) -1307);
            
            if (default1 == null) {
                mplew.write(9);
                mplew.writeInt(1103663);
            }
        ...
        if (GameConstants.isKhali(chr.getJob()) || GameConstants.isAdel(chr.getJob())
                || GameConstants.isArk(chr.getJob()) || GameConstants.isIllium(chr.getJob()) || GameConstants.isMercedes(chr.getJob())) {
            mplew.writeInt(3);
        } else {
            mplew.writeInt(0);
        }
        
        mplew.writeInt(chr.getKeyValue(100229, "hue")); // 핑크빈 색 바꾸기
        mplew.write(0);
        mplew.writeInt(0); // 1 ?
        ...
        mplew.writeShort(second || isBeta ? chr.getSecondFace() : chr.getFace());
        mplew.writeShort(second || isBeta ? chr.getSecondSkinColor() : chr.getSkinColor());
    }
