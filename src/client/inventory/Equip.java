...
public class Equip
    ...
    private Map<String, String> enhanceKeyValues;
    ...
    public static Equip calculateEquipStats(Equip eq) {
        ...
        if (eq.getUpgradeSlots() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.SLOTS);
        }
        if (eq.getLevel() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.LEVEL);
        }
        if (eq.getFlag() != 0) {
            eq.getSpecialStats().add(EquipSpecialStat.FLAG);
        }
        if (eq.getIncSkill() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.INC_SKILL);
        }
        if (eq.getEquipLevel() > 0 || eq.getBaseLevel() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.ITEM_LEVEL);
        }
        if (eq.getItemEXP() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.ITEM_EXP);
        }
        if (eq.getDurability() > -1) {
            eq.getSpecialStats().add(EquipSpecialStat.DURABILITY);
        }
        if (eq.getViciousHammer() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.VICIOUS_HAMMER);
        }
        if (eq.getPVPDamage() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.PVP_DAMAGE);
        }
        if (eq.getEnchantBuff() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.ENHANCT_BUFF);
        }
        if (eq.getReqLevel() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.REQUIRED_LEVEL);
        } else if (eq.getReqLevel() < 0) {
            eq.getSpecialStats().add(EquipSpecialStat.DOWNLEVEL);
        }
        if (eq.getYggdrasilWisdom() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.YGGDRASIL_WISDOM);
        }
        if (eq.getFinalStrike()) {
            eq.getSpecialStats().add(EquipSpecialStat.FINAL_STRIKE);
        }
        if (eq.getBossDamage() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.IndieBdr);
        }
        if (eq.getIgnorePDR() > 0) {
            eq.getSpecialStats().add(EquipSpecialStat.IGNORE_PDR);
        }
    ...
    public short getTotalStr() {
        return (short) (this.str + this.enchantStr + this.starforceStr);
    }

    public short getTotalDex() {
        return (short) (this.dex + this.enchantDex + this.starforceDex);
    }

    public short getTotalInt() {
        return (short) (this._int + this.enchantInt + this.starforceInt);
    }

    public short getTotalLuk() {
        return (short) (this.luk + this.enchantLuk + this.starforceLuk);
    }
    ...
    public short getTotalWatk() {
        return (short) (this.watk + this.enchantWatk + this.starforceWatk);
    }

    public short getTotalMatk() {
        return (short) (this.matk + this.enchantMatk + this.starforceMatk);
    }
    ...
    public void setRandomStat() {
        ...
        if (equipStats != null) {
            for (Entry<String, Integer> equipStat : equipStats.entrySet()) {
                if (equipStat.getKey().equals("STR")) {
                    setStr(equipStat.getValue().shortValue());
                } else if (equipStat.getKey().equals("DEX")) {
                    setDex(equipStat.getValue().shortValue());
                } else if (equipStat.getKey().equals("INT")) {
                    setInt(equipStat.getValue().shortValue());
                } else if (equipStat.getKey().equals("LUK")) {
                    setLuk(equipStat.getValue().shortValue());
                } else if (equipStat.getKey().equals("PAD")) {
                    setWatk(equipStat.getValue().shortValue());
                } else if (equipStat.getKey().equals("MAD")) {
                    setMatk(equipStat.getValue().shortValue());
                }
            }
        }
        
        ArrayList<Pair<Integer, Integer>> randomStats = new ArrayList<>();

        for (int i = 0; i < 6; i++) {
            int random = Randomizer.nextInt(100) + 1;

            if (random >= 1 && random <= 60) {
                randomStats.add(new Pair<>(1, 10));
            } else if (random >= 61 && random <= 80) {
                randomStats.add(new Pair<>(11, 20));
            } else if (random >= 81 && random <= 90) {
                randomStats.add(new Pair<>(21, 30));
            } else if (random >= 91 && random <= 95) {
                randomStats.add(new Pair<>(31, 40));
            } else if (random >= 96 && random <= 99) {
                randomStats.add(new Pair<>(41, 50));
            } else {
                randomStats.add(new Pair<>(51, 100));
            }
        }
        ...
        int count = 0;
        for (Pair<Integer, Integer> randomStat : randomStats) {
            if (count == 0) {
                int randomStr = Randomizer.rand(randomStat.left, randomStat.right);

                if (highestStat < randomStr) {
                    highestStat = randomStr;
                }

                setStr((short) (getStr() + randomStr));
            }
            if (count == 1) {
                int randomDex = Randomizer.rand(randomStat.left, randomStat.right);

                if (highestStat < randomDex) {
                    highestStat = randomDex;
                }

                setDex((short) (getDex() + randomDex));
            }
            if (count == 2) {
                int randomInt = Randomizer.rand(randomStat.left, randomStat.right);

                if (highestStat < randomInt) {
                    highestStat = randomInt;
                }

                setInt((short) (getInt() + randomInt));
            }
            if (count == 3) {
                int randomLuk = Randomizer.rand(randomStat.left, randomStat.right);

                if (highestStat < randomLuk) {
                    highestStat = randomLuk;
                }

                setLuk((short) (getLuk() + randomLuk));
            }
            if (count == 4) {
                int randomWatk = Randomizer.rand(randomStat.left, randomStat.right);

                if (highestStat < randomWatk) {
                    highestStat = randomWatk;
                }

                setWatk((short) (getWatk() + randomWatk));
            }
            if (count == 5) {
                int randomMatk = Randomizer.rand(randomStat.left, randomStat.right);

                if (highestStat < randomMatk) {
                    highestStat = randomMatk;
                }

                setMatk((short) (getMatk() + randomMatk));
            }

            count++;
        }

        if (highestStat >= 1 && highestStat <= 19) {
            setOwner("최하급");
        } else if (highestStat >= 20 && highestStat <= 39) {
            setOwner("하급");
        } else if (highestStat >= 40 && highestStat <= 59) {
            setOwner("중급");
        } else if (highestStat >= 60 && highestStat <= 79) {
            setOwner("상급");
        } else if (highestStat >= 80 && highestStat <= 99) {
            setOwner("최상급");
        } else {
            setOwner("유일");
        }
