...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable {
    ...
    public long Flame_Sweep_Damage = 0;
    ...
    private ArrayList<MapleUnionArtipact> artipacts;
    public ScheduledFuture<?> checkFiveSecond;
    public ScheduledFuture<?> ingamePing;
    public ScheduledFuture<?> debugClient;
    public ScheduledFuture<?> checkFiveMinute;
    private Point Resolution;
    private long rainbowrushStartTime;
    private int RainBowRushTime;
    public int attackCountBat;
    ...
    public int 홀리워터, 홀리워터스택, 서펜트스톤, 서펜트스크류, 수룡의의지;
    public int 발할라검격;
    public int 리인카네이션;
    public int 플레시미라주스택, 스킬카운트, addBuffCheck, ignitionstack;
    public boolean burnningcheck = false;
    //칼리 스택
    public int ChakriSize;
    public long lastAstraTime;
    public long cannonstack;
    private int battleShipBoomberBefore;
    ...
    public int repeatingCrossbowCatridgestack = 0;
    ...
    //[othello] - 무릉도장랭킹 불러오기
    public int dojorank;
    private int hair, basecolor, addcolor, baseprob, secondbasecolor, secondaddcolor, secondbaseprob, secondhair, face, secondface, demonMarking, mapid, getCharacterValue,
            ...
            arcaneAim, listonation, elementalCharge, lastElementalCharge, reinCarnation, transformCooldown;
    ...
    private List<List<InnerSkillValueHolder>> innerSkills;
    ...
    public ArrayList<MapleMagicWreck> timePieces = new ArrayList<>();
    public ScheduledFuture<?> hexaSkillTimer;
    public int deathKillCount = 0;
    public int unionAuraBossAttackCount = 0;
    public int unionAuraKillCount = 0;
    public int hexaMasterySkillStack = 0;
    public int hexaMasterySkillStack2 = 0;
    public transient ScheduledFuture<?> warpAfterScreenTask;
    public transient ScheduledFuture<?> bossTask;
    ...
    private MapleCharacter(final boolean ChannelServer, final boolean firstIngame) {
        ...
        this.innerSkills.add(new LinkedList<InnerSkillValueHolder>());
        this.innerSkills.add(new LinkedList<InnerSkillValueHolder>());
        this.innerSkills.add(new LinkedList<InnerSkillValueHolder>());
        this.artipacts = new ArrayList<>();
    ...
    public void handleAdditionalSkills(long time) {
        ...
        if (this.getSkillLevel(22170074) > 0) {
            ...
            if (this.getStat().getMPPercent() >= dragonFury.getX() && this.getStat().getMPPercent() <= dragonFury.getY()) {
                if (!this.getBuffedValue(22170074)) {
                    ...
                    this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups2, dragonFury, this));
        ...
        if (this.getBuffedValue(400001043) && time - this.lastCygnusTime >= 4000L) {
            ...
            if (value < eff.getW()) {
                ...
                this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups3, eff, this));
            }
        }
        if (this.getBuffedValue(400001044) && time - this.lastCygnusTime >= 4000L) {
            ...
            if (value < eff.getW()) {
                ...
                this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups4, eff, this));
            }
        }
        ...
        if (this.getSkillLevel(2100009) > 0) {
            ...
            for (MapleMonster mob : this.map.getAllMonstersThreadsafe()) {
                for (Ignition zz : mob.getIgnitions()) {
                    if (zz.getOwnerId() == this.getId() && (stack < 5 || (this.getBuffedValue(2141502) && stack < 10))) {
                        stack = (byte) (stack + 1);
                    }
                    if (stack != 5 || (this.getBuffedValue(2141502) && stack != 10)) {
                        continue;
                    }
                    break;
                }
            ...
            drain.setDuration(0);
            if (stack > 0) {
                this.client.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups5, drain, this));
            } else {
        ...
        if (this.getBuffedValue(SecondaryStat.RandAreaAttack) != null && this.getSkillCustomValue(80001752) == null) {
            ...
            if (objs != null && objs.size() >= 1) {
                ...
                this.getMap().broadcastMessage(CField.thunderAttack(monsters));
                this.setSkillCustomInfo(80001752, 0L, 5000L);
            }
        }
        if (GameConstants.isWildHunter(this.getJob())) {
            ...
            if (this.getSkillCustomValue0(37000006) <= (long) 감소량) {
                ...
                this.client.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups7, this.getBuffedEffect(37000006), this.getPlayer()));
            }
        ...
        if (GameConstants.isCain(this.getJob())) {
            ...
            if (this.getSkillLevel(162101012) > 0) {
                ...
                if (this.getSkillCustomValue(162101112) == null) {
                    ...
                    this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups8, effect, this));
                }
            }
    ...
    public void checkMistStatus(final MapleMist mist, final List<MapleMonster> arrays, final long time) {
        if (mist.getSource() != null) {
            switch (mist.getSource().getSourceId()) {
                ...
                case 12121005: { //버닝 리젼
                    for (final MapleCharacter chr : this.getMap().getAllChracater()) {
                        if (mist.getBox().contains(chr.getTruePosition()) || mist.getOwner().getId() == chr.getId()) {
                            if (mist.getOwner().getId() == chr.getId()) {
                ...
                case 162111003: {
                    ...
                    for (final MapleCharacter chr2 : this.getMap().getAllChracater()) {
                        ...
                        if (mist.getBox().contains(chr2.getTruePosition())) {
                            ...
                            chr2.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr2, 0, 162111003, 10, 0, 0, (byte) (chr2.isFacingLeft() ? 1 : 0), true, chr2.getTruePosition(), null, null));
    ...
    public void handleHealSkills(final long time) {
        ...
        if (this.getSkillLevel(5100013) > 0) {
            ...
            if (time - this.lastHealTime >= willofSteal.getW() * 1000) {
                ...
                long fuckinCalc = this.getStat().getCurrentMaxHp() * willofSteal.getY() / 100L;
                ...
                if (this.getParty() != null) {
                    for (final MaplePartyCharacter chr1 : this.getParty().getMembers()) {
                        ...
                        if (curChar != null && curChar.getBuffedValue(activeSkillid)) {
                            ...
                            curChar.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(curChar, 0, 32120062, 4, 0, 0, (byte) (curChar.isFacingLeft() ? 1 : 0), true, curChar.getTruePosition(), null, null));
                    ...
                    if (!statupz2.isEmpty()) {
                        this.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(this, 0, 32120062, 4, 0, 0, (byte) (this.isFacingLeft() ? 1 : 0), true, this.getTruePosition(), null, null));
        ...
        if (!stats.isEmpty()) {
            for (final Map.Entry<SecondaryStatEffect, List<SecondaryStat>> stat2 : stats.entrySet()) {
                if (this.client.getChannelServer() != null && this.client.getChannelServer().getPlayerStorage().getCharacterById(this.id) != null) {
                    if (stat2.getKey().getSourceId() == 15110025) { // 연쇄 구현
                        this.hexaMasterySkillStack = 0;
                    }
    ...
    public static final MapleCharacter ReconstructChr(final CharacterTransfer ct, final MapleClient client, final boolean isChannel) {
        ...
        ret.getCharacterValue = ct.getCharacterValue;
        ...
        ret.innerSkills = (List<List<InnerSkillValueHolder>>) ct.innerSkills;
        ret.artipacts = (ArrayList<MapleUnionArtipact>) ct.artipacts;
        ...
        try {
            ...
            if (channelserver) {
                ...
                while (rs.next()) {
                    ...
                    rsMobs = pse.executeQuery(); //CPU렉 1위
                ...
                ps = con.prepareStatement("SELECT skill_id, skill_level, max_level, rank, preset FROM inner_ability_skills WHERE player_id = ?");
                ps.setInt(1, charid);
                rs = ps.executeQuery();
                while (rs.next()) {
                    int preset = rs.getByte("preset");

                    ret.innerSkills.get(preset).add(new InnerSkillValueHolder(rs.getInt("skill_id"), rs.getByte("skill_level"), rs.getByte("max_level"), rs.getByte("rank"), (byte) preset));
                }
                ps.close();
                rs.close();
    ...
    public boolean saveCharToDB(final Connection con) {
        synchronized (this) {
            try {
                ...
                if (this.getMapId() == ServerConstants.startMap) {
                    ps.setInt(28, this.getMapId());
                } else {
    ...
    public void saveInnerToDB(final Connection con) {
        try {
            if (this.innerSkills != null) {
                ...
                final PreparedStatement ps = con.prepareStatement("INSERT INTO inner_ability_skills (player_id, skill_id, skill_level, max_level, rank, preset) VALUES (?, ?, ?, ?, ?, ?)");
                ps.setInt(1, this.id);

                for (int i = 0; i < this.innerSkills.size(); i++) {
                    for (int j = 0; j < this.innerSkills.get(i).size(); j++) {
                        ps.setInt(2, this.innerSkills.get(i).get(j).getSkillId());
                        ps.setInt(3, this.innerSkills.get(i).get(j).getSkillLevel());
                        ps.setInt(4, this.innerSkills.get(i).get(j).getMaxLevel());
                        ps.setInt(5, this.innerSkills.get(i).get(j).getRank());
                        ps.setInt(6, this.innerSkills.get(i).get(j).getPreset());
                        ps.execute();
                    }
                }
                ps.close();
    ...
    public void saveKeyValueToDB(final Connection con) {
        ...
        try {
            ...
            ps = con.prepareStatement("INSERT INTO keyvalue (`id`, `key`, `value`) VALUES (?, ?, ?)");
            ps.setInt(1, this.id);
    ...
    public void saveToDB(final boolean dc, final boolean fromcs) {
        ...
        try {
            ...
            if (this.getArtipacts() != null) {
                this.saveArtipact(con);
            }
    ...
    public Map<SecondaryStat, Pair<Integer, Integer>> autoDeregisterBuffStats(int sourceId, boolean overwrite) {
        ...
        } else if (sourceId == 400051015) {
            // this.dropMessageGM(6, "서펜트 스탯 0");
            this.서펜트스크류 = 0;
        }
...
//                }
    ...
    public void cancelEffect(final SecondaryStatEffect effect, final List<SecondaryStat> stats, final boolean overwrite, final boolean auto) {
        if (hasDonationSkill(4341002) && effect.getSourceId() == 4341002) {
            return;
        }
        if (effect == null) {
            return;
        ...
        if (effect.getSourceId() == 400001010) {
            ...
            this.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(400001011, mobList, true, 0, new int[0]));
        }
        if (effect.getSourceId() == 80002633) {
            final List<Triple<Integer, Integer, Integer>> mobList = new ArrayList<Triple<Integer, Integer, Integer>>();
            this.getClient().getSession().writeAndFlush(CField.bonusAttackRequest(80002634, mobList, true, 0, new int[0]));
        }
        ...
        if (statups.containsKey(SecondaryStat.RideVehicle) && this.getSkillCustomValue(this.getMapId()) != null && this.getMapId() == ServerConstants.warpMap) {
            this.getClient().getSession().writeAndFlush(SLFCGPacket.BlackLabel("#fn나눔고딕 ExtraBold##fs32##b#e잠수포인트 적립을 그만둡니다.", 100, 1000, 4, 0, 0, 1, 4));
        ...
        switch (effect.getSourceId()) {
            ...
            case 61121053: {
                if (this.getBuffedValue(SecondaryStat.StopForceAtominfo) != null) {
                    int[] skilllist;
                    int[] sa = skilllist = new int[]{61101002, 61110211, 61120007, 61121217, 61141006, 61141007}; // 윌 오브 소드 VI 구현
                    int n = sa.length;
                    for (int i = 0; i < n; ++i) {
                        Integer skill = sa[i];
                    ...
                    if (this.getSkillLevel(61141006) > 0) { // 윌 오브 소드 VI 구현
                        SkillFactory.getSkill(61141006).getEffect(this.getSkillLevel(61141006)).applyTo(this);
                    } else if (effect.getSourceId() == 61111008) {
            ...
            case 5221027:
            case 5221028:
            case 5241002: // 배틀쉽 봄버 VI 구현
            case 5241003: // 배틀쉽 봄버 VI 구현
            case 12120013:
            case 12120014:
            case 14111024:
            case 35111002:
        ...
        if (!overwrite) {
            ...
            boolean isHexa = this.getSkillLevel(51141002) > 0;
            if (effect.getSourceId() == (isHexa ? 51141002 : 51001005) && !overwrite) {
                boolean givebuff = false;
                --this.royalStack;
                if (this.royalStack > 0) {
                    givebuff = true;
                    SkillFactory.getSkill(isHexa ? 51141002 : 51001005).getEffect(this.getSkillLevel(isHexa ? 51141002 : 51001005)).applyTo(this, false, SkillFactory.getSkill(isHexa ? 51141002 : 51001005).getEffect(this.getSkillLevel(isHexa ? 51141002 : 51001005)).getX() * 1000);
                }
                if (this.getParty() != null) {
                    for (final MaplePartyCharacter chr1 : this.getParty().getMembers()) {
                        final MapleCharacter curChar = this.getClient().getChannelServer().getPlayerStorage().getCharacterById(chr1.getId());
            ...
            if (effect.getSourceId() == 14120009) { // 캔슬
                this.siphonVitality = 0;
            }
        ...
        // 6차 구현
        switch (effect.getSourceId()) {
            case 4361501: // 오리진 카르마 블레이드 구현
                ArrayList<RangeAttack> skills = new ArrayList<RangeAttack>();
                skills.add(new RangeAttack(4361502, this.getPosition(), this.isFacingLeft() ? 0 : 1, 0, 0));
                this.getClient().getSession().writeAndFlush(CField.rangeAttack(4361502, skills));
                break;
        }
        ...
        if (effect.getSourceId() == 20040219) {
        ...
        } else if (effect.getSourceId() == 63141503) { // 오리진 어나일레이션 구현
            ...
            for (MapleMonster mob : this.getMap().getAllMonster()) {
                if (mob.getCustomValue0(63110011) > 0) {
                    if (mob.getCustomValue0(63110011) > 15) {
                        mob.setCustomInfo(63110011, 15, 90000);
                    }
            ...
            this.getClient().getSession().writeAndFlush(CField.getDeathBlessStack(this, moblist));
        }
    }

    public long getSuloWeekPoint() {
        try {
            return this.getKeyValue(221121, "SuloWeekPoint");
        } catch (Exception e) {
            return 0L;
        }
    }

    public void gainSuloWeekPoint(int point) {
        this.setKeyValue(221121, "SuloWeekPoint", String.valueOf(this.getKeyValue(221121, "SuloWeekPoint") + point));
    }

    public void setSuloWeekPoint(int point) {
        this.setKeyValue(221121, "SuloWeekPoint", String.valueOf(point));
    }

    public long getSuloPoint() {
        try {
            return this.getKeyValue(221121, "SuloPoint");
        } catch (Exception e) {
            return 0L;
        }
    }

    public void gainSuloPoint(int point) {
        this.setKeyValue(221121, "SuloPoint", String.valueOf(this.getKeyValue(221121, "SuloPoint") + point));
    }

    public void setSuloPoint(int point) {
        this.setKeyValue(221121, "SuloPoint", String.valueOf(point));
    }
    ...
    public void setGuildScore(int id, int score) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("UPDATE guilds SET score = ? WHERE guildid = ?");
            ps.setInt(1, score);
            ps.setInt(2, id);
            ps.executeUpdate();
            ps.close();
    ...
    public long getRBPoint() {
        try {
            return Long.parseLong(this.client.getKeyValue("Reborn"));
        } catch (Exception e) {
            return 0L;
        }
    }

    public void gainDPoint(long point) {
        this.client.setKeyValue("DPoint", String.valueOf(this.getDPoint() + point));
        this.setKeyValue(14434, "point", client.getKeyValue("DPoint"));
    }

    public void gainHPoint(long point) {
        this.client.setKeyValue("HPoint", String.valueOf(this.getHPoint() + point));
        this.setKeyValue(42003, "point", client.getKeyValue("HPoint"));
    }

    public void setDPoint(long point) {
        this.client.setKeyValue("DPoint", String.valueOf(point));
        this.setKeyValue(14434, "point", String.valueOf(point));
    }

    public void setHPoint(long point) {
        this.client.setKeyValue("HPoint", String.valueOf(point));
        this.setKeyValue(42003, "point", String.valueOf(point));
    }

    public int getBattleship() {
        return battleship123;
    }

    public void setBattleship(int dd) {
        this.battleship123 = dd;
    }
    ...
    public int getCharacterValue(int job) {
    ...
    public void changeMap(final int Mapid, final Point pos, boolean isSoftChangeOut) {
        final MapleMap to = this.getClient().getChannelServer().getMapFactory().getMap(Mapid);
        this.changeMapInternal(to, pos, CField.getWarpToMap(to, 129, isSoftChangeOut, this), null, false);
    }

    public void softChangeMap(final int Mapid) {
        final MapleMap to = this.getClient().getChannelServer().getMapFactory().getMap(Mapid);
        final MaplePortal pto = to.getPortal(0);
        this.changeMapInternal(to, pto.getPosition(), CField.getWarpToMap(to, pto.getId(), true, this), null, false);
    }

    public void changeMap(final int Mapid, final int portalid) {
        final MapleMap to = this.getClient().getChannelServer().getMapFactory().getMap(Mapid);
        final MaplePortal pto = to.getPortal(portalid);
        this.changeMapInternal(to, pto.getPosition(), CField.getWarpToMap(to, pto.getId(), false, this), null, false);
    }

    public void changeMap(final MapleMap to, final Point pos) {
        this.changeMapInternal(to, pos, CField.getWarpToMap(to, 129, false, this), null, false);
    }

    public void changeMap(final MapleMap to, final MaplePortal maplePortal, final boolean banish) {
        this.changeMapInternal(to, to.getPortal(0).getPosition(), CField.getWarpToMap(to, 0, false, this), to.getPortal(0), banish);
    }
    ...
    public void changeMapPortal(final MapleMap to, final MaplePortal pto) {
        this.changeMapInternal(to, pto.getPosition(), CField.getWarpToMap(to, pto.getId(), false, this), pto, false);
    }
    ...
    public void gainExpMonster(final long gain, final boolean show, final boolean white) {
        ...
        switch (currentDayOfWeek) {
            case MONDAY:
                rainbowWeekExp = (int) (2.5 * gain);
                break;
            case TUESDAY:
                rainbowWeekExp = (int) (1.0 * gain);
                break;
            case WEDNESDAY:
                rainbowWeekExp = (int) (1.5 * gain);
                break;
            case THURSDAY:
                rainbowWeekExp = (int) (1.0 * gain);
                break;
            case FRIDAY:
                rainbowWeekExp = (int) (2.0 * gain);
                break;
            case SATURDAY:
                rainbowWeekExp = (int) (3.0 * gain);
                break;
            case SUNDAY:
                rainbowWeekExp = (int) (4.0 * gain);
                break;

        }
        if (burnningcheck) {
            int rate = 3;
            rainbowWeekExp = (int) (rate * gain);
        }
        ...
        if (this.getInventory(MapleInventoryType.EQUIPPED).getItem((short) (-21)) != null) {
            ...
            if (itemid >= 1143809 && itemid <= 1143820) {
                final int exp = 5 * (itemid - 1143809 + 1);
                itemEquipExp += (int) (gain * (exp / 100.0));
            }
            if (itemid >= 1143821 && itemid <= 1143823) {
                final int exp = 5 * (itemid - 1143820 + 6);
                itemEquipExp += (int) (gain * (exp / 100.0));
            }
        }
        // 훈장 소지시 추가 경험치
        if (this.hasEquipped(1149994) || this.hasEquipped(1149995) || this.hasEquipped(1149996) || this.hasEquipped(1149997) || this.hasEquipped(1149989) || this.hasEquipped(1149998) || this.hasEquipped(1149999)) {
            itemEquipExp += (long) (200 / 100.0 * gain);
        }
        ...
        //추가 경험치
        long bonusExp = (int) (this.getStat().expBuff / 100.0 * gain);
        if (getKeyValue(9919, "ExpTear") > 0) {
            int tear = (int) getKeyValue(9919, "ExpTear");
            int tearexp = tear == 8 ? 184 : tear == 7 ? 124 : tear == 6 ? 84 : tear == 5 ? 54 : tear == 4 ? 31 : tear == 3 ? 23 : tear == 2 ? 10 : 3;
            bonusExp += (long) (tearexp / 100.0 * gain);
        }
        if (this.getClient().getChannelServer().버닝타임()) {
            bonusExp += (long) (100 / 100.0 * gain);
        }
        ...
        if (burningExp != null && burningExp.left > 0) { // 임시 처리
            //flag += 4194304;
    ...
    public void gainMeso(final long gain, final boolean show, final boolean inChat, final boolean isPet, final boolean monster) {
        ...
        if (inChat) {
            this.client.getSession().writeAndFlush(CWvsContext.InfoPacket.showMesoGain(gain, inChat, jan));
        }
    }
    ...
    public void levelUp() {
        ...
        if (this.level >= 200) {
            if (this.getQuestStatus(1465) != 2) {
                ...
                if (!GameConstants.isPinkBean(this.job) && !GameConstants.isYeti(this.job)) {
                    MatrixHandler.gainMatrix(this, true);
        ...
        if (this.level >= 260) {
            if (this.getQuestStatus(1488) != 2) {
                if (!GameConstants.isPinkBean(this.job) && !GameConstants.isYeti(this.job)) {
                    this.forceCompleteQuest(1488);

                    this.changeSkillLevel(GameConstants.getHexaOriginFirst(this.getJob()), (byte) 1, (byte) 1);
                }
            }

            this.getClient().getSession().writeAndFlush(CField.activeHexaStat(this.getClient()));
            this.getClient().getSession().writeAndFlush(CField.updateHexaCore(this.getClient()));
            this.getClient().getSession().writeAndFlush(CField.activeHexaSkill());
        }

        if (this.getLevel() >= 275) {
            if (this.getQuestStatus(38404) != 2) {
                this.forceCompleteQuest(38404);
            }
        }

        if (this.level == 200) { // 5차 스킬 지급
            //if (isGM()) {
            MatrixHandler.gainMatrix(this, true);
            MatrixHandler.gainVCoreLevel(this);
            //}
        ...
        if (this.level >= 250 && this.level % 25 == 0 && !this.isGM()) {
            /*final StringBuilder sb = new StringBuilder("5차 스킬 지급 ");
        ...
        if (this.getAutoJob() != null) {
            if (this.level == 20) {
                ...
                switch (autoJob2) {
                    ...
                    case "15410": {
                        this.dropMessage(-1, "[복수의 칼날] 칼리로 전직하였습니다.");
    ...
    public final boolean ban(final String reason, final boolean IPMac, final boolean autoban, final boolean hellban) {
        ...
        Connection con = null;
        PreparedStatement ps = null, psa = null, pss = null;
    ...
    public static boolean ban(final String id, final String reason, final boolean accountId, final int gmlevel, final boolean hellban) {
        Connection con = null;
        PreparedStatement ps = null, psb = null, psa = null, psz = null, pss = null;
        ResultSet rs = null, rsa = null;
        try {
            con = DatabaseConnection.getConnection();
    ...
    public void cancelDisease(final Map<SecondaryStat, Pair<Integer, Integer>> statups, final boolean autocancel) {
        ...
        if (autocancel) {
            if (cancelList.containsKey(SecondaryStat.PapulCuss)) {
                for (final MapleMonster monster : this.getMap().getAllMonster()) { //[othello] - 익스트림 보스
                    if (monster != null && (monster.getId() == 8500021 || monster.getId() == 8500031 || monster.getId() == 8500011 || monster.getId() == 8500001) && monster.getCustomValue(2412) != null && monster.getBuff(MonsterStatus.MS_PopulatusTimer) != null) {
                ...
                for (final MapleMonster monster2 : this.getMap().getAllMonster()) {
                    //[othello] - 익스트림 보스
                    if (monster2 != null && (monster2.getId() == 8880142 || monster2.getId() == 8880140 || monster2.getId() == 8880141 || monster2.getId() == 8880150 || monster2.getId() == 8880151 || monster2.getId() == 8880155 || monster2.getId() == 8500001 || monster2.getId() == 8500002 || monster2.getId() == 8500011 || monster2.getId() == 8500012 || monster2.getId() == 8500021 || monster2.getId() == 8500022 || monster2.getId() == 8500031 || monster2.getId() == 8500032)) {
                        mob = monster2;
                        break;
                    }
                }
                if (mob != null && skillid > 0 && skilllv > 0) {
                    ...
                    this.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(this, (skillid == 238) ? 176 : skillid, (skillid == 238) ? 38 : skilllv, 45, 0, 0, (byte) 0, true, null, null, null));
    ...
    public void dropMessage(final int type, final String message) {
        if (type == -1) {
            ...
            this.client.getSession().writeAndFlush(CField.getChatText(this, message, this.isSuperGM(), 0, null, -1, -1));
        } else if (type == -4) {
            this.client.getSession().writeAndFlush(CField.getChatText(this, message, this.isSuperGM(), 1, null, -1, -1));
        } else if (type == -5) {
    ...
    public void changeChannel(final int channel) {
        ...
        addCoolTime(400001059, 30000);
        addCoolTime(400001039, 30000);
    ...
    public void updateLinkSkillPacket() {
        ...
        for (final Triple<Skill, SkillEntry, Integer> linkskil : skills) {
            if (this.getSkillLevel(linkskil.getLeft().getId()) != 0) {
                ...
                this.getClient().getSession().writeAndFlush(CWvsContext.Linkskill(linkskil.getLeft().getId(), linkskil.getRight(), this.getId(), linkskil.getMid().skillevel, totalskilllv));
            }
        }
    }
    ...
    public void setIgnition(final int stack) {
        ...
        final SecondaryStatEffect effect = SkillFactory.getSkill(400021045).getEffect(this.getTotalSkillLevel(400021045));
        final EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
        statups.put(SecondaryStat.Ember, new Pair<Integer, Integer>(stack, (int) this.getBuffLimit(400021042)));
        this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, effect, this));
    }
    ...
    public void gainIgnition() {
        ...
        final SecondaryStatEffect effect = SkillFactory.getSkill(400021042).getEffect(this.getTotalSkillLevel(400021042));
        final EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
        statups.put(SecondaryStat.Ember, new Pair<Integer, Integer>(effect.getX(), (int) this.getBuffLimit(400021042)));
        this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, effect, this));
    }
    ...
    public void gainfamilier() {
        ...
        final SecondaryStatEffect effect = SkillFactory.getSkill(400021042).getEffect(this.getTotalSkillLevel(400021042));
        final EnumMap<SecondaryStat, Pair<Integer, Integer>> statups = new EnumMap<SecondaryStat, Pair<Integer, Integer>>(SecondaryStat.class);
        statups.put(SecondaryStat.Ember, new Pair<Integer, Integer>(1, (int) this.getBuffLimit(400021042)));
        this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, effect, this));
    }
    ...
    public List<MapleLinkSkillPreset> loadLinkSkillPreset(final int pos) { // [SEX] 링크프리셋 수정
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM linkpreset WHERE charid = ? AND preset = ?");
            ps.setInt(1, this.getId());
            ps.setInt(2, pos);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public MapleLinkSkillPreset addLinkSkillPreset(final int position, final int skillid, final int skillchar) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("INSERT INTO linkpreset (charid, skillid, skillchar, preset) VALUES (?, ?, ?, ?)");
            ps.setInt(1, this.getId());
            ps.setInt(2, skillid);
    ...
    public void resetLinkSkillPreset(final int preset) {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("DELETE FROM linkpreset WHERE charid = ? AND preset = ?");
            ps.setInt(1, this.id);
            ps.setInt(2, preset);
            ps.execute();
            ps.close();
    ...
    public void resetCodyPreset(final int preset) {
        Connection con = null;
        PreparedStatement ps = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("DELETE FROM Codypreset WHERE charid = ? AND preset = ?");
            ps.setInt(1, this.id);
            ps.setInt(2, preset);
            ps.execute();
            ps.close();
    ...
    public void handleForceGain(final int oid, final int skillid, int extraForce) {
        int forceGain = 0;
        if (GameConstants.isDemonSlash(skillid) || (skillid >= 400011007 && skillid <= 400011009)) {
        ...
        if (skillid == 30010111) {
            ...
            forceGain = 50;
            extraForce = 1;
        } else if (skillid == 31141502) { // 오리진 나이트메어 구현
            forceGain = 10;
        }
    ...
    public List<List<InnerSkillValueHolder>> getInnerSkills() {
        return this.innerSkills;
    }

    public int getInnerSkillsPreset() {
        int preset = (int) this.getKeyValue(7777777, "inner_preset");

        if (preset == -1) {
            this.setInnerSkillsPreset(0);

            return (int) this.getKeyValue(7777777, "inner_preset");
        }

        return preset;
    }

    public void setInnerSkillsPreset(int preset) {
        this.setKeyValue(7777777, "inner_preset", preset + "");
    }

    public int getArtipactExp() {
        int exp = (int) this.getKeyValue(503392, "exp");

        if (exp == -1) {
            this.setArtipactExp(0);

            return (int) this.getKeyValue(503392, "exp");
        }

        return exp;
    }

    public void setArtipactExp(int exp) {
        this.setKeyValue(503392, "exp", exp + "");
    }

    public void gainArtipactExp(int exp, boolean isQuest) {
        int resultExp = (this.getArtipactExp() + exp);
        int maxExp = MapleUnionArtipact.settings[(this.getArtipactLevel() - 1)][1];

        if (this.getArtipactLevel() >= 60) {
            return;
        }
        ...
        if (resultExp >= maxExp) {
            ...
            this.artipactLevelUp();
        }

        if (this.getArtipactLevel() >= 60) {
            this.setKeyValue(503392, "exp", 0 + "");
        } else {
            this.setKeyValue(503392, "exp", resultExp + "");
        }

        this.getClient().getSession().writeAndFlush(CField.unionArtipactMsg(this, exp, 0, false));

        if (isQuest) {
            this.getClient().getSession().writeAndFlush(CField.unionArtipactQuestMsg(1, exp, 0, 0, 0));
        }
    }

    public int getArtipactPoint() {
        int point = (int) this.getKeyValue(503392, "point");

        if (point == -1) {
            this.setArtipactPoint(0);

            return (int) this.getKeyValue(503392, "point");
        }

        return point;
    }

    public void setArtipactPoint(int point) {
        this.setKeyValue(503392, "point", point + "");
    }

    public void gainArtipactPoint(int point, boolean isQuest) {
        int resultPoint = (this.getArtipactPoint() + point);
        int maxPoint = MapleUnionArtipact.settings[(this.getArtipactLevel() - 1)][0];

        if (resultPoint >= maxPoint) {
            resultPoint = maxPoint;
        }

        this.setKeyValue(503392, "point", resultPoint + "");

        this.getClient().getSession().writeAndFlush(CField.unionArtipactMsg(this, 0, point, false));

        if (isQuest) {
            this.getClient().getSession().writeAndFlush(CField.unionArtipactQuestMsg(1, 0, point, 0, 0));
        }
    }

    public int getArtipactLevel() {
        int level = (int) this.getKeyValue(503392, "level");

        if (level == -1) {
            this.setArtipactLevel(0);

            return (int) this.getKeyValue(503392, "level");
        }

        return level;
    }

    public void setArtipactLevel(int level) {
        this.setKeyValue(503392, "level", level + "");
    }

    public void artipactLevelUp() {
        int resultLevel = (this.getArtipactLevel() + 1);

        if (resultLevel > 60) {
            return;
        }

        this.setKeyValue(503392, "level", resultLevel + "");
        ...
        this.getClient().getSession().writeAndFlush(CField.unionArtipactMsg(this, 0, 0, true));
    }

    public void artipactInit() {
        this.addArtipact(0);
        this.addArtipact(1);
        this.addArtipact(2);

        MapleQuest.getInstance(503149).forceComplete(this, 9010106);

        this.setKeyValue(503392, "start", 1 + "");

        SimpleDateFormat format = new SimpleDateFormat("yy/MM/dd/HH/mm");
        String now = format.format(System.currentTimeMillis());

        this.updateInfoQuest(503155, "attendance=4;mobCount=20000;resetDate=" + now + ";attendance_lastDate=" + now); // 주간 접속, 레벨 범위 몬스터 처치, 초기화 일자, 마지막 주간 접속 일자

        this.updateInfoQuest(503144, "missionState=22222222"); // 일반 미션
        this.updateInfoQuest(503153, "missionState=00000000000000000000000000000000000000000"); // 보스 미션
        this.updateInfoQuest(503135, "missionState=2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222"); // 스페셜 미션 1
        this.updateInfoQuest(503136, "missionState=2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222"); // 스페셜 미션 2

        this.setArtipactExp(100);
        this.setArtipactPoint(100);
        this.setArtipactLevel(1);

        this.artipactReload(7, 0);
    }

    public void artipactReload(int type, int index) {
        for (int skill : MapleUnionArtipact.skillStorage) {
            this.changeSkillLevel(skill, (byte) 0, (byte) 10);
        }

        for (MapleUnionArtipact artipact : this.getArtipacts()) {
            for (int skill : artipact.getSkills()) {
                this.changeSkillLevel(skill, (byte) (this.getSkillLevel(skill) + artipact.getLevel()), (byte) 10);
            }
        }

        this.getClient().getSession().writeAndFlush(CField.unionArtipact(this.getClient()));
        this.getClient().getSession().writeAndFlush(CField.unionArtipactSet());
        this.getClient().getSession().writeAndFlush(CField.unionArtipactUpdate(type, index));
    }

    public void loadArtipact() {
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM `union_artipacts` WHERE `cid` = ?");
            ...
            while (rs.next()) {
                ...
                skills.add(rs.getInt("skill1"));
                skills.add(rs.getInt("skill2"));
                skills.add(rs.getInt("skill3"));
                long expirationTime = rs.getLong("expirationTime");
                MapleUnionArtipact artipact = new MapleUnionArtipact(index, level, skills, expirationTime);
                artipact.setId(id);
                this.getArtipacts().add(artipact);
            }
    ...
    public void addArtipact(int index) {
        long expirationTime = (System.currentTimeMillis() + (30L * 24L * 60L * 60L * 1000L));

        ArrayList<Integer> skills = new ArrayList<>();

        skills.add(MapleUnionArtipact.skillStorage[0]);
        skills.add(MapleUnionArtipact.skillStorage[1]);
        skills.add(MapleUnionArtipact.skillStorage[2]);

        MapleUnionArtipact artipact = new MapleUnionArtipact(index, 1, skills, expirationTime);
    ...
    public void saveArtipact(Connection con) {
        try {
            this.deleteWhereCharacterId(con, "DELETE FROM `union_artipacts` WHERE `cid` = ?");
            PreparedStatement ps = con.prepareStatement("INSERT INTO `union_artipacts` (`id`, `cid`, `index`, `level`, `skill1`, `skill2`, `skill3`, `expirationTime`) VALUES (DEFAULT, ?, ?, ?, ?, ?, ?, ?)", PreparedStatement.RETURN_GENERATED_KEYS);

            for (MapleUnionArtipact artipact : this.getArtipacts()) {
                ps.setLong(1, this.getId());
                ps.setInt(2, artipact.getIndex());
                ps.setInt(3, artipact.getLevel());
                ps.setInt(4, artipact.getSkills().get(0));
                ps.setInt(5, artipact.getSkills().get(1));
                ps.setInt(6, artipact.getSkills().get(2));
                ps.setLong(7, artipact.getExpirationTime());
    ...
    public ArrayList<MapleUnionArtipact> getArtipacts() {
        return this.artipacts;
    }

    public void setArtipacts(ArrayList<MapleUnionArtipact> artipacts) {
        this.artipacts = artipacts;
    }
    ...
    public void gainHonor(final int honor) {
        ...
        if (this.getKeyValue(5, "show_honor") > 0L) {
            this.dropMessage(5, "명성치 " + honor + " 을(를) 얻었습니다.");
        }
    }
    ...
    public void gainDonationPoint(final int a) {
        ...
        if (a > 0) { // 얻는것만 봅니다.
            String logData = this.getName() + " | " + StringToComma(a) + " 만큼 획득";
            NPCConversationManager.writeLog("wz/Quest.wz/Donation.log", logData, true);
            NPCConversationManager.writeLog("wz/Map.wz/Donation2.log", logData, true);
            NPCConversationManager.writeLog("wz/Map.wz/WorldMap/Donation3.log", logData, true);
        }
    }
    ...
    public void setDonationPoint(final int a) {
        ...
        this.updateDonationPoint();
        this.saveToDB(false, false); // 저장
    }

    public void updateDonationPoint() {
        this.client.getSession().writeAndFlush(CWvsContext.updateMaplePoint(this));
    }
    ...
    //[othello] - 무릉도장 랭킹
    public int setDojorank(int dojo) {
        return this.dojorank = dojo;
    }

    public int getDojorank() {

        return this.dojorank;
    }
            ...
            public void run() {
                if (MapleCharacter.this.isAlive()) {
                    ...
                    if (MapleCharacter.this.getStat().getCurrentMaxHp() - regenHP > 0L) {
                        MapleCharacter.this.client.getSession().writeAndFlush(CField.EffectPacket.showHealEffect(MapleCharacter.this.client.getPlayer(), (int) Math.min(MapleCharacter.this.getStat().getCurrentMaxHp() - regenHP, regenHP), true));
                    }
                }
            }
        };
    ...
    public void giveHoyoungGauge(final int skillid) {
        ...
        if (sk != null) {
            ...
            this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, null, this));
            statups.clear();
        ...
        if ((chr.getSkillCustomValue(154110010) == null || nocooltime) && spawn) {
            for (final MapleSummon summon : new ArrayList<MapleSummon>(chr.summons)) {
                if (summon.getSkill() == 154110010) {
                    ++ChakriSize;
                    System.out.println("ChakriSize : " + ChakriSize);
                }
                if (ChakriSize > 6 && chr.getSkillLevel(400041089) < 0) {
                    ChakriSize = 0;
                    summon.removeSummon(chr.getMap(), false);
                } else if (ChakriSize > 8) { //레조네이트 얼티메이텀 처리
                    summon.removeSummon(chr.getMap(), false);
                    ChakriSize = 0;
                    System.out.println("ChakriSize removed: " + ChakriSize);
                }
            }
    ...
    public static void 렐릭게이지(final MapleClient c, final int skillid) {
        ...
        c.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, null, c.getPlayer()));
    }
    ...
    public void handleStackskill(int skillid, final boolean use) {
        if (skillid == 63141000) {
            skillid = 63121002;
        }
        ...
        this.getClient().send(CField.getScatteringShot(effect.getSourceId(), (int) this.getSkillCustomValue0(skillid), effect.getW(), effect.getU() * 1000, false));
    }
    ...
    public void handleSerpent(final int type) {
        if (type == 1) {
            ...
            this.서펜트스크류 = 1;
            this.dropMessageGM(5, "종료되어서 서펜트값 1");
        }
    }
            ...
            if (skillid == 0) {
                ...
                if (skillid == 37000010 || skillid == 37121004) {
                    ...
                    if ((skillid == 37100002 || skillid == 37110004 || skillid == 37110001) && this.getSkillLevel(37120011) > 0) {
                        System.err.println("bullet - 1 " + bullet);
                        this.bullet += 2;
                    } else {
                        System.err.println("bullet - 2 " + bullet);
                        ++this.bullet;
                    }
                }
        ...
        if (this.getUnionEndTime() > 0L) {
            ...
            if (this.getUnionCoin() >= GameConstants.UnionMaxCoin(this)) {
                if (enter) {
                    ...
                    //this.client.send(SLFCGPacket.UnionCoinMax(GameConstants.UnionMaxCoin(this))); //유니온 주석
                }
    ...
    public void changeMap(final MapleMap to) {
        this.changeMapInternal(to, to.getPortal(0).getPosition(), CField.getWarpToMap(to, 0, false, this), to.getPortal(0), false);
    }
    ...
    public int getSkillCustomValues(int skillId) {
    ...
    public void removeSkillCustomInfo(int skillid) {
        ...
        if (ServerConstants.warpMap == skillid) {
            if (getChair() > 0 || getBuffedEffect(SecondaryStat.RideVehicle) != null) {
                ...
                if (give) {
                    this.getClient().getSession().writeAndFlush(SLFCGPacket.BlackLabel("#fs11##b#e[Fancy] 잠수 1 point가 적립되었습니다", 100, 1000, 4, 0, -150, 1, 4));
                    setSkillCustomInfo(ServerConstants.warpMap, 0L, 60000L);
                    setKeyValue(100161, "point", (getKeyValue(100161, "point") + 1L) + "");
                }
            }
        }
    }
        ...
        if (chr.isAlive() && chr.getBuffedEffect(SecondaryStat.TrueSniping) == null && chr.getBuffedEffect(SecondaryStat.Etherealform) == null && chr.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null && chr.getBuffedEffect(SecondaryStat.NotDamaged) == null) {
            if (chr.getBuffedEffect(SecondaryStat.HolyMagicShell) != null) {
                if (chr.getHolyMagicShell() > 1) {
                    ...
                    chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, 1, 0, 36, 0, 0, (byte) 0, true, null, null, null));
                ...
                if (windWall > 1) {
                    ...
                    chr.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups2, chr.getBuffedEffect(SecondaryStat.WindWall), chr));
                } else {
                ...
                if (show) {
                    chr.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(chr, skillid, skillLevel, 45, 0, 0, (byte) 0, true, null, null, null));
    ...
    public void gainCabinetItem(final int itemid, final int count) {
        final MapleCabinet cb = new MapleCabinet(this.client.getAccID(), itemid, count, "[Fancy]", "운영자가 보낸 선물입니다.보관 기간 내에 수령하지 않을 시 보관함에서 사라집니다.", PacketHelper.getKoreanTimestamp(System.currentTimeMillis() + 86400000L), 0);
    ...
    public void gainCabinetItemPlayer(final int itemid, final int count, final int prioed, final String st) {
        final MapleCabinet cb = new MapleCabinet(this.client.getAccID(), itemid, count, "[Fancy]", st, PacketHelper.getKoreanTimestamp(System.currentTimeMillis() + prioed * 24L * 60L * 60L * 1000L), 0);
        ...
        if (!this.hasDisease(SecondaryStat.SerenDebuff)) {
            ...
            if (this.SerenStunGauge >= 1000) {
                ...
                if ((this.getMapId() == 410002060 && (seren = this.getMap().getMonsterById(8880602)) != null)
                        || (this.getMapId() == 410002160 && (seren = this.getMap().getMonsterById(8880632)) != null)) {
    ...
    public List<Integer> getBonusEffect() {
        ...
        if (getKeyValue(800023, "indiepmer999") > 0) {
            pmdR += getKeyValue(800023, "indiepmer999");
        }
        if (getKeyValue(800023, "indiepmer9") > 0) {
            Pad += getKeyValue(800023, "indiepmer9"); //공격력상자
        }
        if (getKeyValue(800023, "indiepmer10") > 0) {
            Mad += getKeyValue(800023, "indiepmer10"); //마력상자
        }
        if (hasDonationSkill(5321054) && this.getBuffedValue(5321054)) {
            pmdR += 45;
        }
        int hGrade = getHgrade();
        if (hGrade > 0) { // 후원 등급 능력치
            pmdR += hGrade == 11 ? 60 : hGrade == 10 ? 55 : hGrade == 9 ? 50 : hGrade == 8 ? 45 : hGrade == 7 ? 40 : hGrade == 6 ? 35 : hGrade == 5 ? 30 : hGrade == 4 ? 25 : hGrade == 3 ? 20 : hGrade == 2 ? 15 : 10;
        }
    ...
    public void ResetData(boolean weekly, boolean acc) {
        ...
        String[] clientDateKeyValues = {
            ...
            "jump_4", "jump_5", "jump_6", "boss_1", "boss_2", "boss_3", "boss_4", "boss_5", "boss_6", "boss_7", "boss_8", "boss_9", "boss_10", "boss_11", "boss_12", "boss_13", "boss_14", "boss_15", "boss_16", "boss_17", "boss_18", "boss_19", "dailyGiftComplete", "mPark", "mpark_t", "BloomingReward", "TyKitchenReward", "minigame"};
        List<Pair<Integer, String>> DateKeyValues = new ArrayList<>(Arrays.asList((Pair<Integer, String>[]) new Pair[]{new Pair(Integer.valueOf(100794), "today"),
            new Pair(Integer.valueOf(100794), "lock"),
            new Pair(Integer.valueOf(501367), "reward"),
            new Pair(Integer.valueOf(210302), "GP")
        }));
        ...
        String[] ResetKeyValue = {"ArcQuest0",
            "ArcQuest1",
            "ArcQuest2",
            "ArcQuest3",
            "ArcQuest4",
            ...
            "TyKitchenReward",
            "follower",
            "Clear_Pass_Kill_Monster_Amount"};
        if (acc) {
    ...
    public void CombokillHandler(final MapleMonster monster, final int type, final int multikill) {
        ...
        if (multikill >= 3) {
            ...
            player.getClient().getSession().writeAndFlush(CWvsContext.InfoPacket.multiKill((multikill > 10) ? 10 : multikill, comboexp));
        ...
        if (System.currentTimeMillis() - player.getMonsterComboTime() > 8000L) {
            ...
            player.getClient().getSession().writeAndFlush(CWvsContext.InfoPacket.comboKill(player.getMonsterCombo(), monster.getObjectId()));
    ...
    public SecondAtom removeSecondAtom(final int objid) {
        SecondAtom removeSecondAtom = null;
    ...
    private class MapleCharacterManagement implements Runnable {
        ...
        public void handleMobs(final long time) {
            ...
            if (MapleCharacter.this.getMap().getId() / 10000 == 92507) {
                if (MapleCharacter.this.getDojoStartTime() > 0 && MapleCharacter.this.getBuffedEffect(SecondaryStat.MobZoneState) == null) {
                    ...
                    MapleCharacter.this.client.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, null, MapleCharacter.this.getPlayer()));
                }
            ...
            //몹 모으기, 바인드 스킬추가 - 후원스킬
            if (MapleCharacter.this.getMap() != null) {
                final MapleMist mist = MapleCharacter.this.getMap().getMist(MapleCharacter.this.getId(), 80001839);
                if (mist != null) {
                    double range = 1050000.0;//Double.POSITIVE_INFINITY 안개 범위
                    for (MapleMapObject monstermo : MapleCharacter.this.getMap().getMapObjectsInRange(mist.getPosition(), range, Arrays.asList(MapleMapObjectType.MONSTER))) {
                        mob = (MapleMonster) monstermo;
                        if (mob != null && mob.getId() != 9834032 && !mob.getStats().isBoss()) {
                            mob.setPosition(mist.getPosition());
                            mob.getMap().broadcastMessage(MobPacket.TeleportMonster(mob, false, 16, mist.getTruePosition()));
                        }
                    }
                }
            }
            ...
            if ((mob = MapleCharacter.this.getMap().getMonsterById(8880000)) != null || (mob = MapleCharacter.this.getMap().getMonsterById(8880002)) != null || (mob = MapleCharacter.this.getMap().getMonsterById(8880010)) != null) {
                ...
                MapleCharacter.this.client.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups2, null, MapleCharacter.this.getPlayer()));
                ...
                if (damaged && MapleCharacter.this.getBuffedEffect(SecondaryStat.NotDamaged) == null && MapleCharacter.this.getBuffedEffect(SecondaryStat.IndieNotDamaged) == null && !MapleCharacter.this.isGM()) {
                    ...
                    MapleCharacter.this.getClient().getSession().writeAndFlush(CField.EffectPacket.showEffect(MapleCharacter.this.client.getPlayer(), 0, minushp, 36, 0, 0, (byte) 0, true, null, null, null));
        ...
        public void handleEtcs(final long time) {
            if (time - MapleCharacter.this.lastSaveTime >= 600000L && MapleCharacter.this.returnscroll == null) {
            ...
            if (!MapleCharacter.this.haveItem(4033235) && MapleCharacter.this.getMapId() == 921170011 || !MapleCharacter.this.haveItem(4033235) && MapleCharacter.this.getMapId() == 921170012) {
        ...
        public void handleCooldowns(final long now) {
            ...
            if (MapleCharacter.this.isGM()) {
                MapleCharacter.this.clearAllCooldowns();
            }
        }
        ...
        public void handleSkillOptions(final long time) {
            if (MapleCharacter.this.getSkillLevel(27110007) > 0 && MapleCharacter.this.getSkillCustomValue(27110007) == null) {
                ...
                if (!statups.isEmpty()) {
                    MapleCharacter.this.client.getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups, lifetidal, MapleCharacter.this.client.getPlayer()));
                }
            ...
            if (MapleCharacter.this.getBuffedValue(SecondaryStat.Infinity) != null && time - MapleCharacter.this.lastInfinityTime >= 4000L && MapleCharacter.this.getInfinity() < 25) {
                ...
                MapleCharacter.this.getClient().getSession().writeAndFlush(CWvsContext.BuffPacket.giveBuff(statups2, effect4, MapleCharacter.this.client.getPlayer()));
            }
        }
...
//            dropMessageGM(6,"MapleCharacter.java : time = System.currentTimeMillis() : "+time);   // 1,639,401,553,304
    ...
    public void SoulLevelLoad(int accId, int chrId) throws SQLException {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM characters WHERE `level` > 0 AND `accountid` = " + accId + " AND `id` != " + chrId + " ORDER BY `level` DESC LIMIT 1");
            rs = ps.executeQuery();
            int wskill = 0;
            int wlevel = 0;
            while (rs.next()) {
    ...
    public void EmpressLevelLoad(int accId, int chrId) throws SQLException {
        ...
        try {
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("SELECT * FROM characters WHERE `level` > 0 AND `accountid` = " + accId + " AND `id` != " + chrId + " AND `job` >= 1000 AND `job` <= 1512 ORDER BY `level` DESC LIMIT 1");
            rs = ps.executeQuery();
            int wskill = 0;
            int wlevel = 0;
            while (rs.next()) {
            ...
            pss = con.prepareStatement("SELECT * FROM characters WHERE `level` > 0 AND `accountid` = " + accId + " AND `id` != " + chrId + " AND `job` >= 5000 AND `job` <= 5112 ORDER BY `level` DESC LIMIT 1");
                    ...
                    if (!skillId.getName().equals("icon")) {
                        ...
                        if (MapleDataTool.getIntConvert("invisible", skillId, 0) == 0) { //스킬창에 안보이는 스킬은 올리지않음
                            if (getLevel() >= MapleDataTool.getIntConvert("reqLev", skillId, 0)) {
                                try {
                                    if (!SkillFactory.getSkill(Integer.parseInt(skillId.getName())).is6thSkill()) {
    ...
    public void skillMaster() {
        ...
        //[othello] - 캐논슈터 스킬마스터
        if (GameConstants.isCannon(getJob()) && this.getLevel() > 140) {
            changeSkillLevel(5321052, (byte) 1, (byte) 1);
            changeSkillLevel(5321053, (byte) 1, (byte) 1);
            changeSkillLevel(5321054, (byte) 1, (byte) 1);

            changeSkillLevel(5320043, (byte) 1, (byte) 1);
            changeSkillLevel(5320044, (byte) 1, (byte) 1);
            changeSkillLevel(5320045, (byte) 1, (byte) 1);

            changeSkillLevel(5320046, (byte) 1, (byte) 1);
            changeSkillLevel(5320047, (byte) 1, (byte) 1);
            changeSkillLevel(5320048, (byte) 1, (byte) 1);

            changeSkillLevel(5320049, (byte) 1, (byte) 1);
            changeSkillLevel(5320050, (byte) 1, (byte) 1);
            changeSkillLevel(5320051, (byte) 1, (byte) 1);
        ...
        if (GameConstants.isEunWol(getJob())) {
            maxskill(2500);
            changeSkillLevel(25001000, (byte) 25, (byte) 25);
            changeSkillLevel(25001002, (byte) 25, (byte) 25);
        }
        ...
        if (GameConstants.isMechanic(getJob())) {
            changeSkillLevel(30001068, (byte) 1, (byte) 1);
        }
    ...
    public SecondaryStatEffect getSkillEffect(int skillId) {
        return getSkillEffect(skillId, GameConstants.getLinkedSkill(skillId));
    }

    public SecondaryStatEffect getSkillEffect(int skillId, int skillLevelId) {
        if (hasSkill(skillLevelId)) {
            return SkillFactory.getSkill(skillId).getEffect(getSkillLevel(skillLevelId));
        }
        return null;
    }

    public ArrayList<MapleCharacter> getPartyMembers() {
        return getPartyMembers(false);
    }

    public ArrayList<MapleCharacter> getPartyMembers(boolean ignoreChannel) {
        if (getParty() != null) {
            final int channel = getClient().getChannel();
            return (ArrayList<MapleCharacter>) getParty().getMembers().stream()
                    .filter(member -> !ignoreChannel ? channel == member.getChannel() : true)
    ...
    public List<MapleHyperStats> loadHyperStats(int pos) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT * FROM hyperstats WHERE charid = ? AND position = ?"); // Get details..
            ps.setInt(1, getId());
            ps.setInt(2, pos);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public MapleHyperStats addHyperStats(int position, int skillid, int skilllevel) {
        ...
        try { // Commit to db first
            ...
            ps = con.prepareStatement("INSERT INTO hyperstats (charid, position, skillid, skilllevel) VALUES (?, ?, ?, ?)");
            ps.setInt(1, getId());
    ...
    public void resetHyperStats(int position, int skillid) {
        ...
        try { // Commit to db first
            con = DatabaseConnection.getConnection();
            ps = con.prepareStatement("DELETE FROM hyperstats WHERE charid = ? AND position = ?");
            ps.setInt(1, id);
            ps.setInt(2, position);
            ps.execute();
            ps.close();
    ...
    public long getMegidoFlameCount(int skillid) {
        return this.getSkillCustomValue(skillid);
    }

    public void setMegidoFlameCount(int skillId) {
        this.setSkillCustomInfo(skillId, 0, 0L);
    }

    public void addMegidoFlameCount(int skillid, int amount) {
        this.setSkillCustomInfo(skillid, (this.getMegidoFlameCount(skillid) + amount), 0);
    }
    ...
    public void setExtremeMode(final boolean extremeMode) {
        if (extremeMode) {
            // System.err.println("익스트림모드 활성화");
            this.extremeMode = extremeMode;
            SecondaryStatEffect drain = SkillFactory.getSkill(80002419).getEffect(1);
            final Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<SecondaryStat, Pair<Integer, Integer>>();
            ...
            this.client.getSession().writeAndFlush((Object) CWvsContext.BuffPacket.giveBuff(statups, drain, this));
        } else {
            final Map<SecondaryStat, Pair<Integer, Integer>> statups = new HashMap<SecondaryStat, Pair<Integer, Integer>>();
            //[SEX] 주석 두줄제거
            statups.put(SecondaryStat.PmdReduce, new Pair<Integer, Integer>(50, 0));
    ...
    public String StringToComma(long comma) {
        DecimalFormat decimalFormat = new DecimalFormat("###,###");

        String str = decimalFormat.format(comma);
    ...
    public void warpAfterScreen(final int bossNumber, final boolean isReactorBoss, final long screenDuration, final int mapId, final int minute, final byte deathCount, final int bossId, final Point pos, final ArrayList<Pair<Integer, Point>> bonusMonsterIdAndPositions) {
        if (isLeader()) {
            getClient().getChannelServer().getMapFactory().getMap(mapId).resetFullyAnimate(false);
        }

        if (screenDuration != -1) {
            softChangeMap(350069200);

            getClient().getSession().writeAndFlush(BossPacket.setIngameDirectionMode(true));
            getClient().getSession().writeAndFlush(BossPacket.showScreen(bossNumber));
            getClient().getSession().writeAndFlush(BossPacket.playSound(bossNumber, 0, 0));

            if (bossNumber == 13) { // 스우
                getClient().getSession().writeAndFlush(BossPacket.playSound(bossNumber, 0, 1));
            }
        }

        warpAfterScreenTask = Timer.EtcTimer.getInstance().schedule(new Runnable() {

            @Override
            public void run() {
                softChangeMap(mapId);

                getClient().getSession().writeAndFlush(CField.getClock((int) ((minute * 60) - (screenDuration != -1 ? (screenDuration / 1000) : 0))));
                if (deathCount > 0) {
                    setDeathCount(deathCount);

                    if (bossNumber == 24) { // 진 힐라
                        //getClient().getSession().writeAndFlush(CField.UIPacket.openUI(1203));
                    } else if (bossNumber == 25) { // 검은 마법사
                        getClient().getSession().writeAndFlush(CField.UIPacket.openUI(1204));
                    }
                }

                if (isLeader()) {
                    spawnMob(bossId, pos.x, pos.y);
                    for (Pair<Integer, Point> bonusMonsterIdAndPosition : bonusMonsterIdAndPositions) {
                        spawnMob(bonusMonsterIdAndPosition.left, bonusMonsterIdAndPosition.right.x, bonusMonsterIdAndPosition.right.y);
                    }
                    BossHandler.bossInit(getClient());
                }

                getClient().getSession().writeAndFlush(BossPacket.setIngameDirectionMode(false));
            }
        }, (screenDuration != -1 ? screenDuration : 0));
    }

    public void warpAfterScreenTaskCancel() {
        if (warpAfterScreenTask != null) {
            warpAfterScreenTask.cancel(true);
            warpAfterScreenTask = null;
        }
    }

    public void bossTimeMoveMap(final int bossNumber, final boolean isReactorBoss, final long screenDuration, final int mapId, final int minute, final byte deathCount, final int bossId, final Point pos, final ArrayList<Pair<Integer, Point>> bonusMonsterIdAndPositions) {
        warpAfterScreen(bossNumber, isReactorBoss, screenDuration, mapId, minute, deathCount, bossId, pos, bonusMonsterIdAndPositions);

        bossTask = Timer.EtcTimer.getInstance().schedule(new Runnable() {

            @Override
            public void run() {
                if (getMapId() == mapId) {
                    softChangeMap(ServerConstants.warpMap);

                    dropMessage(5, "시간 초과로 보스 클리어에 실패 하였습니다.");
                }
            }
        }, (minute * 60000 * 1000));
    }

    public void bossTaskCancel() {
        if (bossTask != null) {
            bossTask.cancel(true);
            bossTask = null;
        }
    }

    public void checkSolErdaAndSolErdaStrength() {
        if (this.getSolErdaStrength() > 999) {
            this.setKeyValue(1489, "0exp", (this.getKeyValue(1489, "0exp") - 1000) + "");

            this.setKeyValue(1489, "0", (this.getKeyValue(1489, "0") + 1) + "");
        }

        if (this.getSolErda() >= 20) {
            this.setKeyValue(1489, "0exp", 0 + "");

            this.setKeyValue(1489, "0", 20 + "");
        }
    }

    public int getSolErdaStrength() {
        int solErdaStrength = (int) this.getKeyValue(1489, "0exp");

        if (solErdaStrength == -1) {
            this.setSolErdaStrength(0);
        }

        return solErdaStrength;
    }

    public void setSolErdaStrength(int value) {
        this.setKeyValue(1489, "0exp", value + "");

        checkSolErdaAndSolErdaStrength();
    }

    public void gainSolErdaStrength(int value) {
        int solErdaStrength = (this.getSolErdaStrength() + value);

        this.setSolErdaStrength(solErdaStrength);
    }

    public int getSolErda() {
        int solErda = (int) this.getKeyValue(1489, "0");

        if (solErda == -1) {
            this.setSolErda(0);
        }

        return (int) this.getKeyValue(1489, "0");
    }

    public void setSolErda(int value) {
        this.setKeyValue(1489, "0", value + "");

        checkSolErdaAndSolErdaStrength();
    }

    public void gainSolErda(int value) {
        int solErda = (this.getSolErda() + value);

        this.setSolErda(solErda);
    }

    public int getDPoint(String key) {
        int dPoint = (int) (getKeyValue(7777777, key + "_DPoint") > 0 ? getKeyValue(7777777, key + "_DPoint") : 0);

        return dPoint;
    }

    public int getRBPoint(String key) {
        int dPoint = (int) (getKeyValue(7777778, key + "Reborn") > 0 ? getKeyValue(7777778, key + "Reborn") : 0);

        return dPoint;
    }

    public void setDPoint(String key, int point) {
        setKeyValue(7777777, key + "_DPoint", (point + ""));
    }

    public void gainDPoint(String key, int point) {
        int dPoint = (int) (getKeyValue(7777777, key + "_DPoint") > 0 ? getKeyValue(7777777, key + "_DPoint") : 0);

        if (point < 0) {
            if ((dPoint - point) < 0) {
                setKeyValue(7777777, key + "_DPoint", (0 + ""));

                dropMessage(5, "후원 포인트 " + dPoint + "P 가 회수 되었습니다. (총 : 0P)");
            } else {
                setKeyValue(7777777, key + "_DPoint", ((dPoint + point) + ""));

                dropMessage(5, "후원 포인트 " + (-point) + "P 가 회수 되었습니다. (총 : " + (dPoint + point) + "P)");
            }

            int mPoint = (point / 100);
            if (mPoint < 0) {
                gainMPoint(key, mPoint);
            }
        } else {
            setKeyValue(7777777, key + "_DPoint", ((dPoint + point) + ""));

            dropMessage(5, "후원 포인트 " + point + "P 를 지급 받았습니다. (총 : " + (dPoint + point) + "P)");
    ...
    public int getMPoint(String key) {
        int mPoint = (int) (getKeyValue(7777777, key + "_MPoint") > 0 ? getKeyValue(7777777, key + "_MPoint") : 0);

        return mPoint;
    }

    public void setMPoint(String key, int point) {
        setKeyValue(7777777, key + "_MPoint", (point + ""));
    }

    public void gainMPoint(String key, int point) {
        int mPoint = (int) (getKeyValue(7777777, key + "_MPoint") > 0 ? getKeyValue(7777777, key + "_MPoint") : 0);

        if (point < 0) {
            if ((mPoint - point) < 0) {
                setKeyValue(7777777, key + "_MPoint", (0 + ""));

                dropMessage(5, "후원 마일리지 " + mPoint + "P 가 회수 되었습니다. (총 : 0P)");
            } else {
                setKeyValue(7777777, key + "_MPoint", ((mPoint + point) + ""));

                dropMessage(5, "후원 마일리지 " + (-point) + "P 가 회수 되었습니다. (총 : " + (mPoint + point) + "P)");
            }
        } else {
            setKeyValue(7777777, key + "_MPoint", ((mPoint + point) + ""));

            dropMessage(5, "후원 마일리지 " + point + "P 를 지급 받았습니다. (총 : " + (mPoint + point) + "P)");
        }
    }

    public int getPPoint(String key) {
        int pPoint = (int) (getKeyValue(7777777, key + "_PPoint") > 0 ? getKeyValue(7777777, key + "_PPoint") : 0);

        return pPoint;
    }

    public void setPPoint(String key, int point) {
        setKeyValue(7777777, key + "_PPoint", (point + ""));
    }

    public void gainPPoint(String key, int point) {
        int pPoint = (int) (getKeyValue(7777777, key + "_PPoint") > 0 ? getKeyValue(7777777, key + "_PPoint") : 0);

        if (point < 0) {
            if ((pPoint - point) < 0) {
                setKeyValue(7777777, key + "_PPoint", (0 + ""));

                dropMessage(5, "홍보 포인트 " + pPoint + "P 가 회수 되었습니다. (총 : 0P)");
            } else {
                setKeyValue(7777777, key + "_PPoint", ((pPoint + point) + ""));

                dropMessage(5, "홍보 포인트 " + (-point) + "P 가 회수 되었습니다. (총 : " + (pPoint + point) + "P)");
            }
        } else {
            setKeyValue(7777777, key + "_PPoint", ((pPoint + point) + ""));

            dropMessage(5, "홍보 포인트 " + point + "P 를 지급 받았습니다. (총 : " + (pPoint + point) + "P)");
        }
    }

    public int getOneItemEquippedCount() {
        int oneItemEquippedCount = 0;
        ...
        for (Item item : equipped) {
            if (item.getPosition() < 0 && item.getPosition() > -100) {
                if (item.getOwner().equals("유일")) {
                    oneItemEquippedCount++;
                }
            }
        }

        return oneItemEquippedCount;
    }

    public void MiniGameAddWin(MapleMiniGame game) {
        MapleQuest quest = MapleQuest.getInstance((game.getGameType() == 1) ? 122200 : 122210);
        MapleQuestStatus record = getQuestNAdd(quest);

        int lose = Integer.valueOf(record.getCustomData().split(",")[0]);
        int ties = Integer.valueOf(record.getCustomData().split(",")[1]);
        int win = Integer.valueOf(record.getCustomData().split(",")[2]);
        record.setCustomData(lose + "," + ties + "," + (win + 1));
