...
function action(mode, type, selection) {
   ...
   if (status == 0) {
      말 = "#fs11#" + 검정 + "안녕하세요 #b#h ##k" + 검정 + "님! 혹시 치장 아이템도 강화가 된다는 사실 알고 계신가요?\r\n\r\n"
      말 += "#b#z4310306##d을 통해 치장 아이템을 강화할 수 있는 시스템입니다.\r\n\r\n"
      말 += "소모 재료 : #i4310306##r#t4310306# 50개#k, #b10억 메소#k\r\n" 
      말 += "강화 수치 : #r올스탯+5, 공격력+3, 마력+3 #d씩 증가합니다\r\n\r\n"
      말 += "한 아이템에 최대 30회까지 강화가 가능하지만 #r펫 장비를 비롯한 일부 아이템#d에는 사용할 수 없습니다.\r\n"
      말 += "#fc0xFFD5D5D5#─────────────────────────────#k\r\n";
      ...
      if (sssss == 1) {
        ...
        if (cm.getPlayer().getMeso() < needPrice) {
                cm.sendOkS("#fs11#"+검정+"메소가 부족해요 한번 강화하는데 10억메소가 필요합니다", 0x04, 9075000);
                cm.dispose();
                return;
        }
         if (!cm.haveItem(coin, price)) {
            cm.sendOkS("#fs11#"+검정+"강화 재료가 부족해요! #i4310306##r#t4310306# 5000개#d가 필요합니다.", 0x04, 9075000);
            cm.dispose();
            return;
         }
         ...
         if (item.getStr() > 149 || item.getDex() > 149 || item.getInt() > 149 || item.getLuk() > 149 || item.getWatk() > 90 || item.getMatk() > 90 || item.getHp() > 2490) {
            cm.sendOkS("#fs11#"+검정+"더 이상 강화는 불가능해요! 한 아이템에 최대 #r30회#d 까지만 강화할 수 있습니다.", 0x04, 9075000);
            cm.dispose();
            return;
         }
