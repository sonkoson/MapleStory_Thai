...
function action(mode, type, sel) {
	...
	if (status == 0) {
		getData();
		var msg = "#fs11#안녕하세요. #eFANCY#n 후원포인트 지급 도우미 릴리입니다.\r\n"+enter;
		msg += "현재 #b#h ##k님의 후원 포인트 : #b"+cm.getPlayer().getDonationPoint()+"P#k"+enter;
		msg += "현재 #b#h ##k님의 FANCY 등급 : #b"+cm.getPlayer().getHgrades()+"#k#b"+enter;
		...
		switch (sel) {
			...
			break;
			case 5:
				var msg = "현재 #b#h ##k님의 FANCY 등급은 #b"+cm.getPlayer().getHgrades()+"#k입니다."+enter;
			...
			case 6:
				if (cm.getPlayer().getHgrade() < 1) {
					cm.sendOk("FANCY 골드 이상만 사용할 수 있는 기능입니다.");
					cm.dispose();
					return;
				}
  				time = new Date();
