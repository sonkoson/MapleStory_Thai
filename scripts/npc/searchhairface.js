...
function action(mode, type, selection) {
            ...
            if (isHairId) { //헤어일 때
                ...
                searchedMolding = searchHair(searchTarget)
                     if (searchedMolding.length > 0) {
                                cm.askCoupon(5150132,searchedMolding);
                            } else {
                                cm.sendOk('#fs11# 검색된 ' + (isHairId ? '헤어가' : '성형이') + ' 없습니다.');
                            }

                               if (Packages.constants.GameConstants.isAngelicBuster(cm.getPlayer())) {

                               if (searchedMolding.length > 0) {
                                                               cm.askCoupon(5150132,1,searchedMolding);
                                                           } else {
                                                               cm.sendOk('#fs11# 검색된 ' + (isHairId ? '헤어가' : '성형이') + ' 없습니다.');
                                                           }
                ...
                searchedMolding = searchFace(searchTarget)
                     if (searchedMolding.length > 0) {
                                cm.askCoupon(5152020,searchedMolding);
                            } else {
                                cm.sendOk('#fs11# 검색된 ' + (isHairId ? '헤어가' : '성형이') + ' 없습니다.');
                            }
                         if (Packages.constants.GameConstants.isAngelicBuster(cm.getPlayer())) {
	...
	cm.getPlayer().dropMessage(5,""+searchedMolding[selection]);
        if (selection >= 0) {
         cm.getPlayer().dropMessage(5,"askCoupon_Hairsearch test ");
...
function setAvatar(args) {
    if (isHair(args)) {
        cm.askCoupon_Hairsearch(args);

    } else if (isFace(args)) {
        cm.askCoupon_Facesearch(args);
