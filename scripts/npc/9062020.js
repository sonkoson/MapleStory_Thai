        ...
        if (status == 0) {
            if (cm.getPlayer().getLevel() >= 100) {
                if (cm.getPlayer().getMapId() == 993000750) {
                    var Fancy = "#fs11##rFancy#k 에서 입소문난 #d설귀도 낚시터!#k\r\n이 곳이 그렇게 #d월척#k이 잘 잡히기로 소문이 자자하지..\r\n";
                ...
                cm.sendOk("#fs11##r낚시 이용은 레벨 100 이상만 이용 가능합니다.", 9062004);
                cm.dispose();
            } 
        } else if (status == 1) {
            if (selection == 0) {
                cm.sendOk("#fs11#나에게 #r낚시 용품#k을 구입한 후 좋은 자리를 선점하게곰.\r\n#r미끼#k를 소지 후 의자에 앉아 있으면 낚시가 진행된다곰.");
                cm.dispose();

            } else if (selection == 1) {
                var FANCY2 = "#fs11##b원하시는 품목을 선택 해보게나..#k\r\n#r인벤토리가 꽉차면 못 받을 수 있으니 주의 바란다네..#k\r\n";
