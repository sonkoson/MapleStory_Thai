...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		cm.sendNextS("#fs11#정말 #r#i"+item.getItemId()+":# #z"+item.getItemId()+":##k을 강화 하겠나? 선택은 되돌릴 수 없으니 신중하게 선택하게나.", 4, 2192030);
	} else if (status == 3) {
		if (sssss == 1) {
			cm.sendScreenText("#fs27##fn나눔고딕 Extrabold##fc0xFF6B66FF#치장아이템 강화#k에 대해 설명 드리겠습니다.", false);
		} else {
			if (!cm.haveItem(coin, price)) {
				cm.sendOkS("#fs11#"+검정+"도와주긴 하겠지만 필요한 재료는 자네가 가져와야 한다네.", 0x04, 2192030);
				cm.dispose();
				return;
			}
			...
			if (item.getStr() > 49 || item.getDex() > 49 || item.getInt() > 49 || item.getLuk() > 49 || item.getWatk() > 49 || item.getMatk() > 49 || item.getHp() > 490) {
				cm.sendOkS("#fs11#"+검정+"더 이상 강화는 불가능하다네.", 0x04, 2192030);
				cm.dispose();
				return;
			}
			if (item.getItemId() >= 1802000 && item.getItemId() <= 1802999) {
				cm.sendOkS("#fs11#"+검정+"펫 장비는 강화가 불가능 하다네.", 0x04, 2192030);
				cm.dispose();
				return;
			}
			if (item.getItemId() >= 1112000 && item.getItemId() <= 1112099 || item.getItemId() >= 1112800 && item.getItemId() <= 1112899) {
				cm.sendOkS("#fs11#"+검정+"커플링과 우정링은 강화가 불가능하다네.", 0x04, 2192030);
				cm.dispose();
				return;
			}
			if (!cm.isCash(item.getItemId())) {
				cm.sendOkS("#fs11#"+검정+"비정상적인 아이템인거 같네.", 0x04, 2192030);
				cm.dispose();
				return;
			}
			var suc = false;
