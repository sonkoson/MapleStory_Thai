...
function action(mode, type, sel) {
    ...
    if (status == 0) {
		...
		if (!cm.haveItem(price, 1)) {
			cm.sendOk("#fs11##fc0xFF000000#초기화를 하기 위해선 #b#z" + price + "##fc0xFF000000# 1개가 필요합니다.");
			cm.dispose();
			return;
		}
		if (item.getCoption3() == 0) {
			cm.sendOk("#fs11#선택하신 아이템의 재설정 횟수가 #r10회#k 남으신거 같은데요?");
			cm.dispose();
			return;
		}
