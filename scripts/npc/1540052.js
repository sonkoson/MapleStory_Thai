...
function action(mode, type, selection) {
	...
	if (status == 0) {
		if (cm.getPlayer().getLevel() < 100) {
			cm.sendOk("#fs11#" + 검정 + "어느 정도 "+서버이름+"서버를 즐긴 뒤, 이용이 가능하다네.");
			cm.dispose();
			return;
		}
		var text = "#fs11#" + 검정 + "어서 오게! 자네도 " + 블루 + ""+서버이름+" 세계#k" + 검정 + "에 초대받았나 보군! ";
		...
		if (selection == 0) {
			if (!overlab_recom(cm.getClient().getAccID(), cm.getPlayer().getName())) {
				cm.sendGetText("#b#fs11#" + cm.getPlayer().getName() + "#k"+검정+"! 자네를 #b"+서버이름+" 세계#k"+검정+"에 초대해준 사람의 #r닉네임#k"+검정+"을 적어보게! 초대해준 대상이 따로 없다면 #r'운영자 닉네임'#k"+검정+"을 적으면 된다네.\r\n추천인 등록은 #r'1회'#k"+검정+" 만 가능하니 신중하게 적어보게!", 2008);
			} else {
				cm.sendOk("#fs11#"+검정+"추천 등록은 한번만 가능하네.");
				cm.dispose();
			}
		} else if (selection == 1) {
			cm.sendSimpleS("#fs11#"+검정+"여긴 "+서버이름+"의 세계를 알려준 용사들의 이름이지.\r\n자네도 "+서버이름+"의 세계를 널리 퍼트려주게!\r\n" + recom_log(), 0x04, 2008);
			status = 2;
			...
			if (recoms_num == 0) cm.sendOk("#fs11#"+검정+"아직 자네를 추천한 사람은 없는거 같네. 하지만 실망하지 말게, 앞으로 열심히 한다면 추천 받을지도 모르네! 크크."), cm.dispose();
			else {
		...
		if (cm.getText().equals("") || cm.getText().equals(cm.getPlayer().getName()) || getAccIdFromDB(cm.getText()) == getAccIdFromDB(cm.getPlayer().getName())) {
			cm.sendOkS(cm.getText().equals("") ? "#fs11#"+검정+"입력을 잘못한거 같네." : "#fs11#"+검정+"초대한 상대의 이름을 적어야 한다네.", 0x04, 2008);
			cm.dispose();
		} else {
			getItem();
			getItem2();
