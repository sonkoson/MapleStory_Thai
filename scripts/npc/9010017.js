...
1
			...
			if (ch >= 0)
			{
				...
				if (chr != null)
				{
					...
					cm.sendSimpleS("#b#e"+chr.getName()+"#k#n 캐릭터에 접속했습니다. 무엇을 도와드릴까요?#b\r\n\r\n#fn돋움체##fs11#"
						...
						+ "   - 보유 아쿠아 코인 량 | "+Comma(chr.itemQuantity(4310237))+"개\r\n"
						...
						+ "   - 보유 후원 포인트 | "+Comma(chr.getDPoint())+"\r\n#fs12##fn돋움#"
						//+ "#L16#홍보포인트 지급#l #L17#후원포인트 지급#l\r\n"
						+ "#L13#네오스톤 지급#l#L14#네오잼 지급#l#L15#네오코어 지급#l\r\n\r\n"
						+ "#L18#네오스톤 지급#l#L19#네오잼 지급#l#L20#네오코어 지급#l\r\n\r\n"
	...
	else if(St == 3)
	{
		...
		switch(S1)
		{
			...
			if(S3 >= 18 && S3 <= 20)
			{
				cm.sendGetNumber(""+chr.getName()+"에게 회수할 포인트를 입력해주세요.", 1, 1, 999999);
				return;
			}
	...
	else if(St == 4)
	{
		...
		switch(S1)
		{
			 ...
			 if(S3 >= 18 && S3 <= 20)
			{
				number = S4;
                if(S3 == 18){  
                    txt = name+" 캐릭터에게 네오스톤을 "+number+"개를 회수하였습니다.";
                    chr.setKeyValue(100711, "point", chr.getKeyValue(100711, "point")-number);
                } else if(S3 == 19) {
                    txt = name+" 캐릭터에게 네오 잼을 "+number+"개를 회수하였습니다.";
                    chr.setKeyValue(100712, "point", chr.getKeyValue(100712, "point")-number);
                } else if(S3 == 20) {
                    txt = name+" 캐릭터에게 네오 코어를 "+number+"개를 회수하였습니다.";
                    chr.setKeyValue(501215, "point", chr.getKeyValue(501215, "point")-number);
                }
                cm.getPlayer().dropMessage(6, txt);
                cm.dispose();
			...
			{
				number = S4;
				txt = name+" 해당 캐릭터의 후원포인트를 " +number+ "로 설정하였습니다.\r\n";

                  chr.setDPoint(number);
				  txt += "설정된 포인트 : " + chr.getDPoint();
				      cm.getPlayer().dropMessage(6, txt);
			}
			break;
		}
