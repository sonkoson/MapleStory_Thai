...
function action(mode, type, selection) {
    ...
    /* 스크립트 메인 부분 */
    if (status == 0) {
        cm.sendSimple("#b#fs11#" + cm.getPlayer().getName() + "#k님, PRAY에 오신 것을 환영합니다.\r\n#r(추천은 1인 1계정 레벨 100이상 가능합니다.)#k\r\n\r\n#fUI/UIWindow.img/UtilDlgEx/list1#\r\n#L0##b추천인#k 등록하기\r\n#L1##b추천인#k 랭킹보기#l\r\n\r\n\r\n#fUI/UIWindow.img/UtilDlgEx/list0#\r\n#L2##b추천인#k 확인하기\r\n");
    } else if (status == 1) {
        if (selection == 0) {
            if (!overlab_recom(cm.getClient().getAccID(), cm.getPlayer().getName())) {
                if (cm.getPlayer().getLevel() >= 100) {
                    cm.sendGetText("#b#fs11#" + cm.getPlayer().getName() + "#k님, 당신을 PRAY으로 이끈 이의 #b닉네임#k을 말씀해주세요.\r\n하지만 #r한 번 등록하면 되돌릴 수 없으니#k 신중해야만 해요.");
                } else {
                    cm.sendOk("#fs11##r100레벨 미만#k은 추천인을 등록할 수 없어요.");
                    cm.dispose();
                }
            } else {
                cm.sendOk("#fs11#추천인은 한 번만 작성 가능하답니다.");
                cm.dispose();
            }
            ...
            if (recoms_num == 0) {
                cm.sendOk("#fs11#아쉽지만 아직 #b" + cm.getPlayer().getName() + "#k님의 이름을 속삭인 분이 없네요.\r\n열심히 #bPRAY#k을 알린다면, 곧 누군가 속삭일테니 실망 말아요."), cm.dispose();
            } else {
