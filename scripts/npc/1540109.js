...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            var leaf = cm.itemQuantity(2632800);
            var text = ""
            text = "#fs11#" + 검정 + "저는 #fc0xFF000087#[해피]#k " + 검정 + "컨텐츠 부서에서 #b라이딩 뽑기#k" + 검정 + "를 담당하고 있는 포밍이라고 합니다."
            text += "\r\n라이딩 뽑기 한번 당 #r#z2632800# 65개#k" + 검정 + "로 가 가능합니다.\r\n\r\n";
            text += "현재 보유중인 #d#z2632800##k 갯수 : #e#r"+leaf+"#k개#n\r\n"
            text += "#fc0xFFD5D5D5#───────────────────────────#k\r\n";
            ...
            if (selection == 12) {
                ...
                item = selection;
                if (!cm.haveItem(2632800, 65)) {
		            cm.sendOk("#fs11##i2632800# #b#z2632800##k 아이템이 부족하거나 없는거 같네요.");
                    cm.dispose();
                } else {
                    cm.sendYesNo("#fs11#정말로 #b라이딩#k을 뽑으시겠습니까?\r\n뽑기 진행 시 #r#z2632800# 65개#k가 소모됩니다.");
                }
            }
        } else if (status == 2) {
            switch (item) {
