...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        chat += "#e" + "⊙ 공격력 + 15" + "#n" + enter;
        chat += "#e#b강화 성공확률 : " + 35 +"% #k| #r강화 실패확률 : " + (100 - 30) + "%#n#k" + enter;
        ...
        var successProb = 15;
        if (cm.haveItem(need, 5)) {
            var prob = Packages.server.Randomizer.rand(1, 100);

            selectedItem = cm.getInventory(1).getItem(selection);

            if (!Packages.constants.GameConstants.isWeapon(selectedItem.getItemId())) {
                cm.sendOk("공격력/마력 강화는 무기 아이템에만 강화 할 수 있습니다.");
                cm.dispose();
                return;
            }
            ...
            switch(enhanceValue) {
                ...
                case ItemResult[14]: // 15강에서는 Stop
                  cm.sendOk("#fs11#해당 아이템은 더 이상 강화할수 없습니다.");
                  cm.dispose();
                  return;
                ...
                var beforeOption = selectedItem.getWatk();

                selectedItem.setWatk(selectedItem.getWatk() + enhanceOption);
                ...
                chat += "#d" + "변경된 능력치 : [공격력] #k" + beforeOption + " + #b15" + enter + enter;

                chat += cm.getPlayer().getName() + "님 이용해주셔서 감사합니다. 오늘도 PRAY 에서 좋은 하루 보내세요.";
                cm.sendOk(chat);
                cm.gainItem(need, -5);
                ...
                chat += "#b" + cm.getPlayer().getName() + "님 이용해주셔서 감사합니다. 오늘도 PRAY 에서 좋은 하루 보내세요.";
                cm.gainItem(need, -5);
                cm.sendOk(chat);
                cm.dispose();
                return;
            ...
            cm.sendOk("#fs11#강화에 필요한 아이템이 부족합니다.");
            cm.dispose();
            return;
        }
        return;
