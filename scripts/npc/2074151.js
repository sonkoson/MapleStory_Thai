...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (size < 5) {
            cm.sendOkS("#fs11#"+검정+"이봐, #h #! 더 이상 #r조합이 불가능한 펫#k"+검정+"을 데리고 있나?\r\n블랙 페스티벌#k"+검정+"을 더 재밌게 즐기기 위해 내가 "+펫+" #b원더 펫#k "+검정+"5마리 당 #b#z5068300##k "+검정+"1개"+검정+"로 교환해주고 있다네.\r\n\r\n하지만 자네는 교환에 필요한 원더 펫이 부족한거 같군. ", 0x04, 2192030);
            cm.dispose();
        } else {
            cm.sendGetNumber(2192030, "#fs11#"+검정+"\r\n이봐, #h #! 더 이상 #r조합이 불가능한 펫#k"+검정+"을 데리고 있나?\r\n블랙 페스티벌#k"+검정+"을 더 재밌게 즐기기 위해 내가 "+펫+" #b원더 펫#k "+검정+"5마리 당 #b#z5068300##k "+검정+"1개"+검정+"로 교환해주고 있다네.\r\n\r\n자네는 현재 " + size + "마리가 있어 #b#z5068300##k"+검정+"를 " + Math.floor((size / 5)) + "개 받을수 있네 몇개를 교환할 건가? \r\n ", Math.floor((size / 5)), 1, Math.floor((size / 5)));
        }

    } else if (status == 1) {
        count = selection;
        cm.sendYesNoS("#fs11#"+검정+"정말 #b원더펫 " + (count * 5) + "마리#k"+검정+"로 #r#z5068300# " + count + " 개#k"+검정+"를 교환할텐가?", 0x04, 2192030);
    } else if (status == 2) {
        inz = cm.getInventory(5)
        for (w = 0; w < inz.getSlotLimit(); w++) {
            if (!inz.getItem(w) || inz.getItem(w).getPet() == null) {
