        ...
        if (status == 0) {
            ...
            if (selection == 1) {
                ...
                if (cm.getPlayer().getNeoStone() < 개수) {
                    cm.sendOkS("#fs11##i4310300# #z4310300# " + (개수 - cm.getPlayer().getNeoStone()) + "개가 부족 합니다.", 2);
                    cm.dispose();
                } else {
                    cm.getPlayer().gainNeoStone(-개수);
                    획득 = "#fs11#" + enter + enter + "축하드립니다~! #h #님! 아래와 같은 아이템이 나왔어요!" + enter + enter + "#fUI/UIWindow2.img/QuestIcon/4/0#" + enter
