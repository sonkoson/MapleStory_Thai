...
var daily = [ // 첫번째
	{'mobid' : 8645123, 'qty' : 800, 't' : 1}, 
{'mobid' : 8645124, 'qty' : 801, 't' : 1},
{'mobid' : 8645125, 'qty' : 802, 't' : 1},
{'mobid' : 8645126, 'qty' : 803, 't' : 1},
{'mobid' : 8645127, 'qty' : 804, 't' : 1},
{'mobid' : 8645131, 'qty' : 805, 't' : 1}
]

var daily2  = [ //두번째
	{'mobid' : 8645123, 'qty' : 801, 't' : 1}, 
{'mobid' : 8645124, 'qty' : 802, 't' : 1},
{'mobid' : 8645125, 'qty' : 803, 't' : 1},
{'mobid' : 8645126, 'qty' : 804, 't' : 1},
{'mobid' : 8645127, 'qty' : 805, 't' : 1},
{'mobid' : 8645131, 'qty' : 806, 't' : 1}
]
...
function action(mode, type, sel) {
	...
	if (status == 0) {
		if (cm.getPlayer().getLevel() < 제한레벨) {
			cm.sendOk("나는 홍염과 불꽃의 스피사를 모시면서 세르니움을 연구하고있지.");
			cm.dispose();
			return;
		}
		getData();
		...
		if (isclear == 2 || isclear == 5) {
			if ((isitem && (cm.haveItem(getk("mobid"), getk("mobq")))) || (!isitem && (getk("count") >= getk("mobq")))) {
				...
				var msg = "#b#h ##k 너 덕분에 수월하게 임무를 완수할 수 있었군."+enter;
				if (isclear == 5) {
					msg += "여기, 이건 오늘의 보상이다."+enter;
					msg += getReward()+enter;
				}
				msg += "#k앞으로도 잘 부탁한다."+enter;

				if (isclear == 2)
					msg += "물론 네가 원할 만한 물건은 챙겨 주도록 하지.";
				else 
					msg += "";
		...
		if (isclear == 5) {
			if ((isitem && (cm.haveItem(getk("mobid"), getk("mobq")))) || (!isitem && (getk("count") >= getk("mobq")))) {
				if (isitem) cm.gainItem(getk("mobid"), -getk("mobq"));
				setk("isclear", "6");
				var msg = "#b#h ##k 너 덕분에 명령을 쉽게 완수할 수 있었군."+enter;
				msg += "여기, 이건 오늘의 보상이다"+enter;
				msg += getReward()+enter;
				msg += "#k앞으로도 잘 부탁한다."+enter;
				msg += "임무를 하나 더 완수하면 일일 보상을 지급해주겠다.\r\n준비가 되면 다시 한번 말을 걸어라";
				cm.sendOk(msg);
				cm.dispose();
			}
		}
		switch (isclear) {
			case 1:
				var msg = "나는 홍염과 불꽃의 스피사를 모시면서 세르니움을 연구하고있지."+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#d#L1#(Lv."+제한레벨+") #b#e[일일 퀘스트]#n#k #d세르니움 조사";

				cm.sendSimple(msg);
			break;
			case 2:
				var msg = "처리해줘야 할 임무는 아래와 같다."+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 세르니움 조사#n#k"+enter;
				msg += "처치 대상 : #b#o"+getk("mobid")+"# "+getk("mobq")+"마리#k"+enter+enter;
				msg += "그럼, 일이 끝난 후에 다시 찾아와라";
			...
			case 4:
				var msg = "나는 홍염과 불꽃의 스피사를 모시면서 세르니움을 연구하고있지."+enter+enter;
				msg += "#fUI/UIWindow2.img/UtilDlgEx/list1#\r\n#d#L1#(Lv."+제한레벨+") #b#e[일일 퀘스트]#n#k #d세르니움 조사 2";
			...
			break;
			case 6:
				var msg = "어서 와라 #h #군."+enter;
				msg += "오늘은 더 이상 임무가 없다."+enter;
		...
		if (getk("isclear") == 1 || getk("isclear") == 4) {
			var msg = "처리해줘야 할 임무는 아래와 같다."+enter+enter;
			msg += isitem ? "#r#e[일일 퀘스트] : 세르니움#n#k"+enter : "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 세르니움#n#k"+enter;
		...
		var msg = "처리해줘야 할 임무는 아래와 같다."+enter+enter;
		msg += isitem ? "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 세르니움#n#k"+enter : "#fUI/UIWindow2.img/UtilDlgEx/list0#\r\n#r#e[일일 퀘스트] : 세르니움#n#k"+enter;
		msg += isitem ? "\r\n목표 전리품 : #b#i"+getk("mobid")+"##z"+getk("mobid")+"# "+getk("mobq")+"개#k"+enter+enter : "처치 대상 : #b#o"+getk("mobid")+"# "+getk("mobq")+"마리#k"+enter+enter;
		msg += "그럼, 일이 끝난 후에 다시 찾아와라";
		cm.sendOk(msg);
		cm.dispose();
