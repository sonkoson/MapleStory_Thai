...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrevS("[#b컨텐츠 시스템#k -> #r"+서버이름+" 스킬#k]을 들어가면 자네의 성장에 필요한 다양한 #r스킬#k들이 있다네.", 0x04, 9401232);
    } else if (status == 2) {
        qm.sendNextPrevS("#r"+서버이름+" 스킬#k들은 각각 다른 효과를 가지고 있고, 스킬을 업그레이드 하면 할 수록 #b효과#k가 #e#b증폭#n#k 된다네.", 0x04, 9401232);
    } else if (status == 3) {
        qm.sendYesNoS("서론이 길었군 크크. 자네가 직접 스킬을 구매 해보게나.", 0x04, 9401232);
    } else if (status == 4) {
        qm.forceStartQuest(true);
        qm.sendOkS("다시 한번 말하지만 [#b컨텐츠 시스템#k -> #r"+서버이름+" 스킬#k]을 통해 강화를 진행하게나.", 4, 9401232);
        qm.dispose();
    }
}
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        if (qm.getClient().getQuestStatus(50001) == 2) {
            qm.dispose();
            return;
        }
        qm.sendNextS("#b"+서버이름+" 스킬#k은 마음에 드나? #r#i4310012:# #z4310012:##k을 이용하여 스킬들을 업그레이드 하면 분명 엄청난 성장을 할 수 있을거라네 크크.", 0x04, 9401232);
    } else if (status == 1) {
        말 = ""+서버이름+" 코인을 모아서 다른 스킬도 한번 구매해보게나. 크크.\r\n\r\n"
