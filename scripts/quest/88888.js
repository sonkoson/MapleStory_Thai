...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        var choose = "#fs11#자네가 받고싶은 칭호를 하나 선택해보게나!\r\n";
        ...
        if (selection == 0) {
            if (qm.haveItem(3700900, 1)) {
                qm.sendOkS("#fs11#" + 검정 + "흠, 자네는 이미 칭호를 가지고 있는것 같네. 만약 잃어버린다면 다시 지급해주도록 하겠네!", 0x04, 9401232);
                qm.dispose();
                return;
            }
            var choose = "#fs11#" + 검정 + " 칭호를 지급 했다네. 더 좋은 칭호를 얻기 위해 #fc0xFF050099#"+서버이름+" #k" + 검정 + "에서 열심히 즐겨보게나!\r\n";
            choose += "#fc0xFFD5D5D5#─────────────────────────────#k#l\r\n\r\n";
