...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNextS("크크. #h #, #r#i4310012:# #z4310012:##k은 #b"+서버이름+" 스킬#k 컨텐츠 뿐만 아니라 다양한 곳에도 사용되는 사실을 알고 있나?", 0x04, 9401232);
    } else if (status == 1) {
        qm.sendNextPrevS("물론 초반엔 빠르게 #b"+서버이름+" 스킬#k을 업그레이드 하는것도 좋지만 #r#e치장 강화#n#k 컨텐츠도 있다는걸 알려주고 싶다네. 크크.", 0x04, 9401232);
    } else if (status == 2) {
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        if (qm.getClient().getQuestStatus(50002) == 2) {
            qm.dispose();
            return;
        }
