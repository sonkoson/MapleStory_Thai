...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNextS("반갑네 #h #! #b#e"+서버이름+"#n#k 페스티벌을 즐기러 자네도 왔구만!", 0x04, 9401232);
    } else if (status == 1) {
        qm.sendNextPrevS("처음 접하더라도 너무 걱정하지 말게! 내가 자네를 위해 준비해둔 것들이 많이 있으니. 차근차근 하다보면 어느순간 자네도 #b#e디.잘.알#n#k이 될걸세 크크.", 0x04, 9401232);
    } else if (status == 2) {
        qm.sendNextPrevS("먼저 #b"+서버이름+" 페스티벌#k 광장에서 나를 찾아오면 자네에게 #r특별한 칭호#k를 주겠네!", 0x04, 9401232);
    } else if (status == 3) {
        qm.sendNextPrevS("#e\"@마을\"#n 명령어를 사용하면 #b"+서버이름+" 페스티벌 광장#k으로 올 수 있다네!", 0x04, 9401232);
    } else if (status == 4) {
        qm.sendYesNoS("하지만 이번엔 내가 특별히 자네를 이쪽으로 안내 해주겠네 #b"+서버이름+" 페스티벌 광장#k에서 이 슈피겔만 님을 찾게나 크크.\r\n\r\n#r※ 퀘스트 진행을 위해 맵이 이동 됩니다.", 4, 9401232);
    } else if (status == 5) {
        //qm.warp(100000051);
        qm.getClient().setCustomKeyValue(50000, "1", "1");
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        if (qm.getClient().getQuestStatus(50000) == 2) {
            qm.dispose();
            return;
        }
        ...
        if (Integer.parseInt(qm.getClient().getKeyValue(questt)) > 0) {
            qm.sendOk("#fs11#" + 검정 + "이미 보상을 받으셨습니다.", 9010061);
            qm.dispose();
            return;
        }
        qm.forceCompleteQuest(true);
        qm.getClient().setKeyValue(questt, "1");
        qm.gainItem(2633202, 1);
        qm.gainItem(4310012, 300);
        qm.getClient().setCustomKeyValue(50000, "1", "0");
        qm.dispose();
    }
}
