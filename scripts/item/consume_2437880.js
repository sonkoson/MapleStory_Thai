...
function action(mode, type, sel) {
    ...
    if (status == 0) {
        ...
        cm.sendGetNumber("#fs11#" + msg, 1, 1, cm.itemQuantity(2437880));
    } else if (status == 2) {
        itemid = Integer.parseInt("171200" + seld);
        if (cm.getInvSlots(1) >= sel * 2) {
            cm.gainItem(itemid, sel * 2);
            cm.gainItem(2437880, -(sel * 1));
            cm.sendOk("#fs11#축하합니다! 상자에서 #b#i" + itemid + "##z" + itemid + "# " + sel * 2 + "개#k가 나왔습니다!");
            cm.dispose();
        } else {
            cm.sendOk("#fs11#장비창을 비우신 후 다시 말을 걸어주세요.");
            cm.dispose();
            return;
        }
    }
