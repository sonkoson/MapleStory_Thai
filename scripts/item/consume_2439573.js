...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        var text = "#fs11#이봐 #e#h ##n! #b세린#k을 240레벨 달성했으니 선물 하나를 주겠네.\r\n";
        ...
        switch (cm.getPlayer().getJob()) {
            ...
            case 3612:
                ...
                break;
            default:
                cm.sendOkS("#fs11#자네는 전직을 하지 않았구만, 전직을 하고 다시 찾아오게.", 0x04, 9401232)
                break;
        }
        cm.sendSimpleS(text, 0x04, 9401232);
    } else if (status == 1) {
        item = selection;
        cm.sendYesNoS("#fs11##i" + item + "# #r#z" + item + "# 아이템#k을 정말로 받을텐가?\r\n\r\n\r\n#Cgray#잘못 선택시엔 다시 지급받기 어렵습니다.", 0x04, 9401232);
    } else if (status == 2) {
        var a = "";
        ...
        for (i = 0; i < 기본지급.length; i++) {
            ...
            if (기본지급[i] != 1190302) {
                ...
                out1.close();
                cm.sendOk("#fs11##r정상적인 접근 방법이 아닙니다.\r\n\r\n#d* 아주 잠깐의 달콤함을 위해 본연의 즐거움을 희생하시겠어요.?", 9401232);
                cm.dispose();
                return;
            }
