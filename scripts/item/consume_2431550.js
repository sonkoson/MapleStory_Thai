	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if (selection >= 1 && selection <=5) {
				cm.sendGetText("#fs11#선물 받을 닉네임을 알려줘!#r\r\n\r\n선물을 하기 위해선 같은 채널에 접속중이여야 해. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않아. 고유 아이템 선물 시 상대방이 이미 아이템을 보유 중일 경우 증발되니까 꼭 확인해줘.");
			} else if (selection == 6) {
				cm.sendOk("#fs11# 선물시스템은 아이템의 종류나 옵션에 관계없이 누구에게나 선물할수 있는 시스템이야. 선물을 하기 위해선 같은 채널에 접속중이여야 해. 잘못된 사용으로 인한 문제는 운영진 측에서 책임지지 않아.");
				cm.dispose();
			}
		} else if (status == 2) {
			if (operation == 1) {
