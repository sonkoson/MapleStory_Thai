...
function action(mode, type, sel) {
	...
	if (status == 0) {
		...
		if (!cm.canHold(items[0][0])) {
			cm.sendOk("#fs11#장비창을 1칸 이상 비운 뒤 다시 사용해보게!");
			cm.dispose();
			return;
		}
		if (cm.itemQuantity(need) < 15) {
			cm.sendOk("조각이 모자란거 같네.");
			cm.dispose();
			return;
		}
		...
		if (seld == 100) {
			...
			var msg = "받고싶은 장비가 이 장비가 확실한가?\r\n"
			msg += "#i" + items[getClass][seld] + "# #z" + items[getClass][seld] + "#\r\n\r\n"
			msg += "#r#e※주의※#k#n\r\n"
			msg += "한 번 조각을 아이템으로 교환하면 #r15개의 조각이 차감#k되며, 다시 다른 아이템으로 교환 할 수 없네.\r\n\r\n"
			msg += "#L0##b네 맞습니다.\r\n"
