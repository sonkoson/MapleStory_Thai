...
meso = Math.floor(Math.random() * 130000000) + 70000000 * 0.5 // 최소 10 최대 35 , 혼테일
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        cm.gainItem(2436616, -1);//아이템 사라지게
        cm.gainMeso(meso);
        cm.sendOk("#fs11# #z2436616#에서 " + Comma(meso) + "#k#n 메소를 얻었습니다!!");
        //World.Broadcast.broadcastMessage(CWvsContext.serverNotice(11, cm.getPlayer().getName() + "님이 상급 유니온 메소 박스에서 " + meso + " 메소를 얻었습니다!"));
        cm.dispose();
    }
}
